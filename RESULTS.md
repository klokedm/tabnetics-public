...
Results below are from the consolidated **Val-18** / **Val-19** / **Val-20** / **Val-21** evidence base — combined validation campaigns with **56,107** successful runs across **278** pipeline profiles.
...
| Total runs (dataset × seed × profile) | 56,107 |
| Datasets with BA ≥ 0.90 | 36 / 63 |
| Datasets with BA ≥ 0.80 | 50 / 63 |
| Perfect classification (BA = 1.0) | 26 datasets |
| SOTA comparison: above / within / below | 41 / 21 / 1 |

---

## Dataset difficulty spectrum

The 63 benchmark datasets span a wide difficulty range. Each bar shows the best balanced accuracy achieved across all 278 profiles.
...
The important read is directional rather than promotional: the newer FLAML, tune-first, ensemble, pipeline, and Val-21 decision-focused lanes are promising on focused slices, but the broad catalog anchor still comes from the mature Val-18 base.
...
|---|---|---|---|---|
| Direct (protocol-matched) | 10 | 6 | 4 | 0 |
| Discounted (close protocol) | 37 | 25 | 12 | 0 |
| Proxy (positioning only) | 16 | 10 | 5 | 1 |
| **Total** | **63** | **41** | **21** | **1** |

The consolidated Val-18 through Val-21 best profiles **exceed** published strict-holdout ranges on 41 of 63 datasets (65%) and reach or exceed them on 62 of 63 (98%). The single below-range dataset is a proxy-confidence genomic multi-class problem where small sample size and high class count push the pipeline’s limits.
