...
| Method | Reference and Tabnetics scope |
|---|---|
| Random over- and under-sampling | Batista, Prati & Monard. ["A study of the behavior of several methods for balancing machine learning training data."](https://doi.org/10.1145/1007730.1007735) *SIGKDD Explorations*, 6(1):20–29, 2004. Tabnetics provides seeded `random_over` duplication and no-replacement `random_under` deletion as simple, non-geometric controls. |
| SMOTE | Chawla et al. ["SMOTE: Synthetic Minority Over-sampling Technique."](https://doi.org/10.1613/jair.953) *JAIR*, 16:321–357, 2002. Tabnetics limits this adapter to dense finite continuous train-fold matrices and never places synthetic rows in executable row context. |
| Predictive propensity matching | Rosenbaum & Rubin. ["The central role of the propensity score in observational studies for causal effects."](https://doi.org/10.1093/biomet/70.1.41) *Biometrika*, 70(1):41–55, 1983, provides the score-and-balance foundation. Tabnetics adapts cross-fitted score matching only as a binary predictive class-balancing primitive; it does not define a treatment, causal estimand, or treatment-effect claim. |

All enabled adapters are default-off and are fitted separately inside each
classifier-CV training fold; validation and test prevalence is untouched.
Evidence review for the bounded public surface deferred `borderline_smote` and
`smote_tomek` pending noise, retention, calibration, and multiclass portfolio
evidence. ADASYN, SMOTE-ENN, NearMiss, KMeans-SMOTE, SVM-SMOTE, inverse/overlap
weighting, Balanced Random Forest, and EasyEnsemble are excluded from this
generic sampler layer. The last two are classifier families; weighting requires
a separate non-causal contract; the geometric/adaptive methods need dedicated
failure-mode evidence before inclusion.
...
| HDRDA-style regularized DA | Yata & Aoshima. ["Effective PCA for high-dimension, low-sample-size data with noise reduction via geometric representations."](https://doi.org/10.1016/j.jmva.2011.09.011) *J. Multivariate Analysis*, 105:193–215, 2012; Aoshima & Yata. ["Two-stage procedures for high-dimensional data."](https://doi.org/10.1080/07474946.2011.619088) *Sequential Analysis*, 30(4):356–399, 2011. `hdrda` is a lightweight internal backend anchored to this HDLSS regularization/noise-reduction line rather than a byte-for-byte reproduction of a single package. |
| Distance-Weighted Discrimination (DWD) | Marron et al. ["Distance-weighted discrimination."](https://doi.org/10.1198/016214507000000677) *JASA*, 102(480):1267–1271, 2007. The `dwd_classifier` backend follows the DWD family and uses a generalized-mean-distance style optimization path for practical sklearn compatibility. |
| ECOC multiclass wrappers | Dietterich & Bakiri. ["Solving multiclass learning problems via error-correcting output codes."](https://doi.org/10.1613/jair.105) *JAIR*, 2:263–286, 1995. `ecoc_hdrda`, `ecoc_dwd`, and `ecoc_svm_linear` wrap binary-capable HDLSS backends with an ECOC scaffold. |
| Random Fourier Features + LR | Rahimi & Recht. ["Random Features for Large-Scale Kernel Machines."](https://papers.neurips.cc/paper/3182-random-features-for-large-scale-kernel-machines) *NeurIPS*, 2007. `rff_lr` adds a controlled nonlinear kernel approximation without leaving the linear-model training regime. |
| Nearest subspace classifier | Tsuda. ["Subspace classifier in the Hilbert space."](https://doi.org/10.1016/S0167-8655(99)00023-9) *Pattern Recognition Letters*, 20(5):513–519, 1999. `near_subspace` is the classical nearest-subspace / reconstruction-error family adapted to the current HDLSS pipeline. |
| Spatial-median DA | Hall, Titterington & Xue. ["Median-Based Classifiers for High-Dimensional Data."](https://doi.org/10.1198/jasa.2009.tm08107) *JASA*, 104(488):1597–1608, 2009. `spatial_median_da` is a lightweight robust distance classifier anchored to spatial/geometric-median HDLSS work rather than an exact reproduction of a single published estimator. |
| Copula discriminant analysis | Han, Zhao & Liu. ["CODA: High Dimensional Copula Discriminant Analysis."](https://jmlr.org/papers/v14/han13a.html) *JMLR*, 14:629–671, 2013; Tekle & de Leon. ["Gaussian copula distributions for mixed data, with application in discrimination."](https://doi.org/10.1080/00949655.2015.1077386) *J. Statistical Computation and Simulation*, 86(9):1643–1659, 2016. `copula_da` is a simplified Gaussian-copula-style backend intended for the pipeline's CDF-to-Gaussian feature space. |
