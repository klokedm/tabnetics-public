...
def test_sklearn_backend_records_svc_probability_runtime_telemetry():
    X, y, n_classes, counts = _toy_data(n_samples=78, n_features=14, n_classes=2, seed=212)
    backend = SklearnBackend(
        candidate_names=("svm_rbf", "svm_linear", "vote_ensemble"),
        enable_svc_probability=True,
    )
    ...
    assert meta["model_cv_svc_probability_enabled"] is True
    assert set(meta["model_cv_svc_probability_candidates"]) == {
        "svm_rbf",
        "svm_linear",
        "vote_ensemble",
    }
    timings = dict(meta["model_cv_candidate_wall_seconds"])
    assert set(timings) >= {"svm_rbf", "svm_linear", "vote_ensemble"}
    assert all(float(value) >= 0.0 for value in timings.values())
    assert "internal probability calibration" in meta["model_cv_svc_probability_runtime_note"]
...
def test_ablatable_cada_tent_and_hinge_variants_produce_valid_probabilities():
    X, y, _, _ = _toy_data(n_samples=96, n_features=28, n_classes=3, seed=38)
    for basis, kwargs in (
        ("tent", {"n_steps": 1}),
        ("tent", {"n_steps": 2}),
        ("hinge", {"control_knots": (1.0 / 3.0, 2.0 / 3.0)}),
    ):
        model = AblatableCADAClassifier(basis=basis, **kwargs)
        model.fit(X, y)
        probs = model.predict_proba(X[:10])
        assert probs.shape == (10, 3)
        np.testing.assert_allclose(np.sum(probs, axis=1), np.ones(10), atol=1e-6)
...
def test_ablatable_cada_screens_augmented_coordinates_to_budget():
    X, y, _, _ = _toy_data(n_samples=64, n_features=40, n_classes=2, seed=39)
    model = AblatableCADAClassifier(
        basis="tent",
        n_steps=2,
        max_aug_features=11,
    )
    model.fit(X, y)
    assert int(model.n_augmented_features_) == 80
    assert int(model.n_selected_augmented_features_) == 11
    assert len(model.selected_aug_indices_) == 11


@pytest.mark.parametrize("candidate_name", ("cada_tent1", "cada_tent2", "cada_hinge2"))
def test_sklearn_backend_supports_ablatable_cada_candidates(candidate_name):
    X, y, n_classes, counts = _toy_data(n_samples=96, n_features=36, n_classes=3, seed=44)
    backend = SklearnBackend(candidate_names=(candidate_name,))
    model, name, score, std, n_splits, meta = backend.fit_and_select(
        X, y, seed=5, n_classes=n_classes, class_counts=counts
    )
    ...
    assert np.isfinite(std) or np.isnan(std)
    assert int(n_splits) >= 0
    assert candidate_name in tuple(meta.get("model_cv_evaluated_candidates", ()))
...
def test_mnpo_classifier_diakrino_family_meta_is_default_off_and_report_only(tmp_path):
    pd = pytest.importorskip("pandas")
    gamma = np.full(36, -7.0, dtype=float)
    gamma[4] = 7.0
    discrete = np.full(36, -7.0, dtype=float)
    discrete[31] = 7.0
    ...
    pd.DataFrame(
        {
            "feature_index": [0, 1, 2],
            "chunk_id": [0, 0, 1],
            "population_family_logits": [gamma, discrete, gamma],
            "feature_selection_logit": [1.0, 3.0, 5.0],
        }
    ).to_parquet(path, index=False)

    disabled = MNPOClassifierBackend(candidate_names=("lr",), use_per_family_flaml=False)
    enabled = MNPOClassifierBackend(
        candidate_names=("lr",),
        oracle_include_diakrino_family_meta=True,
        diakrino_sidecar_path=str(path),
        use_per_family_flaml=False,
    )

    assert disabled._diakrino_family_meta_features(n_features=3) == {"enabled": False}
    meta = enabled._diakrino_family_meta_features(n_features=3)
    assert meta["enabled"] is True
    assert meta["loaded"] is True
    assert meta["n_features_match"] is True
    assert meta["summary"]["family"]["discrete_skip_count"] == 1
    assert meta["summary"]["selection_dispersion"]["calibration"] == "chunk_zscore"
    assert meta["summary"]["selection_dispersion"]["score_column"] == "feature_selection_logit"
...
def test_flaml_fit_and_select_raises_import_error_when_missing(monkeypatch):
    X, y, n_classes, counts = _toy_data(n_samples=90, n_features=10, n_classes=2, seed=7)
    backend = FLAMLBackend()
    monkeypatch.setitem(sys.modules, "flaml", None)
...
class TestHDRDAClassifier:
    def test_predict_proba_rows_sum_to_one(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=10)
        clf = HDRDAClassifier()
        clf.fit(X, y)
    ...
    def test_predict_returns_valid_classes(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=11)
        clf = HDRDAClassifier()
        clf.fit(X, y)
        preds = clf.predict(X)
        assert set(preds).issubset(set(np.unique(y)))

    def test_hdlss_extreme_regime(self):
        # p >> n scenario.
        X, y, _, _ = _toy_data(n_samples=30, n_features=500, n_classes=2, seed=12)
        clf = HDRDAClassifier(alpha=0.3)
        clf.fit(X, y)
        proba = clf.predict_proba(X)
        assert proba.shape == (30, 2)
        np.testing.assert_allclose(proba.sum(axis=1), 1.0, atol=1e-6)

    def test_sklearn_backend_supports_hdrda(self):
        X, y, n_classes, counts = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=13)
        backend = SklearnBackend(candidate_names=("hdrda",))
        model, name, score, std, n_splits, meta = backend.fit_and_select(
            X, y, seed=0, n_classes=n_classes, class_counts=counts,
        )
        assert name == "hdrda"
        assert score > 0.0
...
class TestDWDClassifier:
    @staticmethod
    def _separable_binary(seed=0):
        rng = np.random.default_rng(seed)
        X = np.vstack([
            rng.normal(-3.0, 0.2, size=(27, 2)),
            rng.normal(3.0, 0.2, size=(7, 2)),
        ])
        y = np.array(["reference"] * 27 + ["target"] * 7)
        return X, y

    def test_binary_score_orientation_is_semantic_and_row_order_independent(self):
        X, y = self._separable_binary(seed=1)
        expected = y.copy()

        for order in (np.arange(y.size), np.arange(y.size)[::-1]):
            clf = DWDClassifier(random_state=42, max_iter=1000).fit(X[order], y[order])
            scores = clf.decision_function(X)
            predictions = clf.predict(X)
            probabilities = clf.predict_proba(X)

            np.testing.assert_array_equal(predictions, expected)
            assert np.all(scores[y == clf.classes_[1]] > 0)
            assert np.all(scores[y == clf.classes_[0]] < 0)
            np.testing.assert_array_equal(
                clf.classes_[np.argmax(probabilities, axis=1)],
                predictions,
            )

    def test_multiclass_ova_scores_point_toward_each_target_class(self):
        rng = np.random.default_rng(2)
        centers = np.array([[-5.0, 0.0], [0.0, 5.0], [5.0, 0.0]])
        labels = np.array(["alpha", "middle", "zeta"])
        X = np.vstack([
            rng.normal(center, 0.2, size=(12, 2))
            for center in centers
        ])
        y = np.repeat(labels, 12)

        clf = DWDClassifier(random_state=7, max_iter=1000).fit(X[::-1], y[::-1])
        scores = clf.decision_function(X)
        predictions = clf.predict(X)
        probabilities = clf.predict_proba(X)
        ...
        np.testing.assert_array_equal(
            clf.classes_[np.argmax(scores, axis=1)],
            predictions,
        )
        np.testing.assert_array_equal(
            clf.classes_[np.argmax(probabilities, axis=1)],
        ...
        for class_index, label in enumerate(clf.classes_):
            class_scores = scores[y == label].mean(axis=0)
            assert np.argmax(class_scores) == class_index

    def test_fixed_seed_and_pickle_round_trip_preserve_scores(self):
        X, y = self._separable_binary(seed=3)
        first = DWDClassifier(random_state=19).fit(X, y)
        second = DWDClassifier(random_state=19).fit(X, y)
        restored = pickle.loads(pickle.dumps(first))

        np.testing.assert_allclose(
            first.decision_function(X),
            second.decision_function(X),
        )
        np.testing.assert_allclose(
    ...
    def test_binary_zero_score_uses_probability_argmax_tie_rule(self):
        X, y = self._separable_binary(seed=4)
        clf = DWDClassifier(random_state=23).fit(X, y)
        clf._binary_models[0]["w"] = np.zeros(X.shape[1], dtype=float)
        clf._binary_models[0]["beta"] = 0.0

        scores = clf.decision_function(X[:2])
        ...
        np.testing.assert_array_equal(predictions, np.repeat(clf.classes_[0], 2))
        np.testing.assert_array_equal(
            clf.classes_[np.argmax(probabilities, axis=1)],
            predictions,
        )
    ...
    def test_legacy_orientation_models_require_refit(self, n_classes):
        X, y, _, _ = _toy_data(
            n_samples=60,
            n_features=10,
            n_classes=n_classes,
        ...
        clf = DWDClassifier(random_state=29).fit(X, y)
        for model in clf._binary_models:
            model["class_neg"] = clf.classes_[0]
        restored = pickle.loads(pickle.dumps(clf))

        with pytest.raises(RuntimeError, match="legacy ambiguous score orientation"):
            restored.predict(X)

    def test_predict_proba_binary_rows_sum_to_one(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=2, seed=20)
        clf = DWDClassifier(random_state=42)
        clf.fit(X, y)
        proba = clf.predict_proba(X)
        assert proba.shape == (X.shape[0], 2)
        np.testing.assert_allclose(proba.sum(axis=1), 1.0, atol=1e-6)

    def test_predict_proba_multiclass_rows_sum_to_one(self):
        X, y, _, _ = _toy_data(n_samples=80, n_features=200, n_classes=3, seed=21)
        clf = DWDClassifier(random_state=42)
        clf.fit(X, y)
        proba = clf.predict_proba(X)
        assert proba.shape == (X.shape[0], 3)
    ...
    def test_predict_returns_valid_classes(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=2, seed=22)
        clf = DWDClassifier(random_state=42)
        clf.fit(X, y)
        preds = clf.predict(X)
        assert set(preds).issubset(set(np.unique(y)))

    def test_sklearn_backend_supports_dwd(self):
        X, y, n_classes, counts = _toy_data(n_samples=60, n_features=200, n_classes=2, seed=23)
        backend = SklearnBackend(candidate_names=("dwd_classifier",))
        model, name, score, std, n_splits, meta = backend.fit_and_select(
            X, y, seed=0, n_classes=n_classes, class_counts=counts,
        )
        assert name == "dwd_classifier"
        assert score > 0.0
...
class TestSparsePLSDAClassifier:
    def test_predict_proba_rows_sum_to_one(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=30)
        clf = SparsePLSDAClassifier(max_components=3, loading_sparsity=0.5)
        clf.fit(X, y)
        proba = clf.predict_proba(X)
        assert proba.shape == (X.shape[0], 3)
        np.testing.assert_allclose(proba.sum(axis=1), 1.0, atol=1e-6)

    def test_sparse_loading_property(self):
        """Verify that fewer features are active than total p."""
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=31)
        clf = SparsePLSDAClassifier(max_components=3, loading_sparsity=0.5)
        clf.fit(X, y)
        assert clf.n_selected_features_ < 200

    def test_predict_returns_valid_classes(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=32)
        clf = SparsePLSDAClassifier(max_components=3)
        clf.fit(X, y)
        preds = clf.predict(X)
        assert set(preds).issubset(set(np.unique(y)))

    def test_sklearn_backend_supports_spls_da(self):
        X, y, n_classes, counts = _toy_data(n_samples=60, n_features=200, n_classes=3, seed=33)
        backend = SklearnBackend(candidate_names=("spls_da_classifier",))
        model, name, score, std, n_splits, meta = backend.fit_and_select(
            X, y, seed=0, n_classes=n_classes, class_counts=counts,
        )
        assert name == "spls_da_classifier"
        assert score > 0.0
...
class TestECOCClassifier:
    def test_binary_passthrough(self):
        X, y, _, _ = _toy_data(n_samples=60, n_features=50, n_classes=2, seed=40)
        clf = ECOCClassifier(
            base_estimator=BiasCorrectedLinearSVM(random_state=42),
            random_state=42,
        )
        clf.fit(X, y)
        proba = clf.predict_proba(X)
    ...
    def test_multiclass_predict_proba_rows_sum_to_one(self):
        X, y, _, _ = _toy_data(n_samples=120, n_features=50, n_classes=5, seed=41)
        clf = ECOCClassifier(
            base_estimator=BiasCorrectedLinearSVM(random_state=42),
            random_state=42,
        )
    ...
    def test_predict_returns_valid_classes(self):
        X, y, _, _ = _toy_data(n_samples=120, n_features=50, n_classes=5, seed=42)
        clf = ECOCClassifier(random_state=42)
        clf.fit(X, y)
        preds = clf.predict(X)
        assert set(preds).issubset(set(np.unique(y)))

    def test_sklearn_backend_supports_ecoc_hdrda(self):
        X, y, n_classes, counts = _toy_data(n_samples=120, n_features=100, n_classes=5, seed=43)
        backend = SklearnBackend(candidate_names=("ecoc_hdrda",))
        model, name, score, std, n_splits, meta = backend.fit_and_select(
            X, y, seed=0, n_classes=n_classes, class_counts=counts,
        )
        assert name == "ecoc_hdrda"
        assert score > 0.0

    def test_sklearn_backend_supports_ecoc_svm_linear(self):
        X, y, n_classes, counts = _toy_data(n_samples=120, n_features=100, n_classes=5, seed=44)
        backend = SklearnBackend(candidate_names=("ecoc_svm_linear",))
        model, name, score, std, n_splits, meta = backend.fit_and_select(
            X, y, seed=0, n_classes=n_classes, class_counts=counts,
        )
        assert name == "ecoc_svm_linear"
        assert score > 0.0


# ---------- Complexity prior coverage ----------


def test_new_classifiers_in_complexity_prior():
    for name in ("hdrda", "dwd_classifier", "spls_da_classifier",
                 "ecoc_hdrda", "ecoc_dwd", "ecoc_svm_linear"):
        assert name in CLASSIFIER_COMPLEXITY_PRIOR, f"{name} missing from CLASSIFIER_COMPLEXITY_PRIOR"
