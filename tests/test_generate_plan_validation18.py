...
def test_build_jobs_validation18_mnpo_matches_val18_plan_profiles(tmp_path):
    ...
    n43_job = by_profile["N43_core_banzhaf_no_worst_class"][0]
    n43_extra_args = list(n43_job.params.get("extra_args") or [])
    n43_args = _extra_args_by_flag(n43_job)
    assert n43_extra_args.count("--classifier-selection-mode") == 1
    assert n43_args["--classifier-selection-mode"] == "mnpo_hybrid"
    assert n43_args["--classifier-oracle-k"] == "2"
    assert n43_args["--classifier-oracle-weighting-mode"] == "tritrust"
    assert "--enable-classifier-oracle-ensemble" in n43_args
    assert "--disable-classifier-oracle-worst-class" in n43_args
    parser = build_arg_parser()
    parsed_n43 = parser.parse_args(
        [
            "--datasets",
            "colon_alon",
            "--fs-method-set",
            ...
            "--output-dir",
            str(tmp_path / "n43_parse"),
            *n43_extra_args,
        ]
    )
    assert str(getattr(parsed_n43, "classifier_selection_mode", "")) == "mnpo_hybrid"
    assert bool(getattr(parsed_n43, "disable_classifier_oracle_worst_class", False)) is True

    n44_args = _extra_args_by_flag(by_profile["N44_core_banzhaf_fs_stability"][0])
    assert n44_args["--classifier-selection-mode"] == "legacy"
    assert "--disable-fs-stability-oracle" not in n44_args
    assert "--disable-fs-complexity-oracle" not in n44_args
    assert "--disable-fs-robust-oracle" in n44_args
    assert "--disable-classifier-oracle-worst-class" not in n44_args
...
def test_build_jobs_validation18_classifiers_adds_tabpfn_full64_lane(tmp_path):
    ...
    assert all(list(j.params.get("preferred_hosts") or []) == ["public-gpu-host"] for j in c10_jobs)
...
def test_validation19_host_assignment_uses_conservative_cpu_only_worker_targets(tmp_path):
    ...
    assert "public-gpu-host" not in host_summary
...
def test_build_jobs_validation20_wave1_matches_plan_profiles(tmp_path):
    ...
    assert list(b05_job.params.get("preferred_hosts") or []) == ["public-gpu-host"]
...
def test_validation20_wave1_host_assignment_uses_three_host_policy(tmp_path):
    ...
    assert "public-gpu-host" in host_summary
...
def test_validation21_phase1_requires_resolved_spec(tmp_path):
    unresolved_path = tmp_path / "val21_spec_unresolved.json"
    unresolved_path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "winner_profile_id": "__winner_profile_id__",
                "winner_fs_method_set": "__winner_fs_method_set__",
                "winner_execution_lane": "__winner_execution_lane__",
                "winner_extra_args": ["__winner_extra_args__"],
                "current_default_profile_id": "__current_default_profile_id__",
                "current_default_fs_method_set": "__current_default_fs_method_set__",
                "current_default_execution_lane": "__current_default_execution_lane__",
                "current_default_extra_args": ["__current_default_extra_args__"],
                "lockout_fallback_fs_method_set": "__lockout_fallback_fs_method_set__",
                "meta_learning_records_path": "__meta_learning_records_path__",
                "tier_classifier_model_path": "__tier_classifier_model_path__",
                "phase2_profile_ids": ["__phase2_profile_ids__"],
            },
            indent=2,
        )
        + "\n",
...
def test_build_jobs_validation21_phase1_matches_manifest(tmp_path):
    ...
    assert list(lockout_job.params.get("preferred_hosts") or []) == ["host1.example.com"]
    ...
    assert set((assignment.get("host_summary") or {}).keys()) == {"host1.example.com", "public-gpu-host"}
