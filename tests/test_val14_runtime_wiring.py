...
def test_dffsconfig_candidate_addition_normalization():
    cfg = DFFSConfig(
        batch_correction="combatseq",
        multiomics_adapter="diablo_blocks",
        selection_strategy="legacy_voting",
        fs_stability_threshold_method="cpss",
        fs_stability_target_pfer=-5.0,
    )
    assert cfg.batch_correction == "combat_seq"
    assert cfg.multiomics_adapter == "metadata_blocks"
    assert cfg.selection_strategy == "legacy_voting"
    assert cfg.fs_stability_threshold_method == "cpss"
    assert cfg.fs_stability_target_pfer == pytest.approx(1e-6)
...
def test_val14_parser_accepts_new_val14_flags():
    args = benchmark.build_arg_parser().parse_args(
        [
            ...
            "/tmp/meta_learning_records.json",
    ...
    assert str(args.meta_learning_records_path) == "/tmp/meta_learning_records.json"
    assert str(args.tier_classifier_mode) == "learned"
    assert str(args.tier_classifier_model_path) == "/tmp/tier_classifier_model.json"
...
def test_parser_accepts_flag_selector_v1_mode():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
            ...
            "flag_selector_v1",
            "--meta-learning-records-path",
            "/tmp/flag_selector_artifact",
        ]
    )
    assert str(args.meta_learning_selector) == "flag_selector_v1"
    assert str(args.meta_learning_records_path) == "/tmp/flag_selector_artifact"


def test_parser_accepts_candidate_addition_flags():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
    ...
    assert str(args.batch_correction) == "combatseq"
    assert str(args.multiomics_adapter) == "diablo_blocks"
    assert str(args.selection_strategy) == "legacy_voting"
    assert str(args.fs_stability_threshold_method) == "cpss"
    assert float(args.fs_stability_target_pfer) == pytest.approx(0.75)
    assert bool(args.include_rp_ensemble_model) is True
...
def test_build_base_config_maps_val14_runtime_fields():
    ...
    assert str(cfg.meta_learning_records_path) == "/tmp/meta_learning_records.json"
...
def test_build_base_config_maps_flag_selector_v1_mode():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
    ...
    spec = benchmark.BENCHMARK_DATASETS["synthetic_medium_mixed"]
    cfg = benchmark._build_base_config(args, spec, seed=11)
    assert str(cfg.meta_learning_selector_mode) == "flag_selector_v1"
    assert str(cfg.meta_learning_records_path) == "/tmp/flag_selector_artifact"


def test_build_base_config_maps_candidate_addition_fields():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
    ...
    assert str(cfg.batch_correction) == "combat_seq"
    assert str(cfg.multiomics_adapter) == "metadata_blocks"
    assert str(cfg.selection_strategy) == "legacy_voting"
    assert str(cfg.fs_stability_threshold_method) == "cpss"
    assert float(cfg.fs_stability_target_pfer) == pytest.approx(0.75)
    assert bool(cfg.include_rp_ensemble_model) is True
    assert bool(cfg.classification.include_rp_ensemble_model) is True


def test_build_base_config_maps_val20_classifier_runtime_overrides():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
    ...
    assert bool(cfg.use_rank_prefilter) is False
    assert float(cfg.prefilter_bh_ttest_alpha) == pytest.approx(0.20)
    assert tuple(cfg.exclude_model_candidates) == ("tabpfn",)
    assert tuple(cfg.classifier_regime_candidate_exclusions) == (
        "hdlss_moderate:tabpfn",
        "standard:tabpfn",
    )
    assert tuple(cfg.classifier_oracle_complexity_prior_overrides) == (
        "lr=0.75",
        "svm_linear=0.85",
    )
    assert tuple(cfg.classification.exclude_model_candidates) == ("tabpfn",)
    assert tuple(cfg.classification.regime_candidate_exclusions) == (
        "hdlss_moderate:tabpfn",
        "standard:tabpfn",
    )
    assert tuple(cfg.classification.oracle_complexity_prior_overrides) == (
        "lr=0.75",
        "svm_linear=0.85",
    )
...
def test_pipeline_snapshot_includes_candidate_addition_fields():
    cfg = DFFSConfig(
        batch_correction="combat_seq",
        multiomics_adapter="metadata_blocks",
        multiomics_feature_blocks={"rna": (0, 1), "protein": (2, 3)},
        selection_strategy="legacy_voting",
        fs_stability_threshold_method="cpss",
        fs_stability_target_pfer=0.5,
        include_rp_ensemble_model=True,
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    snap = pipe._config_snapshot()
    assert str(snap.get("batch_correction", "")) == "combat_seq"
    assert str(snap.get("multiomics_adapter", "")) == "metadata_blocks"
    assert snap.get("multiomics_feature_blocks", {}) == {"rna": [0, 1], "protein": [2, 3]}
    assert str(snap.get("selection_strategy", "")) == "legacy_voting"
    assert str(snap.get("fs_stability_threshold_method", "")) == "cpss"
    assert float(snap.get("fs_stability_target_pfer", 0.0)) == pytest.approx(0.5)
    assert bool(snap.get("include_rp_ensemble_model", False)) is True


def test_pipeline_snapshot_includes_val20_classifier_runtime_fields():
    cfg = DFFSConfig(
        prefilter_bh_ttest_alpha=0.20,
        exclude_model_candidates=("tabpfn",),
        classifier_regime_candidate_exclusions=("hdlss_moderate:tabpfn", "standard:tabpfn"),
        classifier_oracle_complexity_prior_overrides=("lr=0.75",),
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    snap = pipe._config_snapshot()
    assert float(snap.get("prefilter_bh_ttest_alpha", 0.0)) == pytest.approx(0.20)
    assert snap.get("exclude_model_candidates", []) == ["tabpfn"]
    assert snap.get("classifier_regime_candidate_exclusions", []) == [
        "hdlss_moderate:tabpfn",
        "standard:tabpfn",
    ]
    assert snap.get("classifier_oracle_complexity_prior_overrides", []) == ["lr=0.75"]
