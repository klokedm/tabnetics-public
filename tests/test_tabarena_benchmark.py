...
from experiments.benchmarking.tabarena_benchmark import (  # noqa: E402
    ...
    _GENERAL_TABULAR_COMPETITIVE_FLAML_ESTIMATORS,
    _GENERAL_TABULAR_COMPETITIVE_MODEL_CANDIDATES,
    _GENERAL_TABULAR_FLAML_ESTIMATORS,
...
def test_general_tabular_profile_uses_tree_weighted_pool_and_skips_hdlss() -> None:
    ...
    assert cfg.classification.backend == "flaml"
    assert cfg.classification.flaml_estimator_list == _GENERAL_TABULAR_FLAML_ESTIMATORS
    assert "copula_da" in cfg.classification.flaml_estimator_list
    assert cfg.classification.use_hybrid_score is True
    assert cfg.classification.runtime_containment_enabled is False
...
def test_general_tabular_competitive_profile_uses_metric_aligned_tune_first_stack() -> None:
    args = build_arg_parser().parse_args(["--profile", "general_tabular_competitive"])
    spec = next(iter(TABARENA_DATASETS.values()))

    cfg = _build_tabarena_config(spec, seed=42, args=args)

    assert cfg.classification.model_candidates == _GENERAL_TABULAR_COMPETITIVE_MODEL_CANDIDATES
    assert cfg.classification.flaml_estimator_list == _GENERAL_TABULAR_COMPETITIVE_FLAML_ESTIMATORS
    assert "tabm" in cfg.classification.model_candidates
    assert "realmlp" in cfg.classification.model_candidates
    assert "tabm" in cfg.classification.flaml_estimator_list
    assert "realmlp" in cfg.classification.flaml_estimator_list
    assert "tabm_official" not in cfg.classification.model_candidates
    assert "realmlp_td" not in cfg.classification.model_candidates
    assert cfg.classification.selection_mode == "tune_first"
    assert cfg.classification.backend == "flaml"
    assert cfg.classification.use_hybrid_score is False
    assert cfg.classification.flaml_metric == "roc_auc"
    assert cfg.classification.oracle_enable_ensemble is True
    assert cfg.classification.oracle_ensemble_voting_mode == "soft"
    assert cfg.classification.oracle_greedy_ensemble is True
    assert cfg.classification.oracle_candidate_pruning is True
    assert cfg.classification.oracle_incumbent_early_stopping is True


def test_general_tabular_competitive_profile_switches_multiclass_metric_to_log_loss() -> None:
    from dataclasses import replace as dc_replace
    ...
    spec = dc_replace(
        ...
        n_classes=max(3, int(base_spec.n_classes)),
    )

    cfg = _build_tabarena_config(spec, seed=42, args=args)

    assert cfg.classification.flaml_metric == "log_loss"
    assert cfg.classification.oracle_ensemble_voting_mode == "soft"


def test_general_tabular_competitive_cli_overrides_wire_new_oracle_controls() -> None:
    args = build_arg_parser().parse_args(
        [
            "--profile", "general_tabular_competitive",
            "--classifier-oracle-ensemble-voting-mode", "hard",
            "--enable-classifier-oracle-greedy-ensemble",
            "--classifier-oracle-greedy-ensemble-rounds", "7",
            "--enable-classifier-oracle-candidate-pruning",
            "--classifier-oracle-candidate-pruning-threshold", "0.05",
            "--enable-classifier-oracle-incumbent-early-stopping",
            "--classifier-flaml-metric", "macro_f1",
            "--disable-hybrid-score",
        ]
    )
    spec = next(iter(TABARENA_DATASETS.values()))
    ...
    assert cfg.classification.oracle_ensemble_voting_mode == "hard"
    assert cfg.classification.oracle_greedy_ensemble is True
    assert cfg.classification.oracle_greedy_ensemble_rounds == 7
    assert cfg.classification.oracle_candidate_pruning is True
    assert cfg.classification.oracle_candidate_pruning_threshold == 0.05
    assert cfg.classification.oracle_incumbent_early_stopping is True
    assert cfg.classification.flaml_metric == "macro_f1"
    assert cfg.classification.use_hybrid_score is False


def test_tabarena_metric_aligned_override_applies_to_legacy_profile() -> None:
    from dataclasses import replace as dc_replace

    args = build_arg_parser().parse_args(
        [
            "--profile", "general_tabular",
            "--tabarena-metric-aligned-tuning",
        ]
    )
    base_spec = next(iter(TABARENA_DATASETS.values()))
    spec = dc_replace(
