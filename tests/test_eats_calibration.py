...
from tabnetics.feature_selection.methods.stability_selection import (
    CPSSThresholdCalibrator,
    EATSThresholdCalibrator,
    cpss_calibrate_threshold,
...
class TestCPSSThresholdCalibrator:
    """Verify CPSS PFER-based threshold calibration."""

    def test_basic_calibration_matches_closed_form(self):
        scores = _make_stability_scores()
        cal = CPSSThresholdCalibrator(target_pfer=2.0, fallback_threshold=0.6)
        threshold, meta = cal.calibrate(scores, avg_selected_per_fit=5.0, n_features=50)
        expected = 0.5 * (1.0 + ((5.0 ** 2) / (2.0 * 50.0)))
        assert threshold == pytest.approx(expected)
        assert meta["cpss_calibrated"] is True
        assert meta["cpss_used_fallback"] is False
        assert meta["cpss_bound_feasible"] is True

    def test_missing_q_estimate_uses_fallback(self):
        scores = _make_stability_scores()
        cal = CPSSThresholdCalibrator(target_pfer=1.0, fallback_threshold=0.61)
        threshold, meta = cal.calibrate(scores, avg_selected_per_fit=None, n_features=50)
        assert threshold == pytest.approx(0.61)
        assert meta["cpss_used_fallback"] is True
        assert meta["cpss_reason"] == "missing_q_estimate"

    def test_infeasible_bound_clips_high(self):
        scores = _make_stability_scores()
        cal = CPSSThresholdCalibrator(target_pfer=0.2, fallback_threshold=0.6, max_threshold=0.99)
        threshold, meta = cal.calibrate(scores, avg_selected_per_fit=12.0, n_features=20)
        assert threshold == pytest.approx(0.99)
        assert meta["cpss_calibrated"] is True
        assert meta["cpss_bound_feasible"] is False

    def test_function_wrapper_matches_class_api(self):
        scores = _make_stability_scores()
        cal = CPSSThresholdCalibrator(target_pfer=1.5, fallback_threshold=0.58)
        t_class, m_class = cal.calibrate(scores, avg_selected_per_fit=4.0, n_features=50)
        t_func, m_func = cpss_calibrate_threshold(
            scores,
            avg_selected_per_fit=4.0,
            n_features=50,
            target_pfer=1.5,
...
class TestStabilityThresholdConfig:
    ...
    def test_config_field_default_fixed(self):
        ...
        assert cfg.fs_stability_target_pfer == pytest.approx(1.0)
