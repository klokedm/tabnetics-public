...
def test_regime_gating_samples_per_class_boundary_matches_val12_threshold(monkeypatch):
    X_low, y_low = _xy(n_samples=12, n_features=40, seed=29)  # n/c = 6
    low_pipe = DistributionFeatureSelectionPipeline(
        _base_config(
            regime_gating_enabled=True,
            regime_gating_difficulty_source="historical",
            regime_gating_simple_methods=("linear_svm", "anova_f"),
            regime_gating_very_hard_min_classes=2,
        )
    )
    _stub_catalog_tier(monkeypatch, low_pipe, "medium")
    low_policy = low_pipe._resolve_method_policy("mock", X_low, y_low)
    assert bool(low_policy["regime_policy_applied"]) is True
    assert str(low_policy["regime_policy_mode"]) == "very_hard_fallback"

    X_high, y_high = _xy(n_samples=16, n_features=40, seed=31)  # n/c = 8
    high_pipe = DistributionFeatureSelectionPipeline(
        _base_config(
            regime_gating_enabled=True,
            regime_gating_difficulty_source="historical",
            regime_gating_simple_methods=("linear_svm", "anova_f"),
    ...
    _stub_catalog_tier(monkeypatch, high_pipe, "medium")
    high_policy = high_pipe._resolve_method_policy("mock", X_high, y_high)
    assert bool(high_policy["regime_policy_applied"]) is False


def test_regime_gating_expanded_feature_trigger_uses_fisher_and_n1(monkeypatch):
    X, y = _xy(n_samples=60, n_features=100, seed=37)
    pipe = DistributionFeatureSelectionPipeline(
        _base_config(
            regime_gating_enabled=True,
            regime_gating_difficulty_source="meta_features",
            regime_gating_use_expanded_features=True,
            regime_gating_simple_methods=("linear_svm", "anova_f"),
            regime_gating_very_hard_min_classes=2,
        )
    )
    ...
    monkeypatch.setattr(
        pipe,
        "_resolve_dataset_tier",
        lambda **kwargs: (
            "medium",
            "meta_features",
            {"fisher_f1": 0.05, "n1_borderline": 0.45, "n": 60.0, "class_count": 2.0},
        ),
    )

    policy = pipe._resolve_method_policy("mock", X, y)
    assert bool(policy["regime_policy_applied"]) is True
    assert bool(policy["regime_policy_trigger_expanded_features"]) is True
    assert str(policy["regime_policy_mode"]) == "very_hard_fallback"
