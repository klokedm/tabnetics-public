...
build_campaign_specs = campaign_status.build_campaign_specs
build_tabarena_snapshot = campaign_status.build_tabarena_snapshot
collect_local_tabarena_shards = campaign_status.collect_local_tabarena_shards
artifact_family_from_job_id = campaign_status.artifact_family_from_job_id
parse_status_filename = campaign_status.parse_status_filename
tabarena_logical_key = campaign_status.tabarena_logical_key
...
def test_tabarena_logical_key_collapses_retry_attempts_per_cell() -> None:
    assert tabarena_logical_key("p07/20260314_213350_tabarena_benchmark/heartbeat.json") == "p07"
    assert tabarena_logical_key("20260314_095054_tabarena_benchmark/heartbeat.json") == ""
...
def test_build_campaign_specs_tracks_overnight_tabarena_probe() -> None:
    repo_root = Path("/tmp/tabnetics")

    spec = build_campaign_specs(repo_root)["tabarena"]

    assert spec.display_name == "TabArena general_tabular_probe"
    assert set(spec.remote_patterns) == {"lab01", "lab02", "arch-ml"}
    assert any("tabarena_general_tabular_probe_retry_20260314_230515" in pattern for pattern in spec.remote_patterns["lab01"])
    assert spec.local_roots["arch-ml"][0] == (
        repo_root / "run_artifacts/tabarena_general_tabular_probe_20260314_213101_pull_latest/archml/base"
    )


def test_build_campaign_specs_includes_val20_wave1() -> None:
    repo_root = Path("/tmp/tabnetics")
    spec = build_campaign_specs(repo_root)["val20"]

    assert spec.display_name == "Val-20 Wave 1"
    assert set(spec.remote_patterns) == {"lab01", "lab02", "arch-ml"}
    assert spec.plan_files[0] == repo_root / "pod_validation/plan_8_val20_wave1.json"
    assert spec.local_roots["arch-ml"][0] == (
        repo_root / "run_artifacts/validation-20/val20_pull_latest/arch-ml"
    )
    assert artifact_family_from_job_id("val20_flaml/V20_F04_sklearn_mnpo_diag24/ds01") == "flaml"


def test_collect_local_tabarena_shards_preserves_host_mapping(tmp_path: Path) -> None:
    lab01_root = tmp_path / "lab01" / "base"
    archml_root = tmp_path / "archml" / "base"
    lab01_run = lab01_root / "p07" / "20260314_213350_tabarena_benchmark"
    lab01_retry = lab01_root / "p07" / "20260314_231038_tabarena_benchmark"
    archml_run = archml_root / "p03" / "20260314_213342_tabarena_benchmark"
    lab01_run.mkdir(parents=True)
    lab01_retry.mkdir(parents=True)
    archml_run.mkdir(parents=True)
    ...
    for run_dir in (lab01_run, archml_run):
        (run_dir / "heartbeat.json").write_text(campaign_status.json.dumps(heartbeat), encoding="utf-8")
        (run_dir / "config.json").write_text(campaign_status.json.dumps(config), encoding="utf-8")
    retry_heartbeat = dict(heartbeat, completed=7)
    (lab01_retry / "heartbeat.json").write_text(campaign_status.json.dumps(retry_heartbeat), encoding="utf-8")
    (lab01_retry / "config.json").write_text(campaign_status.json.dumps(config), encoding="utf-8")

    shards, roots_seen = collect_local_tabarena_shards(
        {
            "lab01": (lab01_root,),
            "arch-ml": (archml_root,),
        }
    ...
    assert roots_seen == [str(lab01_root), str(archml_root)]
    shard_hosts = {shard["host"] for shard in shards}
    assert shard_hosts == {"lab01", "arch-ml"}
    latest_lab01 = next(shard for shard in shards if shard["host"] == "lab01")
    assert latest_lab01["completed"] == 7
