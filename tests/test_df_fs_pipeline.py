...
def test_fs_oracle_enable_stability_alias_disables_legacy_flag():
    cfg = DFFSConfig(fs_oracle_enable_stability=False)

    assert cfg.fs_oracle_enable_stability is False
    assert cfg.use_stability_oracle is False


def test_pipeline_worker_scope_restores_prior_thread_local_setting():
    previous = get_sklearn_n_jobs()
    try:
        set_sklearn_n_jobs(17)

        class _Probe:
            config = DFFSConfig(n_jobs=2)

        @df_fs_pipeline._scope_pipeline_sklearn_n_jobs
        def _observe(self):
            return get_sklearn_n_jobs()

        assert _observe(_Probe()) == 2
        assert get_sklearn_n_jobs() == 17
        assert _Probe.config.classification.n_jobs == 2
        ...
        backend = SklearnBackend(candidate_names=("rf",), n_jobs=2)
        model = backend._build_candidates(X_train=X, y_train=y, seed=17)["rf"]
        assert model.n_jobs == 2
    finally:
        set_sklearn_n_jobs(previous)
...
class _FixedProbaClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self):
        self.classes_ = np.array([0, 1, 2], dtype=int)

    def predict_proba(self, X):
        X = np.asarray(X)
        return np.tile(np.array([[0.65, 0.25, 0.10]], dtype=float), (X.shape[0], 1))
...
class _RequestedDeviceClassifier(BaseEstimator, ClassifierMixin):
...
def test_safe_log_loss_uses_model_labels_when_test_fold_misses_class():
    ...
    value = DistributionFeatureSelectionPipeline._safe_log_loss(
        model, X, y, descriptor
    )

    assert np.isfinite(value)


def _selected_identity(canonical_name, *, members=()):
    return {
        "classification_backend_used": "test",
        "classification_selected_identity": {
            "requested_name": canonical_name or "mnpo_ensemble",
            "outward_name": canonical_name or "mnpo_ensemble",
            "canonical_name": canonical_name,
            "registry_anchor_name": canonical_name,
            "effective_model_name": canonical_name or "mnpo_ensemble",
            "composite_identity": (
                "mnpo_ensemble" if canonical_name is None else None
            ),
...
def test_pipeline_requested_device_direct_and_composite_provenance():
    from tabnetics.classification.backends import _SoftVotingPredictAdapter
    ...
    pipe = DistributionFeatureSelectionPipeline(
        DFFSConfig(tabentics_diakrino_device="cuda:2")
    )

    lr = LogisticRegression(max_iter=2000).fit(X, y)
    lr_descriptor = pipe._inspect_selected_classifier(
        lr,
        X,
        model_cv_meta=_selected_identity("lr"),
        effective_model_name="lr",
    )
    assert lr_descriptor.requested_device is None
    assert lr_descriptor.requested_device_aggregation == "unavailable"

    native = _RequestedDeviceClassifier(device="auto").fit(X, y)
    native_descriptor = pipe._inspect_selected_classifier(
        native,
        X,
        model_cv_meta=_selected_identity("tabentics_diakrino"),
        effective_model_name="tabentics_diakrino",
    )
    assert native_descriptor.requested_device == "cuda:2"
    assert native_descriptor.requested_device_aggregation == "direct"
    assert native_descriptor.observed_device is None

    tabpfn = _RequestedDeviceClassifier(device="cpu").fit(X, y)
    tabpfn_descriptor = pipe._inspect_selected_classifier(
        tabpfn,
        X,
        model_cv_meta=_selected_identity("tabpfn"),
        effective_model_name="tabpfn",
    )
    assert tabpfn_descriptor.requested_device == "cpu"
    assert tabpfn_descriptor.requested_device_aggregation == "direct"
    assert tabpfn_descriptor.observed_device is None
    ...
    composite = VotingClassifier(
        estimators=[
            (
                "native",
                _SoftVotingPredictAdapter(
                    _RequestedDeviceClassifier(device="auto")
                ),
            ),
    ...
    composite_descriptor = pipe._inspect_selected_classifier(
        composite,
        X,
        model_cv_meta=_selected_identity(None, members=members),
        effective_model_name="mnpo_ensemble",
    )
    assert composite_descriptor.requested_device is None
    assert composite_descriptor.requested_device_aggregation == "member_specific"
    assert [member.requested_device for member in composite_descriptor.members] == [
        "cuda:2",
        None,
    ]


def test_model_cv_enable_svc_probability_builds_probability_candidates():
    X, y = make_classification(
        n_samples=48,
        n_features=8,
        n_informative=5,
    ...
    cfg = DFFSConfig(model_cv_enable_svc_probability=True)
    pipe = DistributionFeatureSelectionPipeline(cfg)

    backend = pipe._make_sklearn_backend(("svm_rbf", "svm_linear", "vote_ensemble"))
    candidates = backend._build_candidates(X_train=X.astype(float), y_train=y.astype(int), seed=177)

    assert candidates["svm_rbf"].probability is True
    assert candidates["svm_linear"].steps[-1][1].probability is True
    vote = candidates["vote_ensemble"]
    assert str(vote.voting) == "soft"
    assert vote.estimators[1][1].steps[-1][1].probability is True


def test_pipeline_svc_probability_flag_materializes_log_loss():
    X, y = make_classification(
        n_samples=72,
        n_features=12,
        n_informative=7,
    ...
    cfg = DFFSConfig(
        ...
        classification=df_fs_pipeline.ClassificationConfig(
            conformal_enabled=True,
            conformal_calibration_fraction=0.25,
            conformal_min_calibration=10,
            conformal_output_sets=True,
        ),
    )
    ...
    result = DistributionFeatureSelectionPipeline(cfg).run(
        X.astype(float),
        y.astype(int),
        dataset_name="svc_proba",
        seed=178,
    )

    assert result.model_name == "svm_rbf"
    assert np.isfinite(result.log_loss)
    assert bool(result.config_snapshot["model_cv_enable_svc_probability"]) is True
    assert bool(result.config_snapshot["model_cv_svc_probability_enabled"]) is True
    assert "svm_rbf" in tuple(result.config_snapshot["model_cv_svc_probability_candidates"])
    assert "svm_rbf" in dict(result.config_snapshot["model_cv_candidate_wall_seconds"])
    assert bool(result.config_snapshot["classifier_conformal_applied"]) is True
    assert result.config_snapshot["classifier_conformal_score_source"] == "predict_proba"
    assert bool(result.config_snapshot["classifier_conformal_used_predict_proba"]) is True
    assert len(result.config_snapshot["classifier_conformal_prediction_sets"]) == result.n_test


def test_pipeline_posthoc_calibration_stage_reports_metrics():
    X, y = make_classification(
        n_samples=84,
        n_features=48,
        n_informative=14,
    ...
    cfg = DFFSConfig(
        ...
        classifier_posthoc_calibration_enabled=True,
        classifier_posthoc_calibration_method="sigmoid",
        classifier_posthoc_calibration_fraction=0.25,
        classifier_posthoc_calibration_min_calibration=6,
    ...
    result = DistributionFeatureSelectionPipeline(cfg).run(
        X.astype(float),
        y.astype(int),
        dataset_name="posthoc_calibration",
        seed=181,
    )
    snap = dict(result.config_snapshot or {})

    assert bool(snap["classifier_posthoc_calibration_enabled"]) is True
    assert str(snap["classifier_posthoc_calibration_method"]) == "sigmoid"
    assert "classifier_posthoc_calibration_applied" in snap
    assert str(snap["classifier_posthoc_calibration_skip_reason"]) in {
        "ok",
        "degenerate_training_data",
        "insufficient_class_counts",
        "insufficient_calibration_holdout",
        "split_missing_class",
    } or str(snap["classifier_posthoc_calibration_skip_reason"]).startswith(
        ("base_fit_", "calibrator_")
    )
    assert bool(snap["calibration_reporting_enabled"]) is True
    assert "calibration_brier" in snap
    assert "calibration_ece" in snap
    if bool(snap.get("calibration_metrics_available", False)):
        assert np.isfinite(float(snap["calibration_brier"]))
        assert np.isfinite(float(snap["calibration_ece"]))
        assert float(snap["calibration_brier"]) >= 0.0
        assert 0.0 <= float(snap["calibration_ece"]) <= 1.0


def test_posthoc_calibration_refinement_stopping_keeps_base_model(monkeypatch):
    X, y = make_classification(
        n_samples=72,
        n_features=12,
        n_informative=7,
    ...
    cfg = DFFSConfig(
        ...
        classifier_posthoc_calibration_refinement_stopping=True,
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    metrics = iter(
        [
    ...
    monkeypatch.setattr(
        pipe,
        "_safe_probability_calibration_metrics",
    ...
    calibrated, meta = pipe._apply_posthoc_calibration(
        model,
        X.astype(float),
        y.astype(int),
        seed=382,
    ...
    assert bool(meta["classifier_posthoc_calibration_enabled"]) is True
    assert bool(meta["classifier_posthoc_calibration_applied"]) is False
    assert meta["classifier_posthoc_calibration_skip_reason"] == "refinement_stopping_brier_worse"
    assert float(meta["classifier_posthoc_calibration_base_brier"]) == pytest.approx(0.10)
    assert float(meta["classifier_posthoc_calibration_calibrated_brier"]) == pytest.approx(0.12)
...
def _distinct_grid(n_unique: int, repeats: int = 4) -> np.ndarray:
    return np.repeat(np.linspace(-2.0, 2.0, int(n_unique)), int(repeats)).astype(float)


def test_distribution_fitter_flex_distinct_gate_default_off_preserves_candidates():
    x = _distinct_grid(5, repeats=10)
    fitter = DistributionFitter(
        DistributionFitterConfig(
            family_set="flex",
            use_support_filtering=False,
            flex_family_distinct_gate_enabled=False,
        )
    )
    audit = fitter.audit_data(x)
    candidates = fitter.generate_candidates(audit)

    assert audit.n_unique == 5
    assert list(candidates) == list(fitter._base_distributions)
    assert set(candidates) == set(fitter._base_distributions)
    assert {"johnsonsu", "tukeylambda", "genhyperbolic", "geninvgauss"} <= set(candidates)


def test_distribution_fitter_flex_distinct_gate_thresholds_all_shape_param_families():
    fitter = DistributionFitter(
        DistributionFitterConfig(
            family_set="flex",
            use_support_filtering=False,
            flex_family_distinct_gate_enabled=True,
            flex_family_min_distinct=15,
        )
    )
    low_audit = fitter.audit_data(_distinct_grid(14))
    high_audit = fitter.audit_data(_distinct_grid(15))
    low_candidates = fitter.generate_candidates(low_audit)
    high_candidates = fitter.generate_candidates(high_audit)
    ...
    assert low_audit.n_unique == 14
    assert high_audit.n_unique == 15
    assert gated_examples.isdisjoint(low_candidates)
    assert gated_examples <= set(high_candidates)
    assert all(fitter._family_param_count(name) < 3 for name in low_candidates)


def test_distribution_fitter_flex_distinct_gate_blocks_heaping_readd():
    x = _distinct_grid(5, repeats=30)
    off = DistributionFitter(
        DistributionFitterConfig(
            family_set="flex",
            use_support_filtering=True,
            flex_family_distinct_gate_enabled=False,
    ...
    audit = on.audit_data(x)
    off_candidates = off.generate_candidates(audit)
    on_candidates = on.generate_candidates(audit)

    assert audit.has_heaping is True
    assert audit.n_unique == 5
    assert "johnsonsu" in off_candidates
    assert "johnsonsu" not in on_candidates
    assert "t" not in on_candidates
    assert {"norm", "laplace"} <= set(on_candidates)


class _FakeDistributionSelector:
...
def _fake_distribution_fit(name: str, simple_score: float):
    params = (0.0, 1.0, 0.0, 1.0) if name == "johnsonsu" else (0.0, 1.0)
...
@pytest.mark.parametrize(
    ("retention_margin", "expected_family", "expected_score"),
    [
        (0.01, "norm", 0.105),
        (0.001, "johnsonsu", 0.100),
    ],
)
def test_distribution_fitter_flex_retention_margin_uses_simple_score_scale(
    monkeypatch,
    retention_margin,
    expected_family,
    expected_score,
    ...
    fitter = DistributionFitter(
        DistributionFitterConfig(
            use_support_filtering=False,
            flex_family_retention_margin=retention_margin,
        )
    )
    results = [
        _fake_distribution_fit("johnsonsu", 0.100),
        _fake_distribution_fit("norm", 0.105),
    ]
    monkeypatch.setattr(fitter, "_build_selector", lambda *args, **kwargs: _FakeDistributionSelector(results))

    summary = fitter.select_best_distribution(_distinct_grid(20), criterion="simple", feature_index=0)

    assert summary.family == expected_family
    assert summary.simple_score == pytest.approx(expected_score)
...
def test_pipeline_emits_reproducible_model_bundle_and_structured_diagnostics():
    ...
    deployed = df_fs_pipeline.DFFSReproducibleModel.from_json_dict(
        bundle,
...
def test_meta_learning_delegation_uses_resolved_split_positions(monkeypatch):
    """Delegation must not replay caller-facing external row IDs internally."""
    X, y = _small_dataset(457)
    train_idx = np.arange(0, 72, 2, dtype=int)
    test_idx = np.arange(1, 72, 2, dtype=int)
    captured: dict[str, object] = {}

    class _DelegatedPipeline:
        def __init__(self, config):
            captured["config"] = config

        def run_pre_split(self, **kwargs):
            captured.update(kwargs)
            return "delegated-result"

    pipe = DistributionFeatureSelectionPipeline(DFFSConfig(random_seed=23))
    monkeypatch.setattr(
        pipe,
        "_resolve_meta_learning_runtime_config",
        lambda X_train, y_train: (DFFSConfig(random_seed=23), {"selected": "fixture"}),
    )
    monkeypatch.setattr(df_fs_pipeline, "DistributionFeatureSelectionPipeline", _DelegatedPipeline)

    result = pipe.run_pre_split(
        X_train=X[train_idx],
        y_train=y[train_idx],
        X_test=X[test_idx],
        y_test=y[test_idx],
        dataset_name="external-row-ids",
        seed=23,
        split_indices_train=train_idx,
        split_indices_test=test_idx,
    )
    ...
    assert tuple(captured["split_indices_train"]) == tuple(range(train_idx.size))
    assert tuple(captured["split_indices_test"]) == tuple(
        range(train_idx.size, train_idx.size + test_idx.size)
    )
    delegated_context = captured["resampling_context"]
    assert isinstance(delegated_context, df_fs_pipeline.FitResamplingContext)
    assert delegated_context.row_ids == tuple((*train_idx.tolist(), *test_idx.tolist()))
    delegated_plan = captured["resolved_outer_split"]
    assert tuple(delegated_plan.primary.train_indices) == tuple(range(train_idx.size))
    assert tuple(delegated_plan.primary.test_indices) == tuple(
        range(train_idx.size, train_idx.size + test_idx.size)
    )
...
def test_pipeline_classifier_conformal_snapshot_fields():
    ...
    assert "classifier_conformal_score_source" in res.config_snapshot
    assert "classifier_conformal_conformity_kind" in res.config_snapshot
    assert "classifier_conformal_model_probability_kind" in res.config_snapshot
    assert "model_cv_svc_probability_enabled" not in res.config_snapshot
...
def test_select_model_via_cv_skips_unavailable_optional_backends(monkeypatch):
    ...
    assert model_name not in {"xgb", "tabpfn"}
...
def test_pipeline_records_tabentics_diakrino_missing_checkpoint_in_snapshot():
    X, y = _small_dataset(958)

    cfg = DFFSConfig(
        random_seed=43,
        model_candidates=("lr", "tabentics_diakrino"),
        include_tabentics_diakrino_model=True,
    ...
    assert np.isfinite(res.balanced_accuracy)
    assert "tabentics_diakrino" in tuple(res.config_snapshot.get("model_cv_effective_candidates", ()))
    assert "tabentics_diakrino" not in tuple(res.config_snapshot.get("model_cv_constructed_candidates", ()))
    assert dict(res.config_snapshot.get("model_cv_candidate_build_failures", {})).get("tabentics_diakrino") == (
        "native DIAKRINO checkpoint not configured"
    )
    assert bool(res.config_snapshot.get("include_tabentics_diakrino_model", False)) is True


def test_pipeline_skips_tabentics_diakrino_checkpoint_without_trust_bundle(tmp_path):
    ...
    cfg_native = TabenticsDiakrinoFSClassifierConfig(
        d_model=16,
        n_heads=2,
        context_layers=1,
        query_layers=1,
    ...
    model = TabenticsDiakrinoFSClassifier(cfg_native)
    torch.save(
        {
            "checkpoint_format": "classifier",
            "model_config": cfg_native.__dict__,
    ...
    cfg = DFFSConfig(
        ...
        tabentics_diakrino_device="cpu",
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    res = pipe.run(X, y, dataset_name="tiny", seed=44)

    failures = dict(res.config_snapshot.get("model_cv_candidate_build_failures", {}))
    assert np.isfinite(res.balanced_accuracy)
    assert "tabentics_diakrino" in tuple(res.config_snapshot.get("model_cv_effective_candidates", ()))
    assert "tabentics_diakrino" not in tuple(res.config_snapshot.get("model_cv_constructed_candidates", ()))
    assert "tabentics_diakrino" not in tuple(res.config_snapshot.get("model_cv_evaluated_candidates", ()))
    assert failures.get("tabentics_diakrino") == (
        "native DIAKRINO checkpoint non-comparable for classification: artifact_manifest_missing"
    )


def test_pipeline_uses_effective_dataset_id_for_tabentics_diakrino_sidecar(
    tmp_path,
    monkeypatch,
):
    pytest.importorskip("torch")
    import tabnetics.classification.diakrino_native as diakrino_native_module

    checkpoint = tmp_path / "trusted_classifier.pt"
    checkpoint.write_bytes(b"checkpoint header is not loaded by fake loader")
    sidecar_path = tmp_path / "feature_scores.parquet"

    def fake_loader(path, map_location=None):
        return object(), {
            "checkpoint_comparable_for_classification": True,
            "checkpoint_usable_by_core_candidate": True,
        }

    class FakeNativeClassifier:
    ...
    cfg = DFFSConfig(
        ...
        diakrino_sidecar_path=str(sidecar_path),
        diakrino_sidecar_dataset_id="",
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    pipe._active_diakrino_dataset_id = "toy_active"

    model, reason = pipe._build_tabentics_diakrino_model(seed=46)

    assert reason is None
    assert isinstance(model, FakeNativeClassifier)
    assert model.sidecar_path == str(sidecar_path)
    assert model.dataset_id == "toy_active"
    assert model.support_joint_serving_cache is True
    assert model.retry_cuda_oom_microbatch is True
    snap = pipe._config_snapshot()
    assert snap["diakrino_sidecar_dataset_id"] == ""
    assert snap["diakrino_sidecar_dataset_id_effective"] == "toy_active"
    assert snap["tabentics_diakrino_support_joint_serving_cache"] is True
    assert snap["tabentics_diakrino_retry_cuda_oom_microbatch"] is True

    explicit_cfg = DFFSConfig(
        random_seed=47,
        include_tabentics_diakrino_model=True,
        tabentics_diakrino_checkpoint=str(checkpoint),
        tabentics_diakrino_device="cpu",
    ...
    explicit_pipe = DistributionFeatureSelectionPipeline(explicit_cfg)
    explicit_pipe._active_diakrino_dataset_id = "toy_active"

    explicit_model, explicit_reason = explicit_pipe._build_tabentics_diakrino_model(seed=47)

    assert explicit_reason is None
    assert isinstance(explicit_model, FakeNativeClassifier)
    assert explicit_model.dataset_id == "explicit_dataset"
    explicit_snap = explicit_pipe._config_snapshot()
    assert explicit_snap["diakrino_sidecar_dataset_id"] == "explicit_dataset"
    assert explicit_snap["diakrino_sidecar_dataset_id_effective"] == "explicit_dataset"


@pytest.mark.parametrize("requested_device", ["cuda", "cuda:0"])
def test_pipeline_skips_tabentics_diakrino_when_explicit_cuda_unavailable(
    tmp_path,
    monkeypatch,
    requested_device,
):
    ...
    checkpoint.write_bytes(b"not loaded when cuda is unavailable")
    ...
    cfg = DFFSConfig(
        ...
        include_tabentics_diakrino_model=True,
        tabentics_diakrino_checkpoint=str(checkpoint),
        tabentics_diakrino_device=requested_device,
    )
    pipe = DistributionFeatureSelectionPipeline(cfg)
    res = pipe.run(X, y, dataset_name="tiny", seed=45)
    ...
    assert "tabentics_diakrino" not in tuple(res.config_snapshot.get("model_cv_evaluated_candidates", ()))
    assert failures.get("tabentics_diakrino") == (
        "native DIAKRINO checkpoint requested CUDA but CUDA is unavailable"
    )
...
def test_pipeline_supports_class_pareto_snapshot():
    ...
    assert np.isfinite(res.macro_f1)
    assert res.selected_features_count > 0
    assert res.selected_features_count == len(set(res.selected_feature_indices_original))
