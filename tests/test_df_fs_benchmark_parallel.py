...
def test_parallel_task_path_rejects_substituted_dispatch(tmp_path, monkeypatch):
    ...
    def fake_task(ds_id, seed, _args):
        return {"rows": [_base_row(ds_id, seed)], "failures": []}

    monkeypatch.setattr(benchmark, "_run_dataset_seed_task", fake_task)
    with pytest.raises(CanonicalExecutionOriginError, match="_run_dataset_seed_task"):
...
def test_nestedcv_audit_injects_grouped_resolved_plans(monkeypatch):
    captured = []
    ...
    class FakePipeline:
        ...
        def run_pre_split(self, **kwargs):
            ...
            raise RuntimeError("intentional capture stop")
    ...
    args = types.SimpleNamespace(
        nestedcv_outer_splits=3,
        nestedcv_outer_repeats=1,
        nestedcv_min_train_per_class=1,
        nestedcv_seed_stride=101,
    )

    result = benchmark._run_nestedcv_audit(
        X,
        y,
        object(),
        "grouped-fixture",
    ...
    assert result["nestedcv_failures"] == 3
    assert result["nestedcv_resampling_policy"]["kind"] == "stratified_group"
    for call in captured:
        assert call["resampling_context"] is context
        plan = call["resolved_outer_split"]
        assert plan.primary.audit.identity_overlap_counts == (("groups", 0),)
...
def test_hard_timeout_worker_kills_overrun_process(monkeypatch):
    ...
    with pytest.raises(CanonicalExecutionOriginError, match="_pipeline_run_worker"):
...
def test_parallel_task_path_rejects_substituted_dispatch_for_shadow_run(tmp_path, monkeypatch):
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_easy_dfshift",
...
def test_clone_config_preserves_df_multimodal_settings():
    base = benchmark.DFFSConfig(
        ...
        diakrino_sidecar_path="/tmp/diakrino-feature-logits.parquet",
        diakrino_cdf_trust_gate_enabled=True,
        diakrino_cdf_trust_entropy_threshold=0.7,
        ...
        fs_diakrino_relevance_min_n_train=17,
        fs_diakrino_relevance_score_column="screening_logit",
        fs_oracle_complexity_conditioning=True,
        dist_config=benchmark.DistributionFitterConfig(
            compute_dip=False,
            diakrino_family_prior_lambda=0.17,
            diakrino_skip_fit_discrete_enabled=True,
            diakrino_sidecar_path="/tmp/diakrino-dist-logits.parquet",
        ),
    )
    cloned = benchmark.clone_config(base)
    assert bool(cloned.dist_config.compute_dip) is False
    assert float(cloned.dist_config.diakrino_family_prior_lambda) == 0.17
    assert bool(cloned.dist_config.diakrino_skip_fit_discrete_enabled) is True
    assert str(cloned.dist_config.diakrino_sidecar_path) == "/tmp/diakrino-dist-logits.parquet"
    assert str(cloned.multimodal_fallback) == "rank_transform"
    assert str(cloned.diakrino_sidecar_path) == "/tmp/diakrino-feature-logits.parquet"
    assert bool(cloned.diakrino_cdf_trust_gate_enabled) is True
    assert float(cloned.diakrino_cdf_trust_entropy_threshold) == 0.7
    assert str(cloned.diakrino_cdf_trust_fallback) == "drop"
    assert bool(cloned.diakrino_stability_surrogate_enabled) is True
    assert bool(cloned.diakrino_regime_conditional) is True
    assert str(cloned.diakrino_prior_score_column) == "screening_logit"
    assert str(cloned.diakrino_screening_score_column) == "prior_logit"
    assert str(cloned.diakrino_prior_calibrate) == "chunk_rank01"
    assert int(cloned.diakrino_prior_top_k) == 13
    assert bool(cloned.fs_use_diakrino_relevance_oracle) is True
    assert int(cloned.fs_diakrino_relevance_min_n_train) == 17
    assert str(cloned.fs_diakrino_relevance_score_column) == "screening_logit"
    assert bool(cloned.fs_oracle_complexity_conditioning) is True


def test_clone_config_preserves_benchmark_router_and_fs_oracle_toggles():
    base = benchmark.DFFSConfig(
        enabled_methods=("group_sparse_lasso",),
        use_stability_oracle=False,
        fs_oracle_enable_stability=False,
        auto_router_enabled=False,
        auto_router_artifact_path="/tmp/tabnetics-router",
        auto_router_fail_open=False,
        auto_router_descriptor_ood_gate_enabled=True,
        diakrino_router_dispersion_descriptor_enabled=True,
    )
    cloned = benchmark.clone_config(base)

    assert tuple(cloned.enabled_methods) == ("group_sparse_lasso",)
    assert bool(cloned.use_stability_oracle) is False
    assert bool(cloned.fs_oracle_enable_stability) is False
    assert bool(cloned.auto_router_enabled) is False
    assert str(cloned.auto_router_artifact_path) == "/tmp/tabnetics-router"
    assert bool(cloned.auto_router_fail_open) is False
    assert bool(cloned.auto_router_descriptor_ood_gate_enabled) is True
    assert bool(cloned.diakrino_router_dispersion_descriptor_enabled) is True


def test_clone_config_does_not_enable_router_when_legacy_field_is_missing():
    class LegacyConfigWithoutAutoRouter(benchmark.DFFSConfig):
        def __getattribute__(self, name):
            if name == "auto_router_enabled":
                raise AttributeError(name)
            return super().__getattribute__(name)

    cloned = benchmark.clone_config(LegacyConfigWithoutAutoRouter())

    assert bool(cloned.auto_router_enabled) is False


def test_legacy_sota_clone_config_preserves_dynamic_diakrino_settings(monkeypatch):
    sklearn_genetic = types.ModuleType("sklearn_genetic")

    class GAFeatureSelectionCV:
        pass

    sklearn_genetic.GAFeatureSelectionCV = GAFeatureSelectionCV
    monkeypatch.setitem(sys.modules, "sklearn_genetic", sklearn_genetic)
    monkeypatch.setitem(sys.modules, "seaborn", types.ModuleType("seaborn"))

    module_path = Path(__file__).resolve().parents[2] / "experiments" / "run_df_fs_sota_benchmark.py"
    spec = importlib.util.spec_from_file_location("sota_benchmark_clone_test", module_path)
    assert spec is not None
    assert spec.loader is not None
    sota = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(sota)

    base = sota.DFFSConfig(
        multimodal_fallback="rank_transform",
        dist_config=sota.DistributionFitterConfig(compute_dip=False),
    )
    for name, value in {
        "diakrino_sidecar_path": "/tmp/diakrino-feature-logits.parquet",
        "diakrino_cdf_trust_gate_enabled": True,
    ...
    for name, value in {
        ...
        setattr(base.dist_config, name, value)

    cloned = sota.clone_config(base)
...
def test_tier_lockout_and_routing_flags_populate_base_config():
    args = benchmark.build_arg_parser().parse_args(
        [
            ...
            "/tmp/tier_classifier_model.json",
    ...
    assert str(cfg.tier_classifier_mode) == "learned"
    assert str(cfg.tier_classifier_model_path) == "/tmp/tier_classifier_model.json"
...
def test_regime_gating_flags_populate_base_config():
    ...
    assert float(cfg.regime_gating_min_samples_per_class) == pytest.approx(7.0)
    assert bool(cfg.regime_gating_use_expanded_features) is True
    assert float(cfg.regime_gating_min_fisher_f1) == pytest.approx(0.08)
    assert float(cfg.regime_gating_max_n1_borderline) == pytest.approx(0.45)
...
def test_meta_learning_and_adaptive_prefilter_paths_populate_base_config():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
    ...
    assert str(cfg.meta_learning_records_path) == "/tmp/val21_meta_records.json"
    assert bool(cfg.prefilter_adaptive_top_k) is True
    assert float(cfg.prefilter_adaptive_top_k_scaling) == pytest.approx(0.65)
...
def test_benchmark_base_config_keeps_explicit_fs_flags_authoritative():
    parser = benchmark.build_arg_parser()
    spec = benchmark.BENCHMARK_DATASETS["synthetic_medium_mixed"]

    args_group = parser.parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
            "--fs-method-set",
            "singleton_group_sparse_lasso",
            "--disable-fs-stability-oracle",
        ]
    )
    cfg_group = benchmark._build_base_config(args_group, spec, seed=11)
    assert tuple(cfg_group.enabled_methods) == ("group_sparse_lasso",)
    assert bool(cfg_group.auto_router_enabled) is False
    assert bool(cfg_group.use_stability_oracle) is False
    assert bool(cfg_group.fs_oracle_enable_stability) is False

    args_pathway = parser.parse_args(
        [
            "--datasets",
            "synthetic_medium_mixed",
            "--fs-method-set",
            "singleton_pathway_group_sparse_lasso",
        ]
    )
    cfg_pathway = benchmark._build_base_config(args_pathway, spec, seed=11)
    assert tuple(cfg_pathway.enabled_methods) == ("pathway_group_sparse_lasso",)
    assert bool(cfg_pathway.auto_router_enabled) is False
...
def test_classifier_oracle_and_posthoc_calibration_flags_populate_base_config():
    args = benchmark.build_arg_parser().parse_args(
        [
            "--datasets",
            "synthetic_easy_dfshift",
    ...
    assert bool(cfg.classifier_oracle_enable_worst_class) is False
    assert bool(cfg.calibration_reporting_enabled) is True
    assert bool(cfg.classifier_posthoc_calibration_enabled) is True
    assert str(cfg.classifier_posthoc_calibration_method) == "isotonic"
    assert float(cfg.classifier_posthoc_calibration_fraction) == pytest.approx(0.30)
    assert int(cfg.classifier_posthoc_calibration_min_calibration) == 12
    assert bool(cfg.classifier_posthoc_calibration_refinement_stopping) is False
