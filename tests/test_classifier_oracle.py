...
def test_classifier_oracle_run_smoke():
    ...
    oracle = ClassifierOracle(
        ...
        include_worst_class_recall=True,
    ...
    assert "worst_class_recall" in dict(out.get("oracle_matrices") or {})
    assert isinstance(out.get("candidate_stats"), dict)
    for stats in dict(out.get("candidate_stats") or {}).values():
        assert "worst_class_recall_score" in dict(stats)
        assert 0.0 <= float(stats["worst_class_recall_score"]) <= 1.0
...
def test_mnpo_backend_non_native_family_falls_back_without_flaml():
    ...
    fallback_reason = str(hpo_meta.get("mnpo_flaml_fallback_reason", ""))
    assert fallback_reason == "family_not_supported_by_flaml" or fallback_reason.startswith("flaml_custom_error:")
...
def test_soft_voting_predict_adapter_rejects_no_proba_estimator():
    X, y, _, _ = _toy_data(n_samples=80, n_features=16, n_classes=3, seed=402)
    base = _NoProbClassifier()
    adapter = _SoftVotingPredictAdapter(base)
    adapter.fit(X, y)
    with pytest.raises(AttributeError, match="does not expose predict_proba"):
