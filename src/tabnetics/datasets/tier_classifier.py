...
_MODEL_CACHE: Dict[Path, Dict[str, Any]] = {}
...
def load_tier_classifier_model(path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if model_path in _MODEL_CACHE:
        return dict(_MODEL_CACHE[model_path])
    with model_path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    _MODEL_CACHE[model_path] = dict(payload)
    return dict(payload)
...
def predict_tier_with_details(
    ...
    *,
    ...
    try:
        model_payload = load_tier_classifier_model(model_path)
