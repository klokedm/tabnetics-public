...
# strict_holdout vs inflated protocol families (source: the validation guide)
KNOWN_SOTA_PROTOCOL_RANGES: Dict[str, Dict[str, Tuple[float, float]]] = {
    ...
    "gli_85": {"strict_holdout": (0.75, 0.87), "inflated": (0.90, 1.00)},
    "cns_pomeroy": {"strict_holdout": (0.74, 0.86), "inflated": (0.80, 0.98)},
    ...
    "nci9_60_9class": {"strict_holdout": (0.50, 0.70), "inflated": (0.66, 0.78)},
    ...
    "cumida_renal_gse53757": {"strict_holdout": (0.80, 0.92), "inflated": (0.88, 0.98)},
    ...
    "rv_coil20": {"strict_holdout": (0.92, 0.99), "inflated": (0.98, 1.00)},
    "rv_relathe": {"strict_holdout": (0.82, 0.92), "inflated": (0.88, 0.96)},
    "rv_pcmac": {"strict_holdout": (0.80, 0.90), "inflated": (0.86, 0.95)},
    "rv_isolet": {"strict_holdout": (0.85, 0.95), "inflated": (0.92, 0.98)},
    "rv_basehock": {"strict_holdout": (0.85, 0.93), "inflated": (0.90, 0.94)},
...
def _finalize_spec(spec: DatasetSpec) -> DatasetSpec:
    ...
    if out.pipeline in {"fs", "integrated"} and out.source_kind in {"synthetic", "validation_catalog"}:
        ...
        if out.pipeline == "integrated":
            ...
            # dataset's SOTA ranges when available (per the validation guide / benchmark runner).
            base = None
            try:
