...
class DWDClassifier(ClassifierMixin, BaseEstimator):
    ...
    def _fit_binary(self, X: np.ndarray, y_binary: np.ndarray) -> dict:
        """Fit binary DWD via L-BFGS on the gmd_dwd objective."""
        n, p = X.shape
        # Encode labels as +1/-1.
        labels = np.where(y_binary == y_binary[0], 1.0, -1.0)
        ...
        return {"w": w, "beta": beta, "class_neg": y_binary[0]}
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if n_classes == 2:
            self._binary_models = [self._fit_binary(X, y)]
            ...
            for c in self.classes_:
                y_bin = np.where(y == c, c, self.classes_[0] if c != self.classes_[0] else self.classes_[1])
                y_binary_01 = np.where(y == c, 1, 0)
                # Use 0/1 encoding for consistent direction.
                unique_bin = np.array([0, 1])
                self._binary_models.append(self._fit_binary(X, y_binary_01))
        return self
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self._multiclass:
            m = self._binary_models[0]
            return np.where(scores >= 0, self.classes_[1] if m["class_neg"] == self.classes_[0] else self.classes_[0],
                            m["class_neg"])
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self._multiclass:
            ...
            m = self._binary_models[0]
            if m["class_neg"] == self.classes_[0]:
                return np.column_stack([1.0 - prob1, prob1])
            return np.column_stack([prob1, 1.0 - prob1])
        # OVA softmax.
