...
def _normalize_feature_blocks(
    feature_blocks: Optional[Dict[str, Sequence[int]]],
    *,
    n_features: int,
) -> Dict[str, Tuple[int, ...]]:
    ...
    if not isinstance(feature_blocks, dict):
        return out
    seen: set[int] = set()
    for raw_name, raw_indices in feature_blocks.items():
        ...
        for idx in idx_arr.tolist():
            idx_i = int(idx)
            if idx_i < 0 or idx_i >= int(n_features) or idx_i in seen:
                continue
            cleaned.append(idx_i)
            seen.add(idx_i)
        if cleaned:
            out[name] = tuple(cleaned)
    return out
...
def apply_multiomics_adapter_train_test(
    ...
    *,
    ...
    integrator: str,
    n_components: int,
    feature_blocks: Optional[Dict[str, Sequence[int]]] = None,
    batch_labels_train: Optional[np.ndarray] = None,
    batch_labels_test: Optional[np.ndarray] = None,
) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """Apply the benchmark-time multiomics adapter to train/test data."""
    mode = str(adapter_mode or "none").strip().lower()
    if mode in {"metadata", "metadata_block", "feature_metadata", "diablo_blocks"}:
        mode = "metadata_blocks"
    ...
    train_blocks: list[tuple[np.ndarray, str]] = []
    test_blocks: list[tuple[np.ndarray, str]] = []
    ...
    elif mode == "metadata_blocks":
        normalized_blocks = _normalize_feature_blocks(
            feature_blocks,
            n_features=int(X_train_arr.shape[1]),
        )
        meta["multiomics_feature_blocks_available"] = bool(len(normalized_blocks) >= 2)
        if len(normalized_blocks) < 2:
            meta["multiomics_adapter_reason"] = "metadata_blocks_missing"
            return X_train_arr, X_test_arr, meta
        for block_name, block_indices in normalized_blocks.items():
            idx = np.asarray(block_indices, dtype=int)
            train_blocks.append((np.asarray(X_train_arr[:, idx], dtype=float), str(block_name)))
            test_blocks.append((np.asarray(X_test_arr[:, idx], dtype=float), str(block_name)))
    else:
        meta["multiomics_adapter_reason"] = "unsupported_mode"
        return X_train_arr, X_test_arr, meta

    meta["multiomics_n_blocks"] = int(len(train_blocks))
    meta["multiomics_block_sizes"] = [int(block.shape[1]) for block, _ in train_blocks]
    meta["multiomics_block_names"] = [str(name) for _, name in train_blocks]
