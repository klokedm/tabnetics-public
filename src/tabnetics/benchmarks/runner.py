...
try:
    ...
    from tabnetics.pipeline.pipeline import ClassificationConfig, DFFSConfig, DistributionFeatureSelectionPipeline, DistributionFitterConfig, _load_feature_selector_cls
    from tabnetics.pipeline.resampling import FitResamplingContext, SplitAssignment, ensure_fit_resampling_context, resolve_assignment, resolve_cv  # type: ignore
...
from tabnetics.benchmarks.profiles import (
    ...
    DATASET_SOTA_CLASSIFIERS,  # noqa: F401 - compatibility re-export
...
from tabnetics.classification.registry import canonical_classifier_name
from tabnetics.feature_selection.diakrino_identity import (
    ArtifactIdentity,
...
from tabnetics.feature_selection.diakrino_trust import (
    DIAKRINO_REQUIRED_CALIBRATION_MODE as DIAKRINO_SIDECAR_REQUIRED_CALIBRATION,
)
from tabnetics.validation.core.provenance import (
    ...
    bind_materialized_input_split_fingerprints,
    ...
    build_noncanonical_execution_provenance,
    capture_loaded_tabnetics_module_closure,
    execution_row_fields,
    validate_loaded_tabnetics_module_closure,
    write_json,
)
...
# Keep a small import-time bootstrap boundary for direct runner/pipeline
# symbols. The complete internal execution closure is captured only after task
# materialization by ``build_canonical_execution_provenance``.
_CANONICAL_PIPELINE_IMPORT_TARGET = DistributionFeatureSelectionPipeline
_CANONICAL_DFFS_CONFIG_IMPORT_TARGET = DFFSConfig
_CANONICAL_DIST_CONFIG_IMPORT_TARGET = DistributionFitterConfig
_CANONICAL_CLASSIFICATION_CONFIG_IMPORT_TARGET = ClassificationConfig
_CANONICAL_FEATURE_SELECTOR_IMPORT_TARGET = _load_feature_selector_cls()

_CANONICAL_BOOTSTRAP_IMPORT_TARGETS = MappingProxyType(
    {
        "benchmark_runner": sys.modules[__name__],
        "classification_config": ClassificationConfig,
        "dffs_config": DFFSConfig,
        "distribution_fitter_config": DistributionFitterConfig,
        "pipeline": DistributionFeatureSelectionPipeline,
        "feature_selector": _CANONICAL_FEATURE_SELECTOR_IMPORT_TARGET,
    }
)


def _current_canonical_bootstrap_import_targets() -> Mapping[str, Any]:
    """Resolve direct symbols while retaining sealed import-time references."""

    runner_module = sys.modules.get(__name__)
    if runner_module is None:
        raise RuntimeError("benchmark runner module is unavailable for execution provenance")
    selector_cls = _load_feature_selector_cls()
    if selector_cls is not _CANONICAL_FEATURE_SELECTOR_IMPORT_TARGET:
        raise CanonicalExecutionOriginError(
            "Canonical benchmark selector lookup no longer matches its sealed import-time identity."
        )
    current: dict[str, Any] = {
        "benchmark_runner": runner_module,
        "classification_config": _CANONICAL_CLASSIFICATION_CONFIG_IMPORT_TARGET,
        "dffs_config": _CANONICAL_DFFS_CONFIG_IMPORT_TARGET,
        "distribution_fitter_config": _CANONICAL_DIST_CONFIG_IMPORT_TARGET,
        "pipeline": _CANONICAL_PIPELINE_IMPORT_TARGET,
        "feature_selector": selector_cls,
    }
    return current


def _assert_canonical_execution_bootstrap_seal() -> None:
    """Check direct bootstrap symbols without treating it as a full closure."""

    current = _current_canonical_bootstrap_import_targets()
    for label, expected in _CANONICAL_BOOTSTRAP_IMPORT_TARGETS.items():
        if current.get(label) is not expected:
            raise CanonicalExecutionOriginError(
                "Canonical execution target "
                f"{label!r} does not match the captured import-time identity."
            )


def _build_canonical_execution_contract(
    args: argparse.Namespace,
    selected_dataset_ids: Sequence[str],
    materialized_input_records: Sequence[Mapping[str, Any]],
    worker_module_closures: Sequence[Mapping[str, Any]] = (),
    worker_module_closures_complete: bool = True,
) -> Dict[str, Any]:
    return build_canonical_execution_provenance(
        args=args,
        selected_dataset_ids=selected_dataset_ids,
        command=sys.argv,
        import_targets=_current_canonical_bootstrap_import_targets(),
        expected_import_targets=_CANONICAL_BOOTSTRAP_IMPORT_TARGETS,
        materialized_input_records=materialized_input_records,
        worker_module_closures=worker_module_closures,
        worker_module_closures_complete=worker_module_closures_complete,
    )


def _build_noncanonical_execution_contract(
    args: argparse.Namespace,
    selected_dataset_ids: Sequence[str],
    materialized_input_records: Sequence[Mapping[str, Any]],
    worker_module_closures: Sequence[Mapping[str, Any]] = (),
    ...
    *,
    ...
    return build_noncanonical_execution_provenance(
        args=args,
        selected_dataset_ids=selected_dataset_ids,
        command=sys.argv,
        import_targets=_current_canonical_bootstrap_import_targets(),
        ...
        preserve_materialized_input_identity=preserve_materialized_input_identity,
    )


def _attach_execution_row_fields(
...
def _attach_execution_frame_fields(frame: pd.DataFrame, fields: Mapping[str, Any]) -> pd.DataFrame:
    for key, value in fields.items():
        frame[str(key)] = value
    return frame


def _attach_materialized_input_identity_fields(
    records: Sequence[Dict[str, Any]],
    materialized_input_records: Sequence[Mapping[str, Any]],
) -> None:
    """Attach each row's exact X/y/source/split identity before CSV emission."""

    by_task = {
        (str(record["dataset_id"]), int(record["seed"])): str(
            record["materialized_input_sha256"]
        )
        for record in materialized_input_records
    }
    for record in records:
        if "dataset_id" not in record or "seed" not in record:
            continue
        key = (str(record["dataset_id"]), int(record["seed"]))
        digest = by_task.get(key)
        if not digest:
            raise RuntimeError(
                "Result record has no matching materialized input identity: "
                f"dataset={key[0]!r} seed={key[1]}."
            )
        record["materialized_input_identity_sha256"] = digest


def _collect_task_execution_module_closures(
    ...
    *,
    ...
    """Collect worker-local closure snapshots returned with one task result."""

    task_key = f"{dataset_id}:{int(seed)}"
    primary = task_result.get("execution_module_closure")
    if isinstance(primary, Mapping):
        closures.append(primary)
    else:
        issues.append(f"{task_key}:task_closure_missing")

    nested = task_result.get("nested_execution_module_closures", ())
    if not isinstance(nested, (list, tuple)):
        issues.append(f"{task_key}:nested_closures_malformed")
    else:
        for index, closure in enumerate(nested):
            if isinstance(closure, Mapping):
                closures.append(closure)
            else:
                issues.append(f"{task_key}:nested_closure_{index}_malformed")
    if task_result.get("execution_module_closure_complete") is not True:
        issues.append(f"{task_key}:worker_closure_incomplete")
...
MODEL_CANDIDATE_PROFILES: Dict[str, Tuple[str, ...]] = {
    ...
    # BeyondArena parity profile: paper methods that have an executable
    # tabnetics backend. TabICLv2/TabDPT remain deterministic skipped methods
    # in the comparison inventory until opt-in adapters are integrated.
    "beyondarena_parity": BEYONDARENA_PARITY_BACKENDS,
}
...
__tabnetics_execution_isolated_state__ = {
    "MODEL_CANDIDATE_PROFILES": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_BOOTSTRAP_IMPORT_TARGETS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_CLASSIFICATION_CONFIG_IMPORT_TARGET": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_DFFS_CONFIG_IMPORT_TARGET": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_DIST_CONFIG_IMPORT_TARGET": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_FEATURE_SELECTOR_IMPORT_TARGET": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_CANONICAL_PIPELINE_IMPORT_TARGET": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    ...
    "_DIAKRINO_CANONICAL_PROTECTED_ARMS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_DIAKRINO_CANONICAL_ARMS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "_DIAKRINO_CLOSEOUT_ARMS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
}
...
class SotaMatchedProtocolError(ValueError):
    """Raised when an opt-in published-classifier protocol is not executable."""


def _sota_protocol_dataset_id(dataset_id: str) -> Optional[str]:
    ds = str(dataset_id).strip()
    if ds in DATASET_SOTA_MATCHED_PROTOCOLS:
        return ds
    parent = _INTEGRATED_PARENT_MAP.get(ds)
    if parent and parent in DATASET_SOTA_MATCHED_PROTOCOLS:
        return parent
    return None


def _get_sota_matched_protocol_for_dataset(
    dataset_id: str,
) -> Optional[SotaMatchedClassifierProtocol]:
    """Return the typed published-classifier contract for a dataset or parent."""

    protocol_dataset_id = _sota_protocol_dataset_id(dataset_id)
    if protocol_dataset_id is None:
        return None
    return DATASET_SOTA_MATCHED_PROTOCOLS[protocol_dataset_id]


def _resolve_sota_matched_protocol_for_dataset(
    dataset_id: str,
) -> Optional[Tuple[SotaMatchedClassifierProtocol, Tuple[str, ...]]]:
    """Resolve a protocol to canonical executable classifier identities.

    Unknown, alias-colliding, or structurally incomplete records are a protocol
    error rather than a silent candidate substitution.
    """

    protocol = _get_sota_matched_protocol_for_dataset(dataset_id)
    if protocol is None:
        return None
    if protocol.match_status == SOTA_MATCH_STATUS_UNAVAILABLE:
        if protocol.candidates:
            raise SotaMatchedProtocolError(
                f"Unavailable SOTA protocol for {dataset_id!r} declares candidates."
            )
        return protocol, tuple()
    if protocol.structured_state_required:
        raise SotaMatchedProtocolError(
            f"SOTA protocol for {dataset_id!r} requires structured rule "
            f"{protocol.structured_rule_id!r}, but the generic matched-classifier "
            "lane has no registered structured-rule executor."
        )
    canonical: list[str] = []
    for candidate in protocol.candidates:
        try:
            resolved = canonical_classifier_name(candidate)
        except KeyError as exc:
            raise SotaMatchedProtocolError(
                f"SOTA protocol for {dataset_id!r} references unknown classifier "
                f"{candidate!r}."
            ) from exc
        canonical.append(resolved)
    if not canonical:
        raise SotaMatchedProtocolError(
            f"SOTA protocol for {dataset_id!r} has no executable candidates."
        )
    if len(canonical) != len(set(canonical)):
        raise SotaMatchedProtocolError(
            f"SOTA protocol for {dataset_id!r} collapses to duplicate canonical classifiers."
        )
    return protocol, tuple(canonical)


def _get_sota_classifiers_for_dataset(dataset_id: str) -> Optional[Tuple[str, ...]]:
    """Compatibility helper returning only validated executable candidates."""

    resolved = _resolve_sota_matched_protocol_for_dataset(dataset_id)
    if resolved is None:
        return None
    _protocol, candidates = resolved
    return candidates or None


def _sota_protocol_record(
    protocol: SotaMatchedClassifierProtocol,
    candidates: Sequence[str],
    *,
    rule_dataset_id: str,
) -> Dict[str, Any]:
    record = dict(protocol.to_record())
    record["rule_dataset_id"] = str(rule_dataset_id)
    record["candidates"] = [str(candidate) for candidate in candidates]
    return record


def _sota_row_metadata(protocol: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if protocol is None:
        return {
            "sota_matched_protocol_status": "not_applicable",
            "sota_matched_rule_dataset_id": "",
            "sota_matched_protocol_source": "",
            "sota_matched_selector_classifier_coupling": "",
            "sota_matched_structured_state_required": 0,
            "sota_matched_structured_rule_id": "",
            "sota_matched_unavailable_requirements": "[]",
            "sota_matched_candidates": "[]",
            "sota_matched_protocol_notes": "",
        }
    return {
        "sota_matched_protocol_status": str(protocol.get("match_status", "unknown")),
        "sota_matched_rule_dataset_id": str(protocol.get("rule_dataset_id", "")),
        "sota_matched_protocol_source": str(protocol.get("source", "")),
        "sota_matched_selector_classifier_coupling": str(
            protocol.get("selector_classifier_coupling", "")
        ),
        "sota_matched_structured_state_required": int(
            bool(protocol.get("structured_state_required", False))
        ),
        "sota_matched_structured_rule_id": str(protocol.get("structured_rule_id", "")),
        "sota_matched_unavailable_requirements": json.dumps(
            list(protocol.get("unavailable_requirements", []) or []),
            sort_keys=True,
        ),
        "sota_matched_candidates": json.dumps(
            list(protocol.get("candidates", []) or []),
            sort_keys=True,
        ),
        "sota_matched_protocol_notes": str(protocol.get("notes", "")),
    }


def _sota_summary_list(value: Any) -> List[str]:
    """Decode the flattened row representation without losing malformed data."""
...
def _sota_summary_protocol(row: Dict[str, Any]) -> Dict[str, Any]:
    """Restore the typed matched-protocol record in compact summaries."""

    return {
        "schema_version": "sota_matched_classifier_protocol_v1",
        "match_status": str(row.get("sota_matched_protocol_status", "not_applicable")),
        "rule_dataset_id": str(row.get("sota_matched_rule_dataset_id", "")),
        "source": str(row.get("sota_matched_protocol_source", "")),
        "selector_classifier_coupling": str(
            row.get("sota_matched_selector_classifier_coupling", "")
        ),
        "structured_state_required": bool(
            row.get("sota_matched_structured_state_required", False)
        ),
        "structured_rule_id": str(row.get("sota_matched_structured_rule_id", "")),
        "unavailable_requirements": _sota_summary_list(
            row.get("sota_matched_unavailable_requirements", "[]")
        ),
        "candidates": _sota_summary_list(row.get("sota_matched_candidates", "[]")),
        "executed_classifier": str(row.get("sota_matched_executed_classifier", "")),
        "executed_canonical_classifier": str(
            row.get("sota_matched_executed_canonical_classifier", "")
        ),
        "notes": str(row.get("sota_matched_protocol_notes", "")),
    }


def _validate_sota_matched_execution(
    protocol: Dict[str, Any],
    *,
    model_name: Any,
) -> tuple[Dict[str, str], str | None]:
    """Ensure the fitted Stage-2 model is one declared by its protocol."""

    raw_model_name = str(model_name or "").strip()
    metadata = {
        "sota_matched_executed_classifier": raw_model_name,
        "sota_matched_executed_canonical_classifier": "",
    }
    try:
        canonical_model_name = canonical_classifier_name(raw_model_name)
    except KeyError:
        return metadata, (
            "Pipeline selected an unknown classifier outside the matched protocol: "
            f"{raw_model_name!r}."
        )
    metadata["sota_matched_executed_canonical_classifier"] = canonical_model_name
    declared_candidates = tuple(
        str(candidate) for candidate in protocol.get("candidates", []) or ()
    )
    if canonical_model_name not in declared_candidates:
        return metadata, (
            "Pipeline selected a classifier outside the matched protocol: "
            f"{canonical_model_name!r} not in {declared_candidates!r}."
        )
...
def _apply_matched_model_candidate_set(cfg: DFFSConfig, candidates: Sequence[str]) -> None:
    # Ensure the config's candidate list is the declared matched set: include
...
def clone_config(cfg: DFFSConfig) -> DFFSConfig:
    dist = DistributionFitterConfig(
        ...
        flex_family_distinct_gate_enabled=bool(
            getattr(cfg.dist_config, "flex_family_distinct_gate_enabled", False)
        ),
        flex_family_min_distinct=int(getattr(cfg.dist_config, "flex_family_min_distinct", 15) or 15),
        flex_family_retention_margin=float(
            getattr(cfg.dist_config, "flex_family_retention_margin", 0.0) or 0.0
        ),
        ...
        diakrino_family_prior_lambda=float(getattr(cfg.dist_config, "diakrino_family_prior_lambda", 0.0) or 0.0),
        diakrino_skip_fit_discrete_enabled=bool(getattr(cfg.dist_config, "diakrino_skip_fit_discrete_enabled", False)),
        diakrino_sidecar_path=str(getattr(cfg.dist_config, "diakrino_sidecar_path", "") or ""),
        diakrino_sidecar_dataset_id=str(getattr(cfg.dist_config, "diakrino_sidecar_dataset_id", "") or ""),
    ...
    return DFFSConfig(
        ...
        diakrino_sidecar_path=str(getattr(cfg, "diakrino_sidecar_path", "") or ""),
        diakrino_sidecar_dataset_id=str(getattr(cfg, "diakrino_sidecar_dataset_id", "") or ""),
        diakrino_prefilter_enabled=bool(getattr(cfg, "diakrino_prefilter_enabled", False)),
        diakrino_prefilter_mode=str(
            getattr(cfg, "diakrino_prefilter_mode", "protected_union") or "protected_union"
        ),
        diakrino_prefilter_lambda=float(getattr(cfg, "diakrino_prefilter_lambda", 0.0) or 0.0),
        diakrino_prefilter_max_extras=int(
            max(0, int(getattr(cfg, "diakrino_prefilter_max_extras", 50) or 0))
        ),
        diakrino_prefilter_score_column=str(
            getattr(cfg, "diakrino_prefilter_score_column", "prior_logit") or "prior_logit"
        ),
        diakrino_prefilter_shadow_probe_indices=tuple(
            int(value)
            for value in tuple(
                getattr(cfg, "diakrino_prefilter_shadow_probe_indices", tuple()) or tuple()
            )
        ),
        diakrino_cdf_trust_gate_enabled=bool(getattr(cfg, "diakrino_cdf_trust_gate_enabled", False)),
        diakrino_cdf_trust_entropy_threshold=float(getattr(cfg, "diakrino_cdf_trust_entropy_threshold", 1.01) or 1.01),
        diakrino_cdf_trust_fallback=str(getattr(cfg, "diakrino_cdf_trust_fallback", "rank_gaussian") or "rank_gaussian"),
        diakrino_stability_surrogate_enabled=bool(getattr(cfg, "diakrino_stability_surrogate_enabled", False)),
        diakrino_regime_conditional=bool(getattr(cfg, "diakrino_regime_conditional", False)),
        diakrino_prior_score_column=str(getattr(cfg, "diakrino_prior_score_column", "prior_logit") or "prior_logit"),
        diakrino_screening_score_column=str(
            getattr(cfg, "diakrino_screening_score_column", "screening_logit") or "screening_logit"
        ),
        diakrino_prior_calibrate=str(getattr(cfg, "diakrino_prior_calibrate", "chunk_zscore") or "chunk_zscore"),
        diakrino_prior_top_k=int(max(0, int(getattr(cfg, "diakrino_prior_top_k", 0) or 0))),
        fs_use_diakrino_relevance_oracle=bool(getattr(cfg, "fs_use_diakrino_relevance_oracle", False)),
        fs_diakrino_relevance_min_n_train=int(
            max(2, int(getattr(cfg, "fs_diakrino_relevance_min_n_train", 100) or 100))
        ),
        fs_diakrino_relevance_score_column=str(
            getattr(cfg, "fs_diakrino_relevance_score_column", "prior_logit") or "prior_logit"
        ),
        fs_oracle_complexity_conditioning=bool(getattr(cfg, "fs_oracle_complexity_conditioning", False)),
        ...
        fs_method_max_rss_mb=cfg.fs_method_max_rss_mb,
        ...
        fs_oracle_enable_stability=bool(
            getattr(cfg, "fs_oracle_enable_stability", cfg.use_stability_oracle)
        ),
        ...
        fs_mrmr_max_unique_pair_evaluations=int(
            getattr(cfg, "fs_mrmr_max_unique_pair_evaluations", 0) or 0
        ),
        fs_mrmr_max_runtime_seconds=float(
            getattr(cfg, "fs_mrmr_max_runtime_seconds", 0.0) or 0.0
        ),
        fs_mrmr_budget_fallback_mode=str(
            getattr(cfg, "fs_mrmr_budget_fallback_mode", "empty") or "empty"
        ),
        ...
        classifier_oracle_enable_worst_class=bool(
            getattr(cfg, "classifier_oracle_enable_worst_class", True)
        ),
        ...
        calibration_reporting_enabled=bool(
            getattr(cfg, "calibration_reporting_enabled", False)
        ),
        classifier_posthoc_calibration_enabled=bool(
            getattr(cfg, "classifier_posthoc_calibration_enabled", False)
        ),
        classifier_posthoc_calibration_method=str(
            getattr(cfg, "classifier_posthoc_calibration_method", "sigmoid") or "sigmoid"
        ),
        classifier_posthoc_calibration_fraction=float(
            getattr(cfg, "classifier_posthoc_calibration_fraction", 0.20) or 0.20
        ),
        classifier_posthoc_calibration_min_calibration=int(
            getattr(cfg, "classifier_posthoc_calibration_min_calibration", 20) or 20
        ),
        classifier_posthoc_calibration_refinement_stopping=bool(
            getattr(cfg, "classifier_posthoc_calibration_refinement_stopping", True)
        ),
        ...
        auto_router_enabled=bool(getattr(cfg, "auto_router_enabled", False)),
        auto_router_artifact_path=str(
            getattr(cfg, "auto_router_artifact_path", "") or ""
        ).strip(),
        auto_router_fail_open=bool(getattr(cfg, "auto_router_fail_open", True)),
        auto_router_descriptor_ood_gate_enabled=bool(
            getattr(cfg, "auto_router_descriptor_ood_gate_enabled", False)
        ),
        diakrino_router_dispersion_descriptor_enabled=bool(
            getattr(cfg, "diakrino_router_dispersion_descriptor_enabled", False)
        ),
...
def _validate_diakrino_canonical_campaign_args(args: argparse.Namespace) -> None:
    """Fail before materialization when the canonical DIAKRINO lane is underspecified."""

    arm = str(getattr(args, "diakrino_feature_selection_arm", "off") or "off").strip().lower()
    split_policy = str(getattr(args, "diakrino_split_policy", "pipeline_seed") or "pipeline_seed").strip().lower()
    contract_path = str(getattr(args, "diakrino_sidecar_contract", "") or "").strip()
    requested = bool(contract_path or arm != "off" or split_policy != "pipeline_seed")
    if not requested:
        return
    if arm not in _DIAKRINO_CANONICAL_ARMS:
        raise ValueError(f"Unsupported canonical DIAKRINO feature-selection arm: {arm!r}.")
    if split_policy != "hf_probe":
        raise ValueError(
            "Canonical DIAKRINO sidecar execution requires --diakrino-split-policy hf_probe."
        )
    if not contract_path:
        raise ValueError(
            "Canonical DIAKRINO sidecar execution requires --diakrino-sidecar-contract; "
            "directory-only or direct-parquet replay is noncanonical."
        )
    checkpoint_sha256 = str(
        getattr(args, "diakrino_expected_checkpoint_sha256", "") or ""
    ).strip().lower()
    if len(checkpoint_sha256) != 64 or any(
        char not in "0123456789abcdef" for char in checkpoint_sha256
    ):
        raise ValueError(
            "Canonical DIAKRINO sidecar execution requires a lowercase SHA-256 via "
            "--diakrino-expected-checkpoint-sha256."
        )
    calibration = str(
        getattr(args, "diakrino_calibration", DIAKRINO_SIDECAR_REQUIRED_CALIBRATION)
        or DIAKRINO_SIDECAR_REQUIRED_CALIBRATION
    ).strip().lower()
    if calibration != DIAKRINO_SIDECAR_REQUIRED_CALIBRATION:
        raise ValueError("Canonical DIAKRINO feature selection requires chunk_zscore calibration.")
    score_column = str(getattr(args, "diakrino_score_column", "") or "").strip()
    if not score_column:
        raise ValueError("Canonical DIAKRINO feature selection requires a sidecar score column.")
    if arm in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS) and (
        score_column != _DIAKRINO_UNIFORM_VIEW_SCORE_COLUMN
    ):
        raise ValueError(
            "protected_uniform_views is coupled exactly to "
            f"--diakrino-score-column {_DIAKRINO_UNIFORM_VIEW_SCORE_COLUMN}."
        )
    if arm in {"protected_union", "protected_direct_fusion"} and score_column != (
        _DIAKRINO_DIRECT_FUSION_SCORE_COLUMN
    ):
        raise ValueError(
            "the direct DIAKRINO fusion arm is coupled exactly to "
            f"--diakrino-score-column {_DIAKRINO_DIRECT_FUSION_SCORE_COLUMN}."
        )
    if int(getattr(args, "diakrino_prefilter_max_extras", 0) or 0) < 0:
        raise ValueError("--diakrino-prefilter-max-extras must be non-negative.")
    if arm in _DIAKRINO_CLOSEOUT_ARMS:
        from tabnetics.feature_selection.diakrino_closeout import DiakrinoCloseoutConfig

        DiakrinoCloseoutConfig(
            shadow_margin_min=float(args.diakrino_shadow_margin_min),
            label_null_margin_min=float(args.diakrino_label_null_margin_min),
            rank_std_max=float(args.diakrino_rank_std_max),
            selected_set_stability_min=float(args.diakrino_stability_min),
            proposal_pool_multiplier=int(args.diakrino_proposal_pool_multiplier),
            discretization_bins=int(args.diakrino_jmi_discretization_bins),
        ).validate()
        is_control = arm in {
            "classical_next_best",
            "random_extras",
        ...
        realized = int(args.diakrino_p4_realized_additions)
        authority_path = str(getattr(args, "diakrino_p4_budget_authority", "") or "").strip()
        if is_control and (realized != -1 or not authority_path):
            raise ValueError(
                "C1-C3 require --diakrino-p4-budget-authority and reject "
                "--diakrino-p4-realized-additions."
            )
        if is_control and (
            len(tuple(getattr(args, "seeds", ()) or ())) != 1
            or len(tuple(getattr(args, "datasets", ()) or ())) != 1
            or bool(tuple(getattr(args, "dataset_sets", ()) or ()))
        ):
            raise ValueError(
                "C1-C3 use a two-phase P4 contract and require exactly one explicit "
                "dataset/seed task per immutable P4 authority artifact."
            )
        if not is_control and realized != -1:
            raise ValueError("P3/P4 require --diakrino-p4-realized-additions -1.")
        if arm == "protected_native_null_jmi" and (
            len(tuple(getattr(args, "seeds", ()) or ())) != 1
            or len(tuple(getattr(args, "datasets", ()) or ())) != 1
            or bool(tuple(getattr(args, "dataset_sets", ()) or ()))
        ):
            raise ValueError(
                "P4 emits one immutable authority artifact and requires exactly one explicit "
                "dataset/seed task."
            )
    train_fraction = float(getattr(args, "diakrino_probe_train_fraction", 0.8) or 0.8)
    if not np.isfinite(train_fraction) or not 0.0 < train_fraction < 1.0:
        raise ValueError("--diakrino-probe-train-fraction must be strictly between zero and one.")
    if tuple(_resolve_model_candidates_from_args(args)) != ("lr",):
        raise ValueError(
            "Canonical DIAKRINO feature-selection arms require the downstream classifier "
            "to be pinned exactly with --model-candidates lr."
        )
    if str(getattr(args, "classification_backend", "sklearn") or "sklearn") != "sklearn":
        raise ValueError("Canonical DIAKRINO feature-selection arms require the sklearn LR backend.")
    if str(getattr(args, "classifier_selection_mode", "legacy") or "legacy") != "legacy":
        raise ValueError("Canonical DIAKRINO feature-selection arms require legacy single-LR selection.")
    if str(getattr(args, "ablation_profile", "core") or "core") != "none":
        raise ValueError(
            "Canonical DIAKRINO arms require --ablation-profile none; compare predeclared arms "
            "as separate, identity-matched runs."
        )
    if bool(getattr(args, "use_sota_matched_classifiers", False)):
        raise ValueError("SOTA-matched classifier overlays are incompatible with pinned-LR DIAKRINO arms.")
    if bool(getattr(args, "enable_nestedcv_audit", False)):
        raise ValueError(
            "Nested-CV audit is incompatible with an outer-support DIAKRINO sidecar because "
            "its inner test folds were visible to the sidecar producer."
        )


def _diakrino_hf_probe_split_indices(
    y: np.ndarray,
    *,
    dataset_id: str,
    base_seed: int,
    ...
    """Reproduce the fresh DIAKRINO emitter's ordered support/query assignment."""

    fraction = float(train_fraction)
    if not np.isfinite(fraction) or not 0.0 < fraction < 1.0:
        raise ValueError("DIAKRINO hf_probe train fraction must be strictly between zero and one.")
    resolved_seed = int(base_seed) + (
        int(zlib.adler32(str(dataset_id).encode("utf-8"))) % 1_000_000
    )
    ...
    for label in np.unique(labels):
        ...
        rng.shuffle(indices)
        if indices.size <= 1:
            train.extend(int(value) for value in indices.tolist())
            continue
        n_test = max(1, int(round(float(indices.size) * (1.0 - fraction))))
        n_test = min(n_test, int(indices.size) - 1)
        test.extend(int(value) for value in indices[:n_test].tolist())
        train.extend(int(value) for value in indices[n_test:].tolist())
    train_indices = np.asarray(sorted(train), dtype=np.int64)
    test_indices = np.asarray(sorted(test), dtype=np.int64)
    assigned = np.sort(np.concatenate((train_indices, test_indices)))
    if (
        train_indices.size <= 0
        or test_indices.size <= 0
        or not np.array_equal(assigned, np.arange(labels.size, dtype=np.int64))
    ):
        raise ValueError(
            f"DIAKRINO hf_probe split is unusable for {dataset_id!r}: "
            f"train={train_indices.size} test={test_indices.size}."
        )
    return train_indices, test_indices, resolved_seed


_DIAKRINO_CANONICAL_CAMPAIGN_SCHEMA_VERSION = "diakrino_canonical_campaign_bindings_v2"
_DIAKRINO_CANONICAL_RESULT_SCHEMA_VERSION = "diakrino_canonical_result_v1"
_DIAKRINO_DIRECT_FUSION_SCORE_COLUMN = "prior_screening_fusion_w075_logit"
_DIAKRINO_UNIFORM_VIEW_SCORE_COLUMN = "diakrino_uniform_view_rank01"
_DIAKRINO_CLOSEOUT_ARMS = frozenset(
    {
...
_DIAKRINO_CANONICAL_ARMS = frozenset(
    {
        "off",
        "protected_union",
        "protected_direct_fusion",
        "protected_uniform_views",
        *_DIAKRINO_CLOSEOUT_ARMS,
    }
)
_DIAKRINO_CANONICAL_PROTECTED_ARMS = _DIAKRINO_CANONICAL_ARMS - {"off"}
_DIAKRINO_CANONICAL_BINDING_DIGEST_FIELDS: Mapping[str, str] = {
    "claim_manifest_sha256": "diakrino_sidecar_manifest_sha256",
    "qualification_record_sha256": "diakrino_qualification_record_sha256",
    "source_emission_manifest_sha256": "diakrino_source_emission_manifest_sha256",
    "feature_logits_sha256": "diakrino_feature_logits_sha256",
    "query_class_logits_sha256": "diakrino_query_class_logits_sha256",
    "aux_logits_sha256": "diakrino_aux_logits_sha256",
}
_DIAKRINO_INFERENCE_VIEWS_DIGEST_FIELD = "inference_views_sha256"
_DIAKRINO_NATIVE_NULLS_DIGEST_FIELD = "native_nulls_sha256"
_DIAKRINO_PAIRED_INFERENCE_VIEWS_DIGEST_FIELD = "paired_inference_views_sha256"


@dataclass(frozen=True)
class _CanonicalDiakrinoTaskBinding:
    campaign_contract_sha256: str
    campaign_contract_path: str
    manifest_path: str
    expected_identity: DiakrinoExpectedIdentity
    feature_logits_path: str
    train_indices: Tuple[int, ...]
    test_indices: Tuple[int, ...]
    split_seed: int
    split_policy: str
    score_column: str
    sealed_feature_scores: Tuple[float, ...]
    row_identity_fields: Mapping[str, Any]
    closeout_evidence: Mapping[str, Any] | None = None

    def resampling_context(
        self,
        *,
        n_rows: int,
        batch_labels: Optional[np.ndarray],
    ) -> Tuple[FitResamplingContext, str]:
        if int(n_rows) != len(self.train_indices) + len(self.test_indices):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO support/query rows do not cover the canonical materialized input."
            )
        base = FitResamplingContext(
            n_rows=int(n_rows),
            row_ids=tuple(range(int(n_rows))),
            batch_ids=(
                tuple()
                if batch_labels is None
                else tuple(np.asarray(batch_labels, dtype=object).ravel().tolist())
            ),
        )
        split_id = f"diakrino-{self.split_policy}-{int(self.split_seed)}"
        assignment = SplitAssignment(
            scope="outer",
            split_id=split_id,
            train_indices=self.train_indices,
            test_indices=self.test_indices,
            source="diakrino_strict_sidecar_identity",
            parent_context_fingerprint=base.base_fingerprint,
        )
        # The immutable outer assignment is selected by its exact split ID at
        # the execution boundary.  Preserve the base IID policy so that the
        # train-only context can resolve fresh deterministic inner CV folds;
        # making the whole context ``supplied`` would require inner assignments
        # that cannot be present in this sidecar-bound outer protocol.
        return base.with_supplied_splits((assignment,), policy=base.policy), split_id

    def row_fields(self, *, arm: str) -> Dict[str, Any]:
        return {
            **dict(self.row_identity_fields),
            "diakrino_result_schema_version": _DIAKRINO_CANONICAL_RESULT_SCHEMA_VERSION,
            "diakrino_campaign_contract_sha256": self.campaign_contract_sha256,
            "diakrino_feature_selection_arm": str(arm),
            "diakrino_pinned_classifier": "lr",
            "diakrino_split_policy": self.split_policy,
            "diakrino_split_seed": int(self.split_seed),
            "diakrino_score_column": str(self.score_column),
            "diakrino_score_calibration": (
                "none"
                if self.score_column == _DIAKRINO_UNIFORM_VIEW_SCORE_COLUMN
                else DIAKRINO_SIDECAR_REQUIRED_CALIBRATION
            ),
            "diakrino_claim_eligible": True,
        }


def _resolve_diakrino_campaign_entry(
    contract_path: str,
    *,
    dataset_id: str,
    seed: int,
    require_inference_views: bool = False,
    require_native_nulls: bool = False,
    require_paired_inference_views: bool = False,
) -> Tuple[Path, DiakrinoExpectedIdentity, str, Mapping[str, str]]:
    ...
    try:
        ...
        raise DiakrinoSidecarIdentityError(
            f"DIAKRINO canonical campaign contract is unreadable: {path}."
        ) from exc
    if not isinstance(payload, Mapping) or str(payload.get("schema_version") or "") != (
        _DIAKRINO_CANONICAL_CAMPAIGN_SCHEMA_VERSION
    ):
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO canonical campaign contract is missing or has an unsupported schema."
        )
    raw_bindings = payload.get("bindings")
    if not isinstance(raw_bindings, list):
        raise DiakrinoSidecarIdentityError("DIAKRINO canonical campaign contract has no bindings list.")
    matches: List[Mapping[str, Any]] = []
    for raw in raw_bindings:
        if not isinstance(raw, Mapping):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO canonical campaign contract contains a malformed binding."
            )
        try:
            row_seed = exact_int64_scalar(
                raw.get("seed"),
                label="DIAKRINO canonical campaign binding seed",
            )
        except DiakrinoSidecarIdentityError as exc:
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO canonical campaign contract contains a malformed binding seed."
            ) from exc
        if str(raw.get("dataset_id") or "") == str(dataset_id) and row_seed == int(seed):
            matches.append(raw)
    if len(matches) != 1:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO canonical campaign contract must contain exactly one binding for "
            f"dataset={dataset_id!r} seed={int(seed)}; found {len(matches)}."
        )
    row = matches[0]
    manifest_raw = str(row.get("manifest_path") or "").strip()
    raw_identity = row.get("expected_identity")
    if not manifest_raw or not isinstance(raw_identity, Mapping):
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO canonical campaign binding lacks manifest_path or expected_identity."
        )
    manifest_path = Path(manifest_raw).expanduser()
    if not manifest_path.is_absolute():
        manifest_path = path.parent / manifest_path
    manifest_path = manifest_path.resolve()
    expected = DiakrinoExpectedIdentity.from_dict(raw_identity)
    if expected.dataset_id != str(dataset_id):
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO canonical campaign entry and expected identity name different datasets."
        )
    pinned_digests: dict[str, str] = {}
    for field in _DIAKRINO_CANONICAL_BINDING_DIGEST_FIELDS:
        value = str(row.get(field) or "").strip().lower()
        if len(value) != 64 or any(char not in "0123456789abcdef" for char in value):
            raise DiakrinoSidecarIdentityError(
                f"DIAKRINO canonical campaign binding has no valid {field}."
            )
        pinned_digests[field] = value
    raw_view_digest = row.get(_DIAKRINO_INFERENCE_VIEWS_DIGEST_FIELD)
    if raw_view_digest is not None or require_inference_views:
        value = str(raw_view_digest or "").strip().lower()
        if len(value) != 64 or any(char not in "0123456789abcdef" for char in value):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO canonical campaign binding has no valid "
                f"{_DIAKRINO_INFERENCE_VIEWS_DIGEST_FIELD}."
            )
        pinned_digests[_DIAKRINO_INFERENCE_VIEWS_DIGEST_FIELD] = value
    raw_native_digest = row.get(_DIAKRINO_NATIVE_NULLS_DIGEST_FIELD)
    if raw_native_digest is not None or require_native_nulls:
        value = str(raw_native_digest or "").strip().lower()
        if len(value) != 64 or any(char not in "0123456789abcdef" for char in value):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO canonical campaign binding has no valid native_nulls_sha256."
            )
        pinned_digests[_DIAKRINO_NATIVE_NULLS_DIGEST_FIELD] = value
    raw_paired_digest = row.get(_DIAKRINO_PAIRED_INFERENCE_VIEWS_DIGEST_FIELD)
    if raw_paired_digest is not None or require_paired_inference_views:
        value = str(raw_paired_digest or "").strip().lower()
        if len(value) != 64 or any(char not in "0123456789abcdef" for char in value):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO canonical campaign binding has no valid "
                f"{_DIAKRINO_PAIRED_INFERENCE_VIEWS_DIGEST_FIELD}."
            )
        pinned_digests[_DIAKRINO_PAIRED_INFERENCE_VIEWS_DIGEST_FIELD] = value
    return manifest_path, expected, sha256_bytes(contract_bytes), pinned_digests


def _resolve_diakrino_canonical_task_binding(
    args: argparse.Namespace,
    *,
    dataset_id: str,
    seed: int,
    X: np.ndarray,
    y: np.ndarray,
    support_indices: Optional[Sequence[int] | np.ndarray] = None,
    query_indices: Optional[Sequence[int] | np.ndarray] = None,
    identity_y: Optional[np.ndarray] = None,
) -> _CanonicalDiakrinoTaskBinding:
    from tabnetics.feature_selection.diakrino_sidecar import DiakrinoSidecar as _DiakrinoSidecar

    manifest_path, producer_expected, campaign_contract_sha256, pinned_digests = (
        _resolve_diakrino_campaign_entry(
            str(getattr(args, "diakrino_sidecar_contract", "") or ""),
            dataset_id=dataset_id,
            seed=int(seed),
            require_inference_views=(
                str(getattr(args, "diakrino_feature_selection_arm", "off") or "off")
                .strip()
                .lower()
                in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS)
            ),
            require_native_nulls=(
                str(getattr(args, "diakrino_feature_selection_arm", "off") or "off")
                .strip()
                .lower()
                in {"protected_native_null_abstain", "protected_native_null_jmi"}
            ),
            require_paired_inference_views=(
                str(getattr(args, "diakrino_feature_selection_arm", "off") or "off")
                .strip()
                .lower()
                in {"protected_native_null_abstain", "protected_native_null_jmi"}
    ...
    if (support_indices is None) != (query_indices is None):
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO explicit split binding requires both support and query indices."
        )
    if support_indices is None:
        train_indices, test_indices, split_seed = _diakrino_hf_probe_split_indices(
            y,
            dataset_id=dataset_id,
            base_seed=int(seed),
            train_fraction=float(getattr(args, "diakrino_probe_train_fraction", 0.8) or 0.8),
        )
        split_policy = "hf_probe"
    else:
        train_indices = exact_int64_vector(
            support_indices, label="DIAKRINO explicit support indices"
        )
        test_indices = exact_int64_vector(
            query_indices, label="DIAKRINO explicit query indices"
        )
        if (
            np.any(train_indices < 0)
            or np.any(test_indices < 0)
            or np.any(train_indices >= int(np.asarray(X).shape[0]))
            or np.any(test_indices >= int(np.asarray(X).shape[0]))
            or np.intersect1d(train_indices, test_indices).size
            or not np.array_equal(
                np.sort(np.concatenate((train_indices, test_indices))),
                np.arange(int(np.asarray(X).shape[0]), dtype=np.int64),
            )
        ):
            raise DiakrinoSidecarIdentityError(
                "DIAKRINO explicit support/query indices must be a disjoint complete partition."
            )
        split_seed = int(seed)
        split_policy = "official_fold"
    identity_labels = np.asarray(y if identity_y is None else identity_y).reshape(-1)
    if identity_labels.shape != np.asarray(y).reshape(-1).shape:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO identity labels must have the same shape as evaluation labels."
        )
    # The reviewed campaign contract is the independent authority for producer
    # source/model/source-manifest identities.  Current X/y/split/order and the
    # CLI-pinned checkpoint are reconstructed here; equality of the resulting
    # binding plus the recorded campaign-contract digest prevents silent edits.
    observed = DiakrinoExpectedIdentity.from_arrays(
        dataset_id=dataset_id,
        checkpoint_sha256=str(
            getattr(args, "diakrino_expected_checkpoint_sha256", "") or ""
        ),
        model_config_sha256=producer_expected.model_config_sha256,
        producer_source_sha256=producer_expected.producer_source_sha256,
        source_manifest_sha256=producer_expected.source_manifest_sha256,
        X=X,
        y=identity_labels,
        ...
        sidecar_schema_version=int(producer_expected.sidecar_schema_version),
    )
    if observed.binding_sha256 != producer_expected.binding_sha256:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO producer expected identity does not match canonical X/y/split/feature order."
        )
    if observed.sidecar_schema_version < 2:
        raise DiakrinoSidecarIdentityError(
            "Canonical DIAKRINO feature-selection evidence requires fresh sidecar schema v2."
        )
    sidecar = _DiakrinoSidecar.load_strict(
        manifest_path,
        observed,
        required_head="feature_selection",
    )
    if sidecar.claim_eligible is not True or sidecar.binding_sha256 != observed.binding_sha256:
        raise DiakrinoSidecarIdentityError("DIAKRINO strict sidecar did not retain claim-eligible identity.")
    score_column = str(getattr(args, "diakrino_score_column", "") or "").strip()
    scores = sidecar.scalar_scores(
        score_column,
        calibrate=(
            "none"
            if score_column == _DIAKRINO_UNIFORM_VIEW_SCORE_COLUMN
            else DIAKRINO_SIDECAR_REQUIRED_CALIBRATION
        ),
        require_calibration_safe=True,
    )
    if (
        scores is None
        or np.asarray(scores).shape != (int(X.shape[1]),)
        or not bool(np.all(np.isfinite(np.asarray(scores, dtype=float))))
    ):
        raise DiakrinoSidecarIdentityError(
            f"DIAKRINO strict sidecar score {score_column!r} is missing, non-finite, or misaligned."
        )
    diagnostics = sidecar.resolution_diagnostics()
    raw_artifacts = diagnostics.get("artifact_identities")
    if not isinstance(raw_artifacts, Mapping):
        raise DiakrinoSidecarIdentityError("DIAKRINO strict manifest has no artifact identities.")
    required_artifact_names = ("feature_logits", "query_class_logits", "aux_logits")
    artifact_identities = {
        name: ArtifactIdentity.from_dict(raw_artifacts[name])
        for name in (
        ...
        if isinstance(raw_artifacts.get(name), Mapping)
    }
    if any(name not in artifact_identities for name in required_artifact_names):
        raise DiakrinoSidecarIdentityError("DIAKRINO strict manifest is missing required artifacts.")
    arm = str(getattr(args, "diakrino_feature_selection_arm", "off") or "off").strip().lower()
    if arm in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS) and (
        "inference_views" not in artifact_identities
    ):
        raise DiakrinoSidecarIdentityError(
            "protected_uniform_views requires a verified inference_views artifact."
        )
    if arm in {"protected_native_null_abstain", "protected_native_null_jmi"} and (
        "native_nulls" not in artifact_identities
    ):
        raise DiakrinoSidecarIdentityError(
            "native-null DIAKRINO arms require a verified native_nulls artifact."
        )
    if arm in {"protected_native_null_abstain", "protected_native_null_jmi"} and (
        "paired_inference_views" not in artifact_identities
    ):
        raise DiakrinoSidecarIdentityError(
            "native-null DIAKRINO arms require a verified paired inference-view artifact."
        )
    feature_logits_path = str(diagnostics.get("source_path") or "")
    if not feature_logits_path:
        raise DiakrinoSidecarIdentityError("DIAKRINO strict sidecar has no resolved feature artifact path.")
    qualification_record_sha256 = str(
        diagnostics.get("qualification_record_sha256") or ""
    ).strip().lower()
    if len(qualification_record_sha256) != 64:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO strict sidecar has no verified qualification-record identity."
        )
    source_emission_manifest_sha256 = str(
        diagnostics.get("source_emission_manifest_sha256") or ""
    ).strip().lower()
    if len(source_emission_manifest_sha256) != 64:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO strict sidecar has no verified source-emission manifest identity."
        )
    claim_manifest_sha256 = str(
        diagnostics.get("claim_manifest_sha256") or ""
    ).strip().lower()
    if len(claim_manifest_sha256) != 64:
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO strict sidecar has no sealed claim-manifest identity."
        )
    observed_digests = {
        "claim_manifest_sha256": claim_manifest_sha256,
        "qualification_record_sha256": qualification_record_sha256,
        "source_emission_manifest_sha256": source_emission_manifest_sha256,
        "feature_logits_sha256": artifact_identities["feature_logits"].sha256,
        "query_class_logits_sha256": artifact_identities["query_class_logits"].sha256,
        "aux_logits_sha256": artifact_identities["aux_logits"].sha256,
    }
    if "inference_views" in artifact_identities:
        observed_digests[_DIAKRINO_INFERENCE_VIEWS_DIGEST_FIELD] = artifact_identities[
            "inference_views"
        ].sha256
    if "native_nulls" in artifact_identities:
        observed_digests[_DIAKRINO_NATIVE_NULLS_DIGEST_FIELD] = artifact_identities[
            "native_nulls"
        ].sha256
    if "paired_inference_views" in artifact_identities:
        observed_digests[_DIAKRINO_PAIRED_INFERENCE_VIEWS_DIGEST_FIELD] = artifact_identities[
            "paired_inference_views"
        ].sha256
    for field, pinned_digest in pinned_digests.items():
        if observed_digests.get(field) != pinned_digest:
            raise DiakrinoSidecarIdentityError(
                f"DIAKRINO canonical campaign binding {field} does not match the strict sidecar."
            )
    raw_view_diagnostics = diagnostics.get("inference_view_diagnostics")
    view_diagnostics = (
        dict(raw_view_diagnostics)
        if isinstance(raw_view_diagnostics, Mapping)
        else {}
    )
    if arm in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS):
        from tabnetics.feature_selection.diakrino_views import (
            DIAKRINO_FROZEN_VIEW_IDS,
            DIAKRINO_VIEW_SCORE_SOURCE,
        )

        if tuple(view_diagnostics.get("view_ids") or ()) != DIAKRINO_FROZEN_VIEW_IDS:
            raise DiakrinoSidecarIdentityError(
                "protected_uniform_views lacks the frozen inference-view ids."
            )
        if str(view_diagnostics.get("score_source") or "") != DIAKRINO_VIEW_SCORE_SOURCE:
            raise DiakrinoSidecarIdentityError(
                "protected_uniform_views has a mismatched score-source identity."
            )
    closeout_evidence: Mapping[str, Any] | None = None
    if arm in _DIAKRINO_CLOSEOUT_ARMS:
        view_values = sidecar.inference_view_values()
        if not isinstance(view_values, Mapping):
            raise DiakrinoSidecarIdentityError("closeout arm lacks strict inference-view values")
        closeout_evidence = {
            "arm": arm,
            "n_features": int(X.shape[1]),
            "real_rank": list(view_values["uniform_rank"]),
            "rank_std": list(view_values["uniform_rank_std"]),
            "ranks_by_view": list(view_values["rank01_by_view"]),
        }
        if arm in {"protected_native_null_abstain", "protected_native_null_jmi"}:
            from tabnetics.feature_selection.diakrino_closeout import (
                validate_paired_native_null_artifact,
            )

            native_payload = sidecar.native_null_payload()
            paired_payload = sidecar.paired_inference_view_payload()
            if not isinstance(native_payload, Mapping) or not isinstance(
                paired_payload, Mapping
            ):
                raise DiakrinoSidecarIdentityError(
                    "closeout arm lacks native-null paired evidence"
                )
            ...
            if (
                set(paired_payload) != expected_paired_keys
                or paired_payload.get("schema_version") != "diakrino_paired_view_ledger_v1"
                or paired_payload.get("binding_sha256") != observed.binding_sha256
                or paired_payload.get("dataset_id") != dataset_id
            ):
                raise DiakrinoSidecarIdentityError(
                    "closeout arm has a malformed paired inference-view ledger"
                )
            validated_native = validate_paired_native_null_artifact(
                native_payload,
                binding_sha256=observed.binding_sha256,
                X_support=np.asarray(X)[train_indices],
                y_support=np.asarray(y).reshape(-1)[train_indices],
                seed=int(seed),
                expected_paired_view_artifact_sha256=artifact_identities[
                    "paired_inference_views"
                ].sha256,
                expected_panel_chunks_by_view=paired_payload["layouts_by_view"],
                expected_real_rank01_by_view=paired_payload["real_rank01_by_view"],
            )
            closeout_evidence = {
                **dict(closeout_evidence),
                "real_rank": list(validated_native.real_rank),
                "rank_std": list(validated_native.rank_std),
                "ranks_by_view": [list(row) for row in validated_native.ranks_by_view],
                "shadow_rank": list(validated_native.shadow_rank),
                "label_null_rank": list(validated_native.label_null_rank),
                "thresholds": {
                    "shadow_margin_min": float(args.diakrino_shadow_margin_min),
                    "label_null_margin_min": float(args.diakrino_label_null_margin_min),
                    "rank_std_max": float(args.diakrino_rank_std_max),
            ...
            if arm in {"classical_next_best", "random_extras", "permuted_diakrino_ranks"}:
                authority, authority_sha256 = read_p4_budget_authority(
                    str(getattr(args, "diakrino_p4_budget_authority", "") or ""),
                    dataset_id=dataset_id,
                    seed=int(seed),
                    campaign_contract_sha256=campaign_contract_sha256,
                    binding_sha256=observed.binding_sha256,
                    native_nulls_sha256=artifact_identities["native_nulls"].sha256,
                    paired_inference_views_sha256=artifact_identities[
                        "paired_inference_views"
                    ].sha256,
                )
                closeout_evidence = {
                    **dict(closeout_evidence),
                    "p4_realized_additions": int(authority["p4_realized_additions"]),
                    "p4_budget_authority_sha256": authority_sha256,
                    "p4_authorized_realized_additions": int(
                        authority["p4_realized_additions"]
                    ),
                }
            else:
                closeout_evidence = {
                    **dict(closeout_evidence),
                    "p4_realized_additions": int(args.diakrino_p4_realized_additions),
                }
    row_fields = {
        **observed.frame_fields(),
        "diakrino_sidecar_manifest_sha256": claim_manifest_sha256,
        "diakrino_source_emission_manifest_sha256": source_emission_manifest_sha256,
        "diakrino_feature_logits_sha256": artifact_identities["feature_logits"].sha256,
        "diakrino_query_class_logits_sha256": artifact_identities["query_class_logits"].sha256,
        "diakrino_aux_logits_sha256": artifact_identities["aux_logits"].sha256,
        "diakrino_qualification_record_sha256": qualification_record_sha256,
    }
    if "inference_views" in artifact_identities:
        row_fields["diakrino_inference_views_sha256"] = artifact_identities[
            "inference_views"
        ].sha256
        row_fields.update(
            {
                "diakrino_inference_view_ids_json": json.dumps(
                    list(view_diagnostics.get("view_ids") or ()),
                    separators=(",", ":"),
                ),
                "diakrino_inference_view_score_source": str(
                    view_diagnostics.get("score_source") or ""
                ),
                "diakrino_uniform_view_rank_std_mean": float(
                    view_diagnostics.get("uniform_rank_std_mean", float("nan"))
                ),
                "diakrino_uniform_view_rank_std_median": float(
                    view_diagnostics.get("uniform_rank_std_median", float("nan"))
                ),
                "diakrino_uniform_view_rank_std_max": float(
                    view_diagnostics.get("uniform_rank_std_max", float("nan"))
                ),
                "diakrino_uniform_view_rank_std_nonzero_fraction": float(
                    view_diagnostics.get(
                        "uniform_rank_std_nonzero_fraction", float("nan")
                    )
                ),
            }
        )
    if "native_nulls" in artifact_identities:
        row_fields["diakrino_native_nulls_sha256"] = artifact_identities[
            "native_nulls"
        ].sha256
    if "paired_inference_views" in artifact_identities:
        row_fields["diakrino_paired_inference_views_sha256"] = artifact_identities[
            "paired_inference_views"
        ].sha256
    if isinstance(closeout_evidence, Mapping) and "p4_budget_authority_sha256" in closeout_evidence:
        row_fields["diakrino_p4_budget_authority_sha256"] = str(
            closeout_evidence["p4_budget_authority_sha256"]
        )
        row_fields["diakrino_p4_authorized_realized_additions"] = int(
            closeout_evidence["p4_authorized_realized_additions"]
        )
    return _CanonicalDiakrinoTaskBinding(
        campaign_contract_sha256=campaign_contract_sha256,
        campaign_contract_path=str(
            Path(str(getattr(args, "diakrino_sidecar_contract", "") or ""))
            .expanduser()
            .resolve()
        ),
        manifest_path=str(manifest_path),
        expected_identity=observed,
        feature_logits_path=feature_logits_path,
        train_indices=tuple(int(value) for value in train_indices.tolist()),
        test_indices=tuple(int(value) for value in test_indices.tolist()),
        split_seed=int(split_seed),
        split_policy=split_policy,
        score_column=score_column,
        sealed_feature_scores=tuple(
            float(value) for value in np.asarray(scores, dtype=float).tolist()
        ),
...
def _revalidate_diakrino_post_run_artifact_pins(
    binding: _CanonicalDiakrinoTaskBinding,
) -> None:
    """Require post-run evidence bytes to equal every initially pinned digest."""

    from tabnetics.feature_selection.diakrino_sidecar import DiakrinoSidecar as _DiakrinoSidecar

    try:
        post_run_campaign_sha256 = sha256_file(binding.campaign_contract_path)
    except Exception as exc:
        raise DiakrinoSidecarIdentityError(
            "post-run DIAKRINO campaign contract is unreadable"
        ) from exc
    if post_run_campaign_sha256 != binding.campaign_contract_sha256:
        raise DiakrinoSidecarIdentityError(
            "post-run DIAKRINO campaign contract changed during execution"
        )
    ...
    if (
        sidecar.claim_eligible is not True
        or sidecar.binding_sha256 != binding.expected_identity.binding_sha256
    ):
        raise DiakrinoSidecarIdentityError(
            "DIAKRINO strict identity was not preserved through execution"
        )
    diagnostics = sidecar.resolution_diagnostics()
    raw_artifacts = diagnostics.get("artifact_identities")
    if not isinstance(raw_artifacts, Mapping):
        raise DiakrinoSidecarIdentityError(
            "post-run DIAKRINO strict manifest has no artifact identities"
        )
    current: dict[str, str] = {
        "diakrino_sidecar_manifest_sha256": str(
            diagnostics.get("claim_manifest_sha256") or ""
        ).strip().lower(),
        "diakrino_source_emission_manifest_sha256": str(
            diagnostics.get("source_emission_manifest_sha256") or ""
        ).strip().lower(),
        "diakrino_qualification_record_sha256": str(
            diagnostics.get("qualification_record_sha256") or ""
        ).strip().lower(),
    }
    for artifact_name, result_field in (
        ("feature_logits", "diakrino_feature_logits_sha256"),
        ("query_class_logits", "diakrino_query_class_logits_sha256"),
        ("aux_logits", "diakrino_aux_logits_sha256"),
        ("inference_views", "diakrino_inference_views_sha256"),
        ("native_nulls", "diakrino_native_nulls_sha256"),
    ):
        raw_identity = raw_artifacts.get(artifact_name)
        if isinstance(raw_identity, Mapping):
            current[result_field] = ArtifactIdentity.from_dict(raw_identity).sha256
    required_fields = tuple(_DIAKRINO_CANONICAL_BINDING_DIGEST_FIELDS.values())
    if "diakrino_inference_views_sha256" in binding.row_identity_fields:
        required_fields = (*required_fields, "diakrino_inference_views_sha256")
    if "diakrino_native_nulls_sha256" in binding.row_identity_fields:
        required_fields = (*required_fields, "diakrino_native_nulls_sha256")
    for field in required_fields:
        expected = str(binding.row_identity_fields.get(field) or "").strip().lower()
        observed = str(current.get(field) or "").strip().lower()
        if len(expected) != 64 or observed != expected:
            raise DiakrinoSidecarIdentityError(
                f"post-run DIAKRINO evidence digest changed during execution: {field}"
            )


def _diakrino_protected_result_row_fields(
    config_snapshot: Mapping[str, Any],
    *,
    arm: str,
) -> Dict[str, Any]:
    """Flatten protected-union diagnostics into a stable analysis schema."""

    defaults: Dict[str, Any] = {
        "diakrino_protected_enabled": False,
        "diakrino_protected_applied": False,
        "diakrino_protected_mode": "off",
        "diakrino_protection_reason": "identity_matched_classical_baseline",
        "diakrino_protected_core_source": "",
        "diakrino_protected_core_size": 0,
        "diakrino_protected_core_retained_size": 0,
        "diakrino_protected_core_retention_rate": float("nan"),
        "diakrino_addition_budget": 0,
        "diakrino_ranked_candidate_count": 0,
        "diakrino_eligible_outside_candidate_count": 0,
        "diakrino_admitted_outside_candidate_count": 0,
        "diakrino_valid_finite_candidate_count": 0,
        "diakrino_budget_scan_count": 0,
        "diakrino_budget_scan_exhausted": False,
        "diakrino_additions": 0,
        "diakrino_classical_agreement_count": 0,
        "diakrino_classical_agreement_rate": float("nan"),
        "diakrino_shadow_probe_candidate_count": 0,
        "diakrino_shadow_probe_candidate_denominator": 0,
        "diakrino_shadow_probe_candidate_fraction": float("nan"),
        "diakrino_shadow_probe_rejected_count": 0,
        "diakrino_final_feature_count": 0,
        "diakrino_final_stability_available": False,
        "diakrino_final_stability_rate": float("nan"),
        "diakrino_final_stability_source": "unavailable",
        "diakrino_shadow_separation": float("nan"),
        "diakrino_label_null_separation": float("nan"),
        "diakrino_selected_set_stability": float("nan"),
        "diakrino_closeout_abstained": False,
        "diakrino_closeout_fallback_exact": False,
        "diakrino_closeout_control_budget_matched": False,
        "diakrino_jmi_ledger_json": "[]",
        "diakrino_requested_enabled_methods_json": "[]",
        "diakrino_protected_effective_methods_json": "[]",
        "diakrino_protected_core_original_indices_json": "[]",
        "diakrino_extra_original_indices_json": "[]",
        "diakrino_final_original_indices_json": "[]",
    }
    if str(arm) not in _DIAKRINO_CANONICAL_PROTECTED_ARMS:
        return defaults
    raw = config_snapshot.get("diakrino_protected_augmentation")
    if not isinstance(raw, Mapping):
        raise DiakrinoSidecarIdentityError(
            "Protected DIAKRINO execution emitted no diakrino_protected_augmentation diagnostics."
        )
    diag = dict(raw)
    if (
        diag.get("enabled") is not True
        or diag.get("applied") is not True
        or str(diag.get("mode") or "")
        != (str(arm) if str(arm) in _DIAKRINO_CLOSEOUT_ARMS else "protected_union")
    ):
        raise DiakrinoSidecarIdentityError(
            "Protected DIAKRINO execution emitted inactive or inconsistent augmentation diagnostics."
        )
    scalar_mapping = {
        "diakrino_protected_core_size": "protected_core_size",
        "diakrino_protected_core_retained_size": "protected_core_retained_size",
        "diakrino_protected_core_retention_rate": "protected_core_retention_rate",
        "diakrino_addition_budget": "diakrino_addition_budget",
        "diakrino_ranked_candidate_count": "diakrino_ranked_candidate_count",
        "diakrino_eligible_outside_candidate_count": "diakrino_eligible_outside_candidate_count",
        "diakrino_admitted_outside_candidate_count": "diakrino_admitted_outside_candidate_count",
        "diakrino_valid_finite_candidate_count": "diakrino_valid_finite_candidate_count",
        "diakrino_budget_scan_count": "diakrino_budget_scan_count",
        "diakrino_budget_scan_exhausted": "diakrino_budget_scan_exhausted",
        "diakrino_additions": "diakrino_additions",
        "diakrino_classical_agreement_count": "diakrino_classical_agreement_count",
        "diakrino_classical_agreement_rate": "diakrino_classical_agreement_rate",
        "diakrino_shadow_probe_candidate_count": "shadow_probe_candidate_count",
        "diakrino_shadow_probe_candidate_denominator": "shadow_probe_candidate_denominator",
        "diakrino_shadow_probe_candidate_fraction": "shadow_probe_candidate_fraction",
        "diakrino_shadow_probe_rejected_count": "shadow_probe_rejected_count",
        "diakrino_final_feature_count": "final_feature_count",
        "diakrino_final_stability_available": "final_stability_available",
        "diakrino_final_stability_rate": "final_stability_rate",
        "diakrino_final_stability_source": "final_stability_source",
    }
    defaults.update(
        {
            "diakrino_protected_enabled": True,
            "diakrino_protected_applied": True,
            "diakrino_protection_reason": str(diag.get("protection_reason") or ""),
            "diakrino_protected_core_source": str(diag.get("protected_core_source") or ""),
            "diakrino_protected_mode": str(diag.get("mode") or ""),
            "diakrino_shadow_separation": diag.get("shadow_separation", float("nan")),
            "diakrino_label_null_separation": diag.get(
                "label_null_separation", float("nan")
            ),
            "diakrino_selected_set_stability": diag.get(
                "selected_set_stability", float("nan")
            ),
            "diakrino_closeout_abstained": bool(diag.get("abstained", False)),
            "diakrino_closeout_fallback_exact": bool(diag.get("fallback_exact", False)),
            "diakrino_closeout_control_budget_matched": bool(
                diag.get("control_budget_matched", False)
            ),
            "diakrino_jmi_ledger_json": json.dumps(diag.get("jmi_ledger") or []),
            **{
                target: diag.get(source, defaults[target])
                for target, source in scalar_mapping.items()
            },
            "diakrino_requested_enabled_methods_json": json.dumps(
                list(diag.get("requested_enabled_methods") or ()), sort_keys=True
            ),
            "diakrino_protected_effective_methods_json": json.dumps(
                list(diag.get("protected_effective_methods") or ()), sort_keys=True
            ),
            "diakrino_protected_core_original_indices_json": json.dumps(
                list(diag.get("protected_core_original_indices") or ())
            ),
            "diakrino_extra_original_indices_json": json.dumps(
                list(diag.get("diakrino_extra_original_indices") or ())
            ),
            "diakrino_final_original_indices_json": json.dumps(
                list(diag.get("final_original_indices") or ())
            ),
        }
    )
    return defaults
...
def _apply_diakrino_canonical_binding_to_config(
    cfg: DFFSConfig,
    *,
    binding: _CanonicalDiakrinoTaskBinding,
    args: argparse.Namespace,
) -> None:
    """Apply one already-validated DIAKRINO binding without changing baseline defaults."""

    arm = str(getattr(args, "diakrino_feature_selection_arm", "off") or "off").strip().lower()
    if arm == "off":
        return
    if arm not in _DIAKRINO_CANONICAL_PROTECTED_ARMS:
        raise ValueError(f"Unsupported canonical DIAKRINO feature-selection arm: {arm!r}.")
    # Canonical execution consumes the already strict-validated in-memory score
    # vector.  Keep the artifact identity in result rows, not as a permissive
    # runtime reload path.
    cfg.diakrino_sidecar_path = ""
    cfg.diakrino_sidecar_dataset_id = str(binding.expected_identity.dataset_id)
    cfg.diakrino_prefilter_enabled = True
    cfg.diakrino_prefilter_mode = "protected_union"
    cfg.diakrino_prefilter_lambda = 1.0
    cfg.diakrino_prefilter_max_extras = int(
        len(binding.sealed_feature_scores)
        if arm in _DIAKRINO_CLOSEOUT_ARMS
        else max(0, int(getattr(args, "diakrino_prefilter_max_extras", 50) or 0))
    )
    cfg.diakrino_prefilter_score_column = str(binding.score_column)
    cfg.diakrino_prior_calibrate = (
        "none"
        if arm in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS)
        else DIAKRINO_SIDECAR_REQUIRED_CALIBRATION
    )
...
def _build_base_config(args: argparse.Namespace, spec: BenchmarkDatasetSpec, seed: int) -> DFFSConfig:
    ...
    classification_cfg = ClassificationConfig(
        ...
        oracle_include_worst_class_recall=not bool(
            getattr(args, "disable_classifier_oracle_worst_class", False)
        ),
        ...
        posthoc_calibration_enabled=bool(
            getattr(args, "enable_classifier_posthoc_calibration", False)
        ),
        posthoc_calibration_method=str(
            getattr(args, "classifier_posthoc_calibration_method", "sigmoid") or "sigmoid"
        ),
        posthoc_calibration_fraction=float(
            getattr(args, "classifier_posthoc_calibration_fraction", 0.20) or 0.20
        ),
        posthoc_calibration_min_calibration=int(
            getattr(args, "classifier_posthoc_calibration_min_calibration", 20) or 20
        ),
        posthoc_calibration_refinement_stopping=not bool(
            getattr(args, "disable_classifier_posthoc_calibration_refinement_stopping", False)
        ),
        stage2_ratio_augmentation_enabled=bool(
    ...
    cfg = DFFSConfig(
        ...
        dist_config=DistributionFitterConfig(
            ...
            flex_family_distinct_gate_enabled=bool(getattr(args, "df_flex_distinct_gate", False)),
            flex_family_min_distinct=int(getattr(args, "df_flex_min_distinct", 15) or 15),
            flex_family_retention_margin=float(getattr(args, "df_flex_retention_margin", 0.0) or 0.0),
        ...
        fs_method_max_rss_mb=float(getattr(args, "fs_method_max_rss_mb", 0.0) or 0.0),
        ...
        **({
        ...
        fs_oracle_enable_stability=not bool(getattr(args, "disable_fs_stability_oracle", False)),
        ...
        fs_mrmr_max_unique_pair_evaluations=int(
            getattr(args, "fs_mrmr_max_unique_pair_evaluations", 0) or 0
        ),
        fs_mrmr_max_runtime_seconds=float(
            getattr(args, "fs_mrmr_max_runtime_seconds", 0.0) or 0.0
        ),
        fs_mrmr_budget_fallback_mode=str(
            getattr(args, "fs_mrmr_budget_fallback_mode", "empty") or "empty"
        ),
        ...
        classifier_oracle_enable_worst_class=not bool(
            getattr(args, "disable_classifier_oracle_worst_class", False)
        ),
        ...
        calibration_reporting_enabled=bool(
            getattr(args, "enable_calibration_reporting", False)
        ),
        classifier_posthoc_calibration_enabled=bool(
            getattr(args, "enable_classifier_posthoc_calibration", False)
        ),
        ...
        classifier_posthoc_calibration_refinement_stopping=not bool(
            getattr(args, "disable_classifier_posthoc_calibration_refinement_stopping", False)
        ),
        classifier_oracle_portfolio_diversity=bool(
            getattr(args, "enable_classifier_oracle_portfolio_diversity", False)
...
_EXECUTION_MODULE_CLOSURE_ATTR = "_tabnetics_execution_module_closure"
_EXECUTION_MODULE_CLOSURE_MISSING_ATTR = "_tabnetics_execution_module_closure_missing"


def _attach_execution_module_closure(
    value: Any,
    closure: Mapping[str, Any],
) -> Any:
    """Carry a child process's validated closure with its pipeline result."""

    recorded = dict(closure)
    if isinstance(value, dict):
        value[_EXECUTION_MODULE_CLOSURE_ATTR] = recorded
        return value
    try:
        setattr(value, _EXECUTION_MODULE_CLOSURE_ATTR, recorded)
    except Exception as exc:
        raise CanonicalExecutionOriginError(
            "Pipeline result cannot carry its execution-module closure."
        ) from exc
    return value


def _execution_module_closure_from(value: Any) -> Mapping[str, Any] | None:
    if isinstance(value, Mapping):
        closure = value.get(_EXECUTION_MODULE_CLOSURE_ATTR)
    else:
        closure = getattr(value, _EXECUTION_MODULE_CLOSURE_ATTR, None)
    return dict(closure) if isinstance(closure, Mapping) else None


def _mark_execution_module_closure_missing(value: Any) -> None:
    try:
        setattr(value, _EXECUTION_MODULE_CLOSURE_MISSING_ATTR, True)
    except Exception:
        pass


def _run_pipeline_instance(
    pipeline: DistributionFeatureSelectionPipeline,
    *,
    X: np.ndarray,
    y: np.ndarray,
    ...
    supplied_split_id: Optional[str],
    capture_artifacts: bool,
    capture_diagnostics: bool,
    external_feature_scores: Optional[np.ndarray],
    diakrino_closeout_evidence: Optional[Mapping[str, Any]] = None,
) -> Any:
    """Run normally, or consume a sealed DIAKRINO vector on the exact supplied split."""

    pipeline._diakrino_closeout_evidence = (  # noqa: SLF001
        None if diakrino_closeout_evidence is None else dict(diakrino_closeout_evidence)
    )
    ...
    if resampling_context is None or not supplied_split_id:
        raise DiakrinoSidecarIdentityError(
            "Sealed DIAKRINO scores require an exact supplied resampling context."
        )
    plan = pipeline._resolve_outer_split_plan(
        np.asarray(y).ravel(),
        seed=int(seed),
        resampling_context=resampling_context,
        supplied_split_id=str(supplied_split_id),
    )
    train_indices = np.asarray(plan.primary.train_indices, dtype=int)
    test_indices = np.asarray(plan.primary.test_indices, dtype=int)
    labels = np.asarray(y).ravel()
    batches = None if batch_labels is None else np.asarray(batch_labels, dtype=object)
    return pipeline.run_pre_split(
        ...
        dataset_name=dataset_name,
        seed=int(seed),
        split_indices_train=train_indices,
        split_indices_test=test_indices,
        batch_labels_train=None if batches is None else batches[train_indices],
        batch_labels_test=None if batches is None else batches[test_indices],
        resampling_context=resampling_context,
        resolved_outer_split=plan,
        capture_artifacts=bool(capture_artifacts),
        capture_diagnostics=bool(capture_diagnostics),
        external_feature_scores=np.asarray(external_feature_scores, dtype=float),
    )
    ...
    try:
        with _maybe_suppress_worker_logs(bool(quiet_worker_logs)):
            ...
            result = _run_pipeline_instance(
                ...
                diakrino_closeout_evidence=diakrino_closeout_evidence,
            )
        result = _attach_execution_module_closure(
            result,
            capture_loaded_tabnetics_module_closure(),
        )
        ...
        try:
            ...
            try:
                error_payload[_EXECUTION_MODULE_CLOSURE_ATTR] = (
                    capture_loaded_tabnetics_module_closure()
                )
            except Exception:
                error_payload[_EXECUTION_MODULE_CLOSURE_MISSING_ATTR] = True
...
def _run_pipeline_with_hard_timeout(
    ...
    # The controller can call this helper directly in tests and integration
    # code, bypassing the task-level preflight. Seal live imported execution
    # bindings before either an in-process pipeline or a timeout child starts.
    validate_loaded_tabnetics_module_closure()
    ...
    if timeout <= 0.0:
        ...
        return _attach_execution_module_closure(
            result,
            capture_loaded_tabnetics_module_closure(),
        )
    ...
    if "resampling_context" in worker_params:
        worker_args.append(resampling_context)
    if "supplied_split_id" in worker_params:
        worker_args.append(supplied_split_id)
    if "external_feature_scores" in worker_params:
        worker_args.append(external_feature_scores)
    if "diakrino_closeout_evidence" in worker_params:
        worker_args.append(diakrino_closeout_evidence)
    ...
    if payload is None:
        if proc.is_alive():
            ...
            timeout_error = TimeoutError(
                f"Timed out after {timeout:.1f}s (hard-killed worker process)"
            )
            _mark_execution_module_closure_missing(timeout_error)
            raise timeout_error
    ...
    try:
        if payload_ok:
            ...
            if _execution_module_closure_from(result) is None:
                closure = payload.get(_EXECUTION_MODULE_CLOSURE_ATTR)
                if isinstance(closure, Mapping):
                    result = _attach_execution_module_closure(result, closure)
            return result
        worker_error = RuntimeError(str(payload.get("error", "Unknown worker failure")))
        closure = payload.get(_EXECUTION_MODULE_CLOSURE_ATTR)
        if isinstance(closure, Mapping):
            _attach_execution_module_closure(worker_error, closure)
        else:
            _mark_execution_module_closure_missing(worker_error)
        raise worker_error
    finally:
        if payload_result_path:
            try:
...
def _run_nestedcv_audit(
    ...
    pool_context = ensure_fit_resampling_context(
        resampling_context,
    ...
    for rep in range(int(max(1, args.nestedcv_outer_repeats))):
        ...
        try:
            rep_plan = resolve_cv(
                ...
                seed=cv_seed,
                purpose="nestedcv_outer",
                n_repeats=1,
                stratified=True,
                shuffle=True,
            )
            plan_fingerprints.append(str(rep_plan.fingerprint))
        except Exception as exc:
            out["nestedcv_failures"] += 1
            out["nestedcv_failure_example"] = str(exc)
            continue

        for fold_idx, resolved_fold in enumerate(rep_plan.splits):
            fold_seed = int(seed + rep * int(args.nestedcv_seed_stride) + 17 * fold_idx)
            eval_idx += 1
            try:
                fold_plan = resolve_assignment(
                    pool_context,
                    resolved_fold.assignment,
                    y=y_arr,
                    purpose="outer",
                )
                split_fingerprints.append(str(fold_plan.primary.fingerprint))
                tr_idx = np.asarray(fold_plan.primary.train_indices, dtype=int)
                te_idx = np.asarray(fold_plan.primary.test_indices, dtype=int)
                res = pipeline.run_pre_split(
                    ...
                    resampling_context=pool_context,
    ...
    out["nestedcv_resampling_context_fingerprint"] = str(
        pool_context.fingerprint
    )
    out["nestedcv_resampling_policy"] = pool_context.policy.to_record()
    out["nestedcv_resampling_plan_fingerprints"] = plan_fingerprints
    out["nestedcv_split_fingerprints"] = split_fingerprints
    if not bal_scores:
        out["nestedcv_skipped"] = 1
...
def _run_dataset_seed_task(
    ...
    # Joblib workers have their own module table, so validate their already
    # loaded package code before task execution as well as at final merge.
    validate_loaded_tabnetics_module_closure()
    ...
    nested_execution_module_closures: List[Mapping[str, Any]] = []
    execution_module_closure_complete = True
    diakrino_binding_fields: Dict[str, Any] = {}
    ...
    def _record_pipeline_execution_closure(value: Any) -> None:
        nonlocal execution_module_closure_complete
        closure = _execution_module_closure_from(value)
        if closure is not None:
            nested_execution_module_closures.append(closure)
            return
        # A task-process closure covers direct execution. A missing closure is
        # material only when the hard-timeout route may have executed code in a
        # child process whose module table is otherwise invisible here.
        if (
            bool(getattr(args, "max_workers", 1) > 1)
            and float(getattr(args, "task_timeout_sec", 0.0) or 0.0) > 0.0
        ):
            execution_module_closure_complete = False

    def _task_result(materialized_input_identity: Mapping[str, Any] | None) -> Dict[str, Any]:
        if diakrino_binding_fields:
            for collection in (rows, failures, model_bundles, run_diagnostics):
                for record in collection:
                    record.update(diakrino_binding_fields)
        return {
            "rows": rows,
            "failures": failures,
            "model_bundles": model_bundles,
            "run_diagnostics": run_diagnostics,
            "materialized_input_identity": materialized_input_identity,
            "execution_module_closure": capture_loaded_tabnetics_module_closure(),
            "nested_execution_module_closures": nested_execution_module_closures,
            "execution_module_closure_complete": execution_module_closure_complete,
        }
    ...
    materialized_input_identity = build_materialized_dataset_input_identity(
        dataset_id=ds_id,
        seed=int(seed),
        data_source=str(data_source),
        source_identity={
            "benchmark_source_kind": str(getattr(spec, "source_kind", "") or ""),
            "validation_dataset_id": str(
                getattr(spec, "validation_dataset_id", "") or ""
            ),
            "validation_loader_kind": str(
                getattr(
                    CATALOG.get(str(getattr(spec, "validation_dataset_id", "") or "")),
                    "loader_kind",
                    "",
                )
                or ""
            ...
            "source_policy": str(promotion_meta["source_policy"]),
            "synthetic_sample_cap": int(args.synthetic_sample_cap),
            "synthetic_feature_cap": int(args.synthetic_feature_cap),
        },
        X=X,
        y=y,
        batch_labels=batch_labels,
    ...
    diakrino_binding: Optional[_CanonicalDiakrinoTaskBinding] = None
    diakrino_resampling_context: Optional[FitResamplingContext] = None
    diakrino_supplied_split_id: Optional[str] = None
    diakrino_contract_path = str(getattr(args, "diakrino_sidecar_contract", "") or "").strip()
    if diakrino_contract_path:
        try:
            diakrino_binding = _resolve_diakrino_canonical_task_binding(
                args,
                dataset_id=ds_id,
                seed=int(seed),
                X=X,
            ...
            diakrino_resampling_context, diakrino_supplied_split_id = diakrino_binding.resampling_context(
                n_rows=int(y.size),
                batch_labels=batch_labels,
            )
            diakrino_binding_fields.update(
                diakrino_binding.row_fields(
                    arm=str(getattr(args, "diakrino_feature_selection_arm", "off") or "off")
                )
            )
        except DiakrinoSidecarIdentityError as exc:
            failures.append(
                {
                    "dataset_id": ds_id,
                    "seed": int(seed),
                    "status": "failed_diakrino_sidecar_binding",
                    "error": str(exc),
                }
            )
            _emit_task_done("failed_diakrino_sidecar_binding")
            return _task_result(materialized_input_identity)

    base_cfg = _build_base_config(args=args, spec=spec, seed=seed)
    if diakrino_binding is not None:
        _apply_diakrino_canonical_binding_to_config(base_cfg, binding=diakrino_binding, args=args)
    configs = _build_ablation_configs(base_cfg, profile=args.ablation_profile)

    # Optional: evaluate the baseline stack with a typed per-dataset
    # published-classifier protocol.  Family proxies and unavailable rules are
    # deliberately distinguished from a faithful reproduction in every record.
    matched_config_protocols: Dict[str, Dict[str, Any]] = {}
    if bool(getattr(args, "use_sota_matched_classifiers", False)):
        unresolved_protocol = _get_sota_matched_protocol_for_dataset(ds_id)
        resolved_protocol: Optional[
            Tuple[SotaMatchedClassifierProtocol, Tuple[str, ...]]
        ] = None
        try:
            resolved_protocol = _resolve_sota_matched_protocol_for_dataset(ds_id)
        except SotaMatchedProtocolError as exc:
            invalid_protocol_record = (
                _sota_protocol_record(
                    unresolved_protocol,
                    unresolved_protocol.candidates,
                    rule_dataset_id=_sota_protocol_dataset_id(ds_id) or str(ds_id),
                )
            ...
            failures.append(
                {
                    "dataset_id": ds_id,
                    "seed": int(seed),
                    "config": "sota_matched_invalid_protocol",
                    "status": "skipped_sota_matched_invalid_protocol",
                    "error": str(exc),
                    "sota_matched_protocol": invalid_protocol_record,
                }
            )
        if resolved_protocol is not None:
            sota_protocol, sota_candidates = resolved_protocol
            rule_dataset_id = _sota_protocol_dataset_id(ds_id) or str(ds_id)
            protocol_record = _sota_protocol_record(
                sota_protocol,
                sota_candidates,
                rule_dataset_id=rule_dataset_id,
            )
            if sota_protocol.match_status == SOTA_MATCH_STATUS_UNAVAILABLE:
                failures.append(
                    {
                        "dataset_id": ds_id,
                        "seed": int(seed),
                        "config": "sota_matched_unavailable",
                        "status": "skipped_sota_matched_unavailable",
                        "error": "Published classifier protocol is not implemented.",
                        "sota_matched_protocol": protocol_record,
                    }
                )
            else:
                config_name = (
                    "sota_matched_exact"
                    if sota_protocol.match_status == SOTA_MATCH_STATUS_EXACT
                    else "sota_matched_family_proxy"
                )
                sota_cfg = clone_config(base_cfg)
                _apply_matched_model_candidate_set(sota_cfg, sota_candidates)
                configs.append((config_name, sota_cfg))
                matched_config_protocols[config_name] = protocol_record
    ...
    for config_name, cfg in configs:
        matched_protocol = matched_config_protocols.get(str(config_name))
        ...
        try:
            result = _run_pipeline_with_hard_timeout(
                ...
                dataset_name=ds_id,
                seed=seed,
                resampling_context=diakrino_resampling_context,
                supplied_split_id=diakrino_supplied_split_id,
                external_feature_scores=(
                    np.asarray(diakrino_binding.sealed_feature_scores, dtype=float)
                    if diakrino_binding is not None
                    and str(
                        getattr(args, "diakrino_feature_selection_arm", "off") or "off"
                    ).strip().lower()
                    in _DIAKRINO_CANONICAL_PROTECTED_ARMS
                    else None
                ),
                diakrino_closeout_evidence=(
                    None
                    if diakrino_binding is None
                    else diakrino_binding.closeout_evidence
                ),
            ...
            _record_pipeline_execution_closure(exc)
            err_txt = str(exc)
            failure = {
                "dataset_id": ds_id,
                "seed": seed,
            ...
            if matched_protocol is not None:
                failure["sota_matched_protocol"] = dict(matched_protocol)
        ...
        _record_pipeline_execution_closure(result)

        diakrino_result_diagnostic_fields: Dict[str, Any] = {}
        if diakrino_binding is not None:
            observed_train = tuple(
                int(value) for value in tuple(result.split_indices_train or tuple())
            )
            observed_test = tuple(
                int(value) for value in tuple(result.split_indices_test or tuple())
            )
            arm = str(
                getattr(args, "diakrino_feature_selection_arm", "off") or "off"
            ).strip().lower()
            binding_error = ""
            try:
                _revalidate_diakrino_post_run_artifact_pins(diakrino_binding)
            except DiakrinoSidecarIdentityError as exc:
                binding_error = f"DIAKRINO artifacts changed during execution: {exc}"
            if not binding_error and (
                observed_train != diakrino_binding.train_indices
                or observed_test != diakrino_binding.test_indices
            ):
                binding_error = "canonical pipeline did not honor the exact DIAKRINO supplied split"
            elif not binding_error and str(result.model_name) != "lr":
                binding_error = (
                    "canonical DIAKRINO run resolved a downstream classifier other than lr: "
                    f"{result.model_name!r}"
                )
            elif not binding_error and arm in _DIAKRINO_CANONICAL_PROTECTED_ARMS and (
                result.config_snapshot.get("diakrino_prefilter_enabled") is not True
                or str(result.config_snapshot.get("diakrino_prefilter_mode") or "")
                != "protected_union"
                or str(result.config_snapshot.get("diakrino_prefilter_score_column") or "")
                != diakrino_binding.score_column
                or str(result.config_snapshot.get("diakrino_prior_calibrate") or "")
                != (
                    "none"
                    if arm in ({"protected_uniform_views"} | _DIAKRINO_CLOSEOUT_ARMS)
                    else DIAKRINO_SIDECAR_REQUIRED_CALIBRATION
                ...
                binding_error = "canonical pipeline did not preserve the protected DIAKRINO config"
            if not binding_error:
                try:
                    diakrino_result_diagnostic_fields = _diakrino_protected_result_row_fields(
                ...
                failures.append(
                    {
                        ...
                        "seed": int(seed),
                        "config": str(config_name),
                        "status": "failed_diakrino_runtime_binding",
                        "error": binding_error,
                    }
                )
                continue

        matched_execution_metadata: Dict[str, str] = {}
        if matched_protocol is not None:
            matched_execution_metadata, execution_error = _validate_sota_matched_execution(
                matched_protocol,
                ...
                failures.append(
                    {
                        ...
                        "seed": int(seed),
                        "config": str(config_name),
                        "status": "skipped_sota_matched_invalid_execution",
                        "error": execution_error,
                        "sota_matched_protocol": dict(matched_protocol),
                        **matched_execution_metadata,
                    }
                )
                continue
        ...
        row = {
            ...
            "config": config_name,
            "protocol": "holdout",
            **_sota_row_metadata(matched_protocol),
            **matched_execution_metadata,
            ...
            "resampling_context_fingerprint": result.resampling_context_fingerprint,
            "fit_context_fingerprint": result.fit_context_fingerprint,
            "outer_split_fingerprint": result.outer_split_fingerprint,
            ...
            "selector_predicted_profile": str(
                result.config_snapshot.get("selector_predicted_profile", "")
            ),
            "selector_raw_profile": str(
                result.config_snapshot.get("selector_raw_profile", "")
            ),
            "selector_confidence": (
                float(result.config_snapshot.get("selector_confidence"))
                if isinstance(result.config_snapshot.get("selector_confidence", None), (int, float, np.floating))
                else float("nan")
            ),
            "selector_used": int(bool(result.config_snapshot.get("selector_used", False))),
            "selector_abstain": int(bool(result.config_snapshot.get("selector_abstain", False))),
        }
        if diakrino_result_diagnostic_fields:
            row.update(diakrino_result_diagnostic_fields)
        ...
        if isinstance(result.model_bundle, dict) and result.model_bundle:
            ...
            bundle.update(_sota_row_metadata(matched_protocol))
            bundle.update(matched_execution_metadata)
        ...
        if isinstance(result.run_diagnostics, dict) and result.run_diagnostics:
            ...
            diag.update(_sota_row_metadata(matched_protocol))
            diag.update(matched_execution_metadata)
    ...
    split_fingerprints = [
        str(row.get("outer_split_fingerprint", "") or "")
        for row in rows
        if str(row.get("outer_split_fingerprint", "") or "")
    ]
    materialized_input_identity = bind_materialized_input_split_fingerprints(
        materialized_input_identity,
        split_fingerprints,
    )
...
def _build_run_summary(
    ...
    raw_execution_provenance = metadata.get("execution_provenance", {})
    execution_provenance = (
        dict(raw_execution_provenance)
        if isinstance(raw_execution_provenance, Mapping)
        else {}
    )
    summary_execution_fields = (
        execution_row_fields(execution_provenance) if execution_provenance else {}
    )
    ...
    for row in rows:
        ...
        entry["sota_matched_protocol"] = _sota_summary_protocol(row)
        ...
        entry.update(summary_execution_fields)
    ...
    matched_protocol_failures: List[Dict[str, Any]] = []
    for failure in failures:
        protocol = failure.get("sota_matched_protocol")
        if not isinstance(protocol, dict) or not protocol:
            continue
        matched_protocol_failures.append(
            {
                "dataset_id": str(failure.get("dataset_id", "")),
                "seed": failure.get("seed"),
                "config": str(failure.get("config", "")),
                "status": str(failure.get("status", "")),
                "error": str(failure.get("error", "")),
                "sota_matched_protocol": dict(protocol),
                "executed_classifier": str(
                    failure.get("sota_matched_executed_classifier", "")
                ),
                "executed_canonical_classifier": str(
                    failure.get("sota_matched_executed_canonical_classifier", "")
                ),
            }
        )
    ...
    summary: Dict[str, Any] = {
        ...
        "sota_matched_protocol_failures": matched_protocol_failures,
...
def run_benchmark(args: argparse.Namespace) -> Any:
    ...
    _validate_diakrino_canonical_campaign_args(args)
    _assert_canonical_execution_bootstrap_seal()
    # This is a fail-fast check only. The closure that attests a run is captured
    # again after all tasks have materialized their inputs.
    validate_loaded_tabnetics_module_closure()
    ...
    execution_provenance_path = run_dir / "df_fs_execution_provenance.json"

    all_rows: List[Dict[str, Any]] = []
    failures: List[Dict[str, Any]] = []
    all_model_bundles: List[Dict[str, Any]] = []
    all_run_diagnostics: List[Dict[str, Any]] = []
    materialized_input_records: List[Mapping[str, Any]] = []
    worker_module_closures: List[Mapping[str, Any]] = []
    worker_module_closure_issues: List[str] = []
    ...
    if args.max_workers <= 1:
        for idx, (ds_id, seed) in enumerate(tasks, start=1):
            ...
            materialized_identity = task_result.get("materialized_input_identity")
            if isinstance(materialized_identity, Mapping):
                materialized_input_records.append(materialized_identity)
            _collect_task_execution_module_closures(
                task_result,
                dataset_id=ds_id,
                seed=seed,
                closures=worker_module_closures,
                issues=worker_module_closure_issues,
            )
        ...
        if len(task_results) != len(tasks):
            worker_module_closure_issues.append(
                "parallel_task_result_count_mismatch:"
                f"expected={len(tasks)} observed={len(task_results)}"
            )
        for (ds_id, seed), task_result in zip(tasks, task_results):
            all_rows.extend(task_result["rows"])
            failures.extend(task_result["failures"])
            all_model_bundles.extend(task_result.get("model_bundles", []))
            all_run_diagnostics.extend(task_result.get("run_diagnostics", []))
    ...
    external_callable_unattested = False
    worker_module_closures_complete = bool(
        int(getattr(args, "max_workers", 1) or 1) <= 1
        or not worker_module_closure_issues
    )
    try:
        execution_provenance = _build_canonical_execution_contract(
            args,
            selected,
            materialized_input_records,
            worker_module_closures,
        ...
        canonical_execution = False
        external_callable_unattested = isinstance(
            exc,
            CanonicalExecutionExternalDependencyError,
        )
        execution_provenance = _build_noncanonical_execution_contract(
            args,
            selected,
            materialized_input_records,
            worker_module_closures,
            ...
            evidence_status=(
                EXTERNAL_CALLABLE_UNATTESTED_EVIDENCE_STATUS
                if external_callable_unattested
                else "noncanonical_input_identity"
            ),
            preserve_materialized_input_identity=external_callable_unattested,
        )
    execution_fields = execution_row_fields(execution_provenance)
    write_json(execution_provenance_path, execution_provenance)
    if canonical_execution or external_callable_unattested:
        _attach_materialized_input_identity_fields(all_rows, materialized_input_records)
        _attach_materialized_input_identity_fields(failures, materialized_input_records)
        _attach_materialized_input_identity_fields(all_model_bundles, materialized_input_records)
        _attach_materialized_input_identity_fields(
            all_run_diagnostics,
            materialized_input_records,
        )
    _attach_execution_row_fields(all_rows, execution_fields)
    _attach_execution_row_fields(failures, execution_fields)
    _attach_execution_row_fields(all_model_bundles, execution_fields)
    _attach_execution_row_fields(all_run_diagnostics, execution_fields)
    ...
    if not all_rows:
        ...
        with failures_path.open("w", encoding="utf-8") as handle:
            json.dump(failures, handle, indent=2, default=str)
        with model_bundles_path.open("w", encoding="utf-8") as handle:
            json.dump(all_model_bundles, handle, indent=2, default=str)
        with run_diagnostics_path.open("w", encoding="utf-8") as handle:
            json.dump(all_run_diagnostics, handle, indent=2, default=str)
        raise RuntimeError(
            "No successful runs were produced; wrote failure diagnostics to "
            f"{failures_path}."
        )
    ...
    _attach_execution_frame_fields(runs_df, execution_fields)
    _attach_execution_frame_fields(summary_df, execution_fields)
    _attach_execution_frame_fields(sota_df, execution_fields)
    _attach_execution_frame_fields(ablation_df, execution_fields)
    ...
    artifact_provenance_path = run_dir / "df_fs_artifact_provenance.json"
    model_bundles_path = run_dir / "df_fs_model_bundles.json"
    run_diagnostics_path = run_dir / "df_fs_run_diagnostics.json"
    ...
    p4_authority_path: Path | None = None
    diakrino_arm = str(getattr(args, "diakrino_feature_selection_arm", "off") or "off").strip()
    if diakrino_arm == "protected_native_null_jmi":
        with runs_path.open(newline="", encoding="utf-8") as handle:
            p4_rows = [
                dict(row)
                for row in csv.DictReader(handle)
                if str(row.get("diakrino_feature_selection_arm") or "")
                == "protected_native_null_jmi"
            ]
        if len(p4_rows) != 1:
            raise DiakrinoSidecarIdentityError(
                "P4 authority emission requires exactly one finalized P4 result row"
            )
        p4_authority_path = write_p4_budget_authority(
            run_dir / "diakrino_p4_budget_authority.json", p4_rows[0]
        )
    ...
    if bool(getattr(args, "enable_shadow_evaluator", False)):
        ...
        if not shadow_df.empty:
            _attach_execution_frame_fields(shadow_df, execution_fields)
    ...
    metadata = {
        ...
        "sota_matched_protocol_catalog": {
            dataset_id: protocol.to_record()
        ...
        "config_flags": {
            "allow_synthetic_fallback": args.allow_synthetic_fallback,
            "use_sota_matched_classifiers": bool(
                getattr(args, "use_sota_matched_classifiers", False)
            ),
            "dataset_integrity_policy": str(getattr(args, "dataset_integrity_policy", "error")),
            "dataset_min_classes": int(getattr(args, "dataset_min_classes", 2) or 2),
            "dataset_min_class_count": int(getattr(args, "dataset_min_class_count", 1) or 1),
            ...
            "fs_method_max_rss_mb": float(
                getattr(args, "fs_method_max_rss_mb", 0.0) or 0.0
            ),
            "fs_linear_svm_max_iter": int(getattr(args, "fs_linear_svm_max_iter", 10000) or 10000),
            ...
            "disable_classifier_oracle_worst_class": bool(
                getattr(args, "disable_classifier_oracle_worst_class", False)
            ),
            ...
            "enable_calibration_reporting": bool(
                getattr(args, "enable_calibration_reporting", False)
            ),
            "enable_classifier_posthoc_calibration": bool(
                getattr(args, "enable_classifier_posthoc_calibration", False)
            ),
            "classifier_posthoc_calibration_method": str(
                getattr(args, "classifier_posthoc_calibration_method", "sigmoid") or "sigmoid"
            ...
            "disable_classifier_posthoc_calibration_refinement_stopping": bool(
                getattr(args, "disable_classifier_posthoc_calibration_refinement_stopping", False)
            ),
        ...
        "outputs": {
            ...
            "execution_provenance": str(execution_provenance_path),
            "artifact_provenance": (
                str(artifact_provenance_path) if canonical_execution else ""
            ),
            "diakrino_p4_budget_authority": (
                str(p4_authority_path) if p4_authority_path is not None else ""
            ),
    ...
    if canonical_execution:
        artifact_provenance = build_canonical_benchmark_artifact_provenance(
            execution_provenance=execution_provenance,
            artifact_paths={
                "df_fs_runs.csv": runs_path,
                "df_fs_summary.csv": summary_path,
                "df_fs_sota_comparison.csv": sota_path,
                "df_fs_ablation_deltas.csv": ablation_path,
                "df_fs_metadata.json": metadata_path,
                "df_fs_execution_provenance.json": execution_provenance_path,
            },
        )
        if p4_authority_path is not None:
            artifact_provenance["diakrino_p4_budget_authority_artifact"] = {
                "path": p4_authority_path.name,
                "sha256": sha256_file(p4_authority_path),
                "size_bytes": int(p4_authority_path.stat().st_size),
            }
        write_json(artifact_provenance_path, artifact_provenance)
    ...
    print(f"Saved execution provenance to: {execution_provenance_path}")
    print(f"Saved metadata to: {metadata_path}")
    if canonical_execution:
        print(f"Saved artifact provenance to: {artifact_provenance_path}")
    else:
        print("Execution marked noncanonical: materialized input identity incomplete")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--diakrino-feature-selection-arm",
        choices=[
            "off",
            "protected_union",
            "protected_direct_fusion",
        ...
        default="off",
        help=(
            "Canonical DIAKRINO feature-selection arm. 'off' is the identity-matched "
            "classical baseline; protected_direct_fusion is P1 and "
            "protected_uniform_views is P2. protected_union remains the P1-compatible "
            "legacy spelling."
        ),
    )
    parser.add_argument(
        "--diakrino-sidecar-contract",
        type=str,
        default="",
        help=(
            "Fresh fail-closed DIAKRINO producer binding contract. Direct parquet/directory "
            "sidecars are intentionally unsupported for canonical evidence."
        ),
    )
    parser.add_argument(
        "--diakrino-expected-checkpoint-sha256",
        type=str,
        default="",
        help="Expected vnext checkpoint content SHA-256 (required for canonical DIAKRINO arms).",
    )
    parser.add_argument(
        "--diakrino-split-policy",
        choices=["pipeline_seed", "hf_probe"],
        default="pipeline_seed",
        help="Outer split policy; canonical DIAKRINO sidecars require exact emitter-aligned hf_probe.",
    )
    parser.add_argument(
        "--diakrino-probe-train-fraction",
        type=float,
        default=0.8,
        help="Support fraction used by the fresh DIAKRINO emitter and canonical supplied split.",
    )
    parser.add_argument(
        "--diakrino-score-column",
        type=str,
        default="prior_screening_fusion_w075_logit",
        help=(
            "Persisted DIAKRINO score consumed by a protected arm. P1 requires "
            "prior_screening_fusion_w075_logit with chunk_zscore; P2 requires "
            "diakrino_uniform_view_rank01 and consumes the sealed aggregate with no "
            "additional calibration."
        ),
    )
    parser.add_argument(
        "--diakrino-calibration",
        choices=["chunk_zscore"],
        default="chunk_zscore",
        help="Required DIAKRINO cross-chunk calibration for canonical feature-selection arms.",
    )
    parser.add_argument(
        "--diakrino-prefilter-max-extras",
        type=int,
        default=50,
        help="Maximum DIAKRINO-only additions after the immutable classical core.",
    )
    parser.add_argument("--diakrino-shadow-margin-min", type=float, default=0.0)
    parser.add_argument("--diakrino-label-null-margin-min", type=float, default=0.0)
    parser.add_argument("--diakrino-rank-std-max", type=float, default=0.25)
    parser.add_argument("--diakrino-stability-min", type=float, default=0.0)
    parser.add_argument("--diakrino-proposal-pool-multiplier", type=int, default=4)
    parser.add_argument("--diakrino-jmi-discretization-bins", type=int, default=5)
    parser.add_argument(
        "--diakrino-p4-realized-additions",
        type=int,
        default=-1,
        help="Exact per-dataset/seed P4 additions required by C1-C3.",
    )
    parser.add_argument(
        "--diakrino-p4-budget-authority",
        type=str,
        default="",
        help="Bundle-attested P4 authority artifact required by C1-C3.",
    )
    ...
    parser.add_argument(
        "--df-flex-distinct-gate",
        action="store_true",
        help="Skip scipy families with shape params unless the feature has enough distinct values (opt-in).",
    )
    parser.add_argument(
        "--df-flex-min-distinct",
        type=int,
        default=15,
        help="Minimum distinct values required by --df-flex-distinct-gate for >=3-param scipy families.",
    )
    parser.add_argument(
        "--df-flex-retention-margin",
        type=float,
        default=0.0,
        help="Simple-score margin a >=3-param family must beat the best simple family by (0 disables).",
    )
    parser.add_argument(
        "--df-estimator",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "decision_tree", "logistic", "flag_selector_v1"],
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Per feature-selector method RSS limit in MiB (0 disables).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Opt-in cap for ordered mRMR redundancy pairs; 0 preserves unlimited legacy behavior.",
    )
    parser.add_argument(
        "--fs-mrmr-max-runtime-seconds",
        type=float,
        default=0.0,
        help="Opt-in mRMR wall-clock cap in seconds; 0 preserves unlimited legacy behavior.",
    )
    parser.add_argument(
        "--fs-mrmr-budget-fallback-mode",
        choices=("empty", "relevance_only"),
        default="empty",
        help="On an opt-in mRMR cap breach, return no candidate or use explicit relevance-only fallback.",
    )
    parser.add_argument(
        "--fs-cmim-min-samples",
        type=int,
    ...
    parser.add_argument(
        "--disable-classifier-oracle-worst-class",
        action="store_true",
        help="Disable worst-class-recall oracle in mnpo_hybrid classifier selection.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Emit final-model ECE/Brier calibration metrics in benchmark output.",
    )
    parser.add_argument(
        "--enable-classifier-posthoc-calibration",
        action="store_true",
        help="Enable opt-in post-hoc probability calibration for the final classifier.",
    )
    parser.add_argument(
        "--classifier-posthoc-calibration-method",
        type=str,
        default="sigmoid",
        choices=["sigmoid", "isotonic"],
        help="Post-hoc calibration method for the final classifier.",
    )
    parser.add_argument(
        "--classifier-posthoc-calibration-fraction",
        type=float,
        default=0.20,
        help="Training fraction reserved for post-hoc calibration when enabled.",
    )
    parser.add_argument(
        "--classifier-posthoc-calibration-min-calibration",
        type=int,
        default=20,
        help="Minimum calibration holdout size for post-hoc classifier calibration.",
    )
    parser.add_argument(
        "--disable-classifier-posthoc-calibration-refinement-stopping",
        action="store_true",
        help="Allow post-hoc calibration even when holdout calibration loss worsens.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "For each dataset, run an additional published-protocol classifier "
            "profile. Result provenance labels it exact, family_proxy, or "
            "unavailable; family proxies are not paper reproductions. The normal "
            "baseline config is always run as well."
        ),
    )
...
if __name__ == "__main__":
    # Re-enter through the package module so canonical provenance never records
    # the CLI's transient ``__main__`` module as the benchmark implementation.
    import importlib

    importlib.import_module("tabnetics.benchmarks.runner").main()
