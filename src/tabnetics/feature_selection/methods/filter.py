...
def _mrmr_pair_redundancy_score(
    X_pool: np.ndarray,
    X_disc: np.ndarray,
    idx: int,
    selected_idx: int,
    *,
    ...
    """Return one ordered candidate-to-selected mRMR redundancy score.

    The key distinction is intentional: ``(candidate, selected)`` is an
    ordered pair.  In particular, ``mutual_info_regression`` is not assumed
    to be numerically symmetric, so callers must not canonicalize this pair.
    """
    if mode == "pearson":
        return float(abs(np.corrcoef(X_pool[:, idx], X_pool[:, selected_idx])[0, 1]))

    if mode == "binned_mi":
        return float(_mutual_information_discrete(X_disc[:, idx], X_disc[:, selected_idx]))

    if mode != "knn_mi":
        raise ValueError(f"Unsupported mRMR redundancy mode: {mode!r}")

    from sklearn.feature_selection import mutual_info_regression

    xi = np.asarray(X_pool[:, idx], dtype=float).reshape(-1, 1)
    y_selected = np.asarray(X_pool[:, selected_idx], dtype=float).ravel()
    score = mutual_info_regression(
        xi,
        y_selected,
        random_state=int(random_state),
    ...
    return float(np.asarray(score, dtype=float).ravel()[0])


_PAIR_CACHE_MISSING = object()
__tabnetics_execution_ephemeral_globals__ = ("_PAIR_CACHE_MISSING",)


class _DirectionalRedundancyCache:
    """Per-run ordered redundancy cache with bounded evaluation accounting."""

    def __init__(self, max_unique_pair_evaluations: int) -> None:
        self.max_unique_pair_evaluations = int(max_unique_pair_evaluations)
        self._scores: dict[tuple[str, int, int], float | None] = {}
        self.unique_pair_evaluations = 0
        self.cache_hits = 0
        self.pair_evaluation_failures = 0
        self.pair_nonfinite_values = 0

    @property
    def size(self) -> int:
        return int(len(self._scores))
    ...
    def get_or_compute(
        ...
        *,
        ...
        """Get an ordered score or return a deterministic budget-stop status."""
        key = (str(mode), int(idx), int(selected_idx))
        cached = self._scores.get(key, _PAIR_CACHE_MISSING)
        if cached is not _PAIR_CACHE_MISSING:
            self.cache_hits += 1
            return cached, "cache_hit"

        if (
            self.max_unique_pair_evaluations > 0
            and self.unique_pair_evaluations >= self.max_unique_pair_evaluations
        ):
            return None, "unique_pair_evaluation_budget_exhausted"

        self.unique_pair_evaluations += 1
        value: float | None
        try:
            raw_value = _mrmr_pair_redundancy_score(
                X_pool,
                X_disc,
                int(idx),
                int(selected_idx),
            ...
            self.pair_evaluation_failures += 1
            value = None

        if value is not None and not np.isfinite(value):
            self.pair_nonfinite_values += 1
            value = None
        self._scores[key] = value
        return value, "computed"


def _mrmr_work_estimates(pool_size: int, target_size: int) -> tuple[int, int]:
    """Return uncached and ordered-cache pair-evaluation counts for greedy mRMR."""
    m = int(max(0, pool_size))
    k = int(min(max(1, target_size), m)) if m else 0
    if m <= 1 or k <= 1:
        return 0, 0
    uncached = int(k * (k - 1) * (3 * m - 2 * k + 1) // 6)
    ordered_unique = int((k - 1) * (2 * m - k) // 2)
    return uncached, ordered_unique
...
def mrmr_jmi_selection(
    ...
    Redundancy values are cached by ordered ``(candidate, selected)`` pair.
    The order is retained even for modes that are mathematically symmetric so
    the k-NN estimator is never assumed symmetric in finite precision.
    """
    try:
        max_unique_pair_evaluations = int(mrmr_max_unique_pair_evaluations)
    except (TypeError, ValueError, OverflowError) as exc:
        raise ValueError("mrmr_max_unique_pair_evaluations must be an integer >= 0") from exc
    if max_unique_pair_evaluations < 0:
        raise ValueError("mrmr_max_unique_pair_evaluations must be >= 0 (0 means unlimited)")

    try:
        max_runtime_seconds = float(mrmr_max_runtime_seconds)
    except (TypeError, ValueError) as exc:
        raise ValueError("mrmr_max_runtime_seconds must be a finite float >= 0") from exc
    if not np.isfinite(max_runtime_seconds) or max_runtime_seconds < 0.0:
        raise ValueError("mrmr_max_runtime_seconds must be a finite float >= 0 (0 means unlimited)")

    budget_fallback_mode = str(mrmr_budget_fallback_mode or "empty").strip().lower()
    if budget_fallback_mode not in {"empty", "relevance_only"}:
        raise ValueError(
    ...
    estimated_uncached_pair_evaluations, estimated_unique_pair_evaluations = _mrmr_work_estimates(
        int(X_pool.shape[1]),
        k_target,
    )
    selected_local = []
    ...
    redundancy_mode_requested = "tiered_mi" if bool(mrmr_mi_redundancy_enabled) else "pearson"
    X_disc = None
    ...
    cache = _DirectionalRedundancyCache(max_unique_pair_evaluations)
    X_disc_arr = np.asarray(X_disc) if X_disc is not None else np.zeros((0, 0), dtype=int)
    selection_complete = True
    stop_reason = "target_reached"

    def _runtime_budget_exhausted() -> bool:
        return bool(
            max_runtime_seconds > 0.0
            and (perf_counter() - started_at) >= max_runtime_seconds
        )

    if _runtime_budget_exhausted():
        selection_complete = False
        stop_reason = "runtime_budget_exhausted"
    elif X_pool.shape[1] <= 1:
        selected_local = [0]
        ...
        while selection_complete and len(selected_local) < k_target:
            if _runtime_budget_exhausted():
                selection_complete = False
                stop_reason = "runtime_budget_exhausted"
                break
            ...
            for idx in range(X_pool.shape[1]):
                ...
                for selected_idx in selected_local:
                    if _runtime_budget_exhausted():
                        ...
                        stop_reason = "runtime_budget_exhausted"
                        break
                    pair_score, pair_status = cache.get_or_compute(
                        X_pool,
                        X_disc_arr,
                        int(idx),
                        int(selected_idx),
                    ...
                    if pair_status == "unique_pair_evaluation_budget_exhausted":
                        selection_complete = False
                        stop_reason = pair_status
                        break
                    if pair_score is not None:
                        redundancy_values.append(float(pair_score))

                if not selection_complete:
                    break
                redundancy = (
                    float(np.mean(redundancy_values)) if redundancy_values else 0.0
                )
    ...
    partial_selected_count = int(len(selected_local))
    ...
    elif budget_fallback_mode == "relevance_only":
        fallback_applied = True
        n_requested = max(0, int(n_target_features))
        fallback_local = np.argsort(relevance_norm)[::-1][:n_requested]
        selected_indices = pool_idx[np.asarray(fallback_local, dtype=int)]
        all_scores = np.zeros(n_features, dtype=float)
        all_scores[pool_idx] = relevance_norm
        all_scores = np.nan_to_num(all_scores, nan=0.0, posinf=0.0, neginf=0.0)
        min_score = float(np.min(all_scores)) if all_scores.size else 0.0
        if min_score < 0:
            all_scores = all_scores - min_score
    ...
    elapsed_seconds = float(max(0.0, perf_counter() - started_at))
    ...
    elif max_unique_pair_evaluations == 0 and max_runtime_seconds == 0.0:
    ...
    results = {
        ...
        'criterion_family': 'mrmr_difference',
        'redundancy_mode': str(redundancy_mode),
        'redundancy_mode_requested': str(redundancy_mode_requested),
        'redundancy_mode_effective': str(redundancy_mode),
        'pool_size': int(pool_idx.size),
        'target_size': int(k_target),
        'cache_mode': 'directional_ordered_pair',
        'cache_enabled': True,
        'cache_size': int(cache.size),
        'estimated_uncached_pair_evaluations': int(estimated_uncached_pair_evaluations),
        'estimated_unique_pair_evaluations': int(estimated_unique_pair_evaluations),
        'unique_pair_evaluations': int(cache.unique_pair_evaluations),
        'cache_hits': int(cache.cache_hits),
        'pair_evaluation_failures': int(cache.pair_evaluation_failures),
        'pair_nonfinite_values': int(cache.pair_nonfinite_values),
        'max_unique_pair_evaluations_requested': int(max_unique_pair_evaluations),
        'max_runtime_seconds_requested': float(max_runtime_seconds),
        'budget_fallback_mode_requested': str(budget_fallback_mode),
        'budget_status': str(budget_status),
        'budget_exhausted': bool(budget_exhausted),
        'stop_reason': str(stop_reason),
        'complete': bool(selection_complete),
        'incomplete': bool(not selection_complete),
        'fallback_applied': bool(fallback_applied),
        'partial_selected_count': int(partial_selected_count),
