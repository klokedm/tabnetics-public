...
except ImportError:  # pragma: no cover - source-tree compatibility
...
from .registry import METHOD_REGISTRY, MethodSpec, get_method_weights, get_experimental_keys, method_excluded_by_default
...
class FeatureSelector:
    ...
    def __init__(self, 
                 ...
                 diakrino_selector_prior_calibration="current_checkpoint_20260628",
                 diakrino_selector_prior_qualification_record="",
                 ...
                 copula_time_budget_seconds=1800.0,
                 ...
                 pathway_group_sparse_lasso_n_groups=50,
                 pathway_group_sparse_lasso_max_group_size=50,
                 ...
                 diakrino_conformal_selection_enabled=False,
                 diakrino_conformal_target_fdp=0.20,
                 diakrino_conformal_calibrate="chunk_zscore",
                 diakrino_conformal_null_fraction=0.50,
                 diakrino_conformal_min_null_scores=4,
                 diakrino_conformal_max_features=0,
        ...
        Args:
            ...
            mrmr_max_unique_pair_evaluations (int): Ordered redundancy-pair cap; 0 keeps legacy unlimited behavior.
            mrmr_max_runtime_seconds (float): mRMR wall-clock cap; 0 keeps legacy unlimited behavior.
            mrmr_budget_fallback_mode (str): 'empty' (default) or opt-in 'relevance_only' on a cap breach.
            ...
            method_timeout_seconds (float): Optional per-method hard wall-clock
                timeout (seconds). <=0 disables.
            method_max_rss_mb (float): Optional per-method resident-memory cap
                in MiB. <=0 disables.
            parallel_n_jobs (int): Number of process-isolated FS method workers.
        ...
        try:
            _method_max_rss_mb = float(method_max_rss_mb)
        except (TypeError, ValueError) as exc:
            raise ValueError("method_max_rss_mb must be a finite float >= 0") from exc
        if not math.isfinite(_method_max_rss_mb) or _method_max_rss_mb < 0.0:
            raise ValueError("method_max_rss_mb must be a finite float >= 0")
        self.method_max_rss_mb = float(_method_max_rss_mb)
        ...
        self.use_diakrino_relevance_oracle = bool(use_diakrino_relevance_oracle)
        self.diakrino_relevance_min_n_train = int(max(2, diakrino_relevance_min_n_train))
        self.diakrino_relevance_score_column = str(diakrino_relevance_score_column or "prior_logit")
        self.use_diakrino_selector_prior = bool(use_diakrino_selector_prior)
        self.diakrino_selector_prior_weight = float(np.clip(diakrino_selector_prior_weight, 0.0, 1.0))
        mode = str(diakrino_selector_prior_calibration or "current_checkpoint_20260628").strip().lower()
        if mode in {"raw", "off"}:
            mode = "none"
        ...
        self.diakrino_selector_prior_qualification_record = str(diakrino_selector_prior_qualification_record or "")
        self._diakrino_selector_prior_gate_meta = {
            "path": self.diakrino_selector_prior_qualification_record,
            "record_loaded": False,
            "allowed": False,
            "reason": "not_checked",
            "failed_gates": [],
        }
        self._diakrino_selector_prior_trust_record = None
        ...
        self.complexity_conditioning = bool(complexity_conditioning)
        ...
        self.oracle = OracleConfig(
            ...
            use_diakrino_relevance_oracle=bool(getattr(self, "use_diakrino_relevance_oracle", False)),
            diakrino_relevance_min_n_train=int(getattr(self, "diakrino_relevance_min_n_train", 100)),
            diakrino_relevance_score_column=str(getattr(self, "diakrino_relevance_score_column", "prior_logit")),
            use_diakrino_selector_prior=bool(getattr(self, "use_diakrino_selector_prior", False)),
            diakrino_selector_prior_weight=float(getattr(self, "diakrino_selector_prior_weight", 1.0)),
            diakrino_selector_prior_calibration=str(
                getattr(self, "diakrino_selector_prior_calibration", "current_checkpoint_20260628")
            ),
            diakrino_selector_prior_qualification_record=str(
                getattr(self, "diakrino_selector_prior_qualification_record", "")
            ),
            ...
            complexity_conditioning=bool(self.complexity_conditioning),
        )
        ...
        # DIAKRINO candidate-method config (native integration §2.1; opt-in via enabled_methods).
        self.diakrino_prior_sidecar_path = str(diakrino_prior_sidecar_path or "")
        self.diakrino_prior_dataset_id = str(diakrino_prior_dataset_id or "")
        self.diakrino_prior_score_column = str(diakrino_prior_score_column or "prior_logit")
        self.diakrino_screening_score_column = str(diakrino_screening_score_column or "screening_logit")
        self.diakrino_prior_calibrate = str(diakrino_prior_calibrate or "chunk_zscore")
        self.diakrino_prior_top_k = int(max(0, diakrino_prior_top_k))
        self.diakrino_conformal_selection_enabled = bool(diakrino_conformal_selection_enabled)
        self.diakrino_conformal_target_fdp = float(np.clip(diakrino_conformal_target_fdp, 0.0, 1.0))
        self.diakrino_conformal_calibrate = str(diakrino_conformal_calibrate or "chunk_zscore").strip().lower()
        self.diakrino_conformal_null_fraction = float(np.clip(diakrino_conformal_null_fraction, 0.0, 1.0))
        self.diakrino_conformal_min_null_scores = int(max(1, int(diakrino_conformal_min_null_scores)))
        self.diakrino_conformal_max_features = int(max(0, int(diakrino_conformal_max_features)))
        self.diakrino_conformal_qualification_record = str(diakrino_conformal_qualification_record or "")
        self._diakrino_conformal_selection_meta = {
            "applied": False,
            "reason": "not_checked",
            "normalize": self.diakrino_conformal_calibrate,
            "normalization_mode": self.diakrino_conformal_calibrate,
            "normalization_family": "chunk_zscore"
            if self.diakrino_conformal_calibrate == "chunk_zscore"
            else self.diakrino_conformal_calibrate,
            "calibration": "within_chunk_mean_std_then_split_conformal_bh"
            if self.diakrino_conformal_calibrate == "chunk_zscore"
            else "split_conformal_bh",
            "calibration_mode": "within_chunk_mean_std_then_split_conformal_bh"
            if self.diakrino_conformal_calibrate == "chunk_zscore"
            else "split_conformal_bh",
            "zscore_applied": bool(self.diakrino_conformal_calibrate == "chunk_zscore"),
            "nn_probe_normalize": self.diakrino_conformal_calibrate,
            "nn_probe_normalization_family": "chunk_zscore"
            if self.diakrino_conformal_calibrate == "chunk_zscore"
            else self.diakrino_conformal_calibrate,
            "nn_probe_calibration": "within_chunk_mean_std_then_split_conformal_bh"
            if self.diakrino_conformal_calibrate == "chunk_zscore"
            else "split_conformal_bh",
            "nn_probe_zscore_applied": bool(self.diakrino_conformal_calibrate == "chunk_zscore"),
        }
        self._current_feature_mapping = None  # staged each fit so DIAKRINO methods can align to originals
        ...
        try:
            self.mrmr_max_runtime_seconds = float(mrmr_max_runtime_seconds)
        except (TypeError, ValueError) as exc:
            raise ValueError(
        ...
        self.mrmr_budget_fallback_mode = str(
            mrmr_budget_fallback_mode or "empty"
        ).strip().lower()
        ...
        self.copula_time_budget_seconds = (
            None
            if copula_time_budget_seconds is None or float(copula_time_budget_seconds) <= 0.0
            else float(copula_time_budget_seconds)
        )
        self.copula_vine_num_threads = (
            None if copula_vine_num_threads is None else int(max(1, copula_vine_num_threads))
        )
        self.copula_lasso_max_iter = int(max(100, copula_lasso_max_iter))
        self.copula_lasso_n_jobs = int(copula_lasso_n_jobs) if copula_lasso_n_jobs is not None else 1
        ...
        self.pathway_group_sparse_lasso_n_groups = int(max(1, pathway_group_sparse_lasso_n_groups))
        self.pathway_group_sparse_lasso_max_group_size = int(max(2, pathway_group_sparse_lasso_max_group_size))
        ...
        self.oracle.complexity_conditioning = bool(self.complexity_conditioning)
    ...
    def from_config(cls, config) -> 'FeatureSelector':
        ...
        return cls(
            ...
            diakrino_prior_sidecar_path=getattr(config.methods, "diakrino_prior_sidecar_path", ""),
            diakrino_prior_dataset_id=getattr(config.methods, "diakrino_prior_dataset_id", ""),
            diakrino_prior_score_column=getattr(config.methods, "diakrino_prior_score_column", "prior_logit"),
            diakrino_screening_score_column=getattr(config.methods, "diakrino_screening_score_column", "screening_logit"),
            diakrino_prior_calibrate=getattr(config.methods, "diakrino_prior_calibrate", "chunk_zscore"),
            diakrino_prior_top_k=int(getattr(config.methods, "diakrino_prior_top_k", 0)),
            diakrino_conformal_selection_enabled=bool(
                getattr(config.methods, "diakrino_conformal_selection_enabled", False)
            ),
            diakrino_conformal_target_fdp=float(getattr(config.methods, "diakrino_conformal_target_fdp", 0.20)),
            diakrino_conformal_calibrate=str(getattr(config.methods, "diakrino_conformal_calibrate", "chunk_zscore")),
            diakrino_conformal_null_fraction=float(getattr(config.methods, "diakrino_conformal_null_fraction", 0.50)),
            diakrino_conformal_min_null_scores=int(getattr(config.methods, "diakrino_conformal_min_null_scores", 4)),
            diakrino_conformal_max_features=int(getattr(config.methods, "diakrino_conformal_max_features", 0)),
            diakrino_conformal_qualification_record=str(
                getattr(config.methods, "diakrino_conformal_qualification_record", "")
            ),
            method_timeout_seconds=config.method_timeout_seconds,
            method_max_rss_mb=float(getattr(config, "method_max_rss_mb", 0.0) or 0.0),
            ...
            use_diakrino_relevance_oracle=bool(
                _oracle_get("use_diakrino_relevance_oracle", getattr(config.mnpo, "use_diakrino_relevance_oracle", False))
            ),
            diakrino_relevance_min_n_train=int(
                _oracle_get("diakrino_relevance_min_n_train", getattr(config.mnpo, "diakrino_relevance_min_n_train", 100))
            ),
            diakrino_relevance_score_column=str(
                _oracle_get("diakrino_relevance_score_column", getattr(config.mnpo, "diakrino_relevance_score_column", "prior_logit"))
            ),
            use_diakrino_selector_prior=bool(
                _oracle_get("use_diakrino_selector_prior", getattr(config.mnpo, "use_diakrino_selector_prior", False))
            ),
            diakrino_selector_prior_weight=float(
                _oracle_get("diakrino_selector_prior_weight", getattr(config.mnpo, "diakrino_selector_prior_weight", 1.0))
            ),
            diakrino_selector_prior_calibration=str(
                _oracle_get(
                    "diakrino_selector_prior_calibration",
                    getattr(config.mnpo, "diakrino_selector_prior_calibration", "current_checkpoint_20260628"),
                )
            ),
            diakrino_selector_prior_qualification_record=str(
                _oracle_get(
                    "diakrino_selector_prior_qualification_record",
                    getattr(config.mnpo, "diakrino_selector_prior_qualification_record", ""),
                )
            ),
            ...
            complexity_conditioning=bool(
                _oracle_get(
                    ...
                    getattr(config.mnpo, "complexity_conditioning", False),
                )
            ),
            ...
            copula_time_budget_seconds=float(
                getattr(config.copula, "copula_time_budget_seconds", 1800.0) or 0.0
            ),
            copula_vine_num_threads=getattr(config.copula, "copula_vine_num_threads", None),
            copula_lasso_max_iter=int(
                getattr(config.copula, "copula_lasso_max_iter", 5000) or 5000
            ),
            copula_lasso_n_jobs=int(
                getattr(config.copula, "copula_lasso_n_jobs", 1) or 1
            ),
            # ── Method-specific ───────────────────────────────────────
            ...
            mrmr_max_unique_pair_evaluations=getattr(
                config.methods, "mrmr_max_unique_pair_evaluations", 0
            ),
            mrmr_max_runtime_seconds=getattr(config.methods, "mrmr_max_runtime_seconds", 0.0),
            mrmr_budget_fallback_mode=getattr(
                config.methods, "mrmr_budget_fallback_mode", "empty"
            ),
    ...
    def _copula_knockoff_selection(self, X, y, n_target_features):
        ...
        return copula_knockoff_selection(
            ...
            copula_time_budget_seconds=self.copula_time_budget_seconds,
            copula_vine_num_threads=self.copula_vine_num_threads,
            copula_lasso_max_iter=self.copula_lasso_max_iter,
            copula_lasso_n_jobs=self.copula_lasso_n_jobs,
        )
    ...
    def _get_cv_strategy(self, n_samples, y, purpose='method_internal'):
        ...
        return get_cv_strategy(
            n_samples,
            y,
            self.problem_type,
            self.random_state,
            self.inner_cv_splits,
            self.inner_cv_repeats,
            purpose,
            split_plan_provider=getattr(self, "_resampling_plan_provider", None),
        )
    ...
    def _prefilter_feature_pool(self, X, y, max_features):
        ...
        try:
            ...
            union_selected = build_prefilter_union_pool(
                ...
                bh_fdr_alpha=float(getattr(self, "prefilter_bh_ttest_alpha", 0.05) or 0.05),
    ...
    def _compute_aligned_diakrino_scores(self, score_column, calibrate="chunk_zscore"):
        """Per-X_uncorr-column DIAKRINO relevance vector from the sidecar, gathered onto the
        current column space via the staged feature_mapping.  Returns ``None`` when the
        sidecar is absent/unreadable or the mapping cannot align (never misaligns)."""
        path = str(getattr(self, "diakrino_prior_sidecar_path", "") or "")
        fm = getattr(self, "_current_feature_mapping", None)
        if not path or fm is None:
            return None
        try:
            from .diakrino_sidecar import DiakrinoSidecar
            sc = DiakrinoSidecar.load(path, dataset_id=str(getattr(self, "diakrino_prior_dataset_id", "") or "") or None)
            if sc is None:
                return None
            orig = sc.scalar_scores(str(score_column), calibrate=str(calibrate))
            if orig is None:
                return None
            orig = np.asarray(orig, dtype=float).ravel()
            fm = np.asarray(fm, dtype=np.int64).ravel()
            if fm.size == 0 or int(fm.max()) >= orig.shape[0]:
                return None
            return orig[fm]
        except Exception:
            return None

    def _compute_diakrino_selector_weights(self):
        """Dataset-level DIAKRINO selector-pool weights from the sidecar.

        Returns ``None`` when absent/unreadable so MNPO falls back to its existing
        reference prior exactly.
        """
        from .diakrino_qualification import qualification_gate_status

        self._diakrino_selector_prior_trust_record = None
        # Feature-selection consumer: gate on the FS-class gate subset so a
        # checkpoint that clears the selection gates but fails the dead
        # classifier/param/structural heads can still drive the selector-prior
        # (T-DIAKRINO-NAT-12 granular gating). The sidecar's own per-head
        # ``consumer_allowed`` trust record is still enforced below.
        gate = qualification_gate_status(
            str(getattr(self, "diakrino_selector_prior_qualification_record", "") or ""),
            consumer_class="feature_selection",
        )
        self._diakrino_selector_prior_gate_meta = dict(gate)
        if not bool(gate.get("allowed", False)):
            return None
        path = str(getattr(self, "diakrino_prior_sidecar_path", "") or "")
        if not path:
            self._diakrino_selector_prior_gate_meta = {
                **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
                "allowed": False,
                "reason": "missing_sidecar_path_after_qualification",
            }
            return None
        try:
            from .diakrino_sidecar import DiakrinoSidecar
            ...
            if sc is None:
                self._diakrino_selector_prior_gate_meta = {
                    **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
                    "allowed": False,
                    "reason": "sidecar_unreadable_after_qualification",
                }
                return None
            trust_record = sc.trust_record()
            self._diakrino_selector_prior_trust_record = trust_record
            if trust_record is None:
                self._diakrino_selector_prior_gate_meta = {
                    **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
                    "allowed": False,
                    "reason": "missing_sidecar_trust_record_after_qualification",
                    "sidecar_trust_record_present": False,
                }
                return None
            weights = sc.selector_weights()
            if not weights:
                reason = "selector_weights_missing_after_qualification"
                heads = trust_record.get("heads") if isinstance(trust_record, dict) else None
                selector_head = heads.get("selector_weights") if isinstance(heads, dict) else None
                if isinstance(selector_head, dict) and not bool(selector_head.get("consumer_allowed", False)):
                    reason = "selector_weights_untrusted_after_qualification"
                self._diakrino_selector_prior_gate_meta = {
                    **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
                    "allowed": False,
                    "reason": reason,
                    "sidecar_trust_record_present": True,
                }
                return None
            self._diakrino_selector_prior_gate_meta = {
                **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
            ...
            self._diakrino_selector_prior_gate_meta = {
                **dict(getattr(self, "_diakrino_selector_prior_gate_meta", {}) or {}),
                "allowed": False,
                "reason": "selector_prior_load_error",
            }
            return None

    def _diakrino_prior_selection(self, X, y, n_target_features):
        """DIAKRINO prior-relevance candidate selector (native integration §2.1; opt-in)."""
        from .methods.diakrino_prior import diakrino_prior_selection
        return diakrino_prior_selection(
            X, y, n_target_features,
            sidecar_path=getattr(self, "diakrino_prior_sidecar_path", ""),
            dataset_id=str(getattr(self, "diakrino_prior_dataset_id", "") or "") or None,
            score_column=getattr(self, "diakrino_prior_score_column", "prior_logit"),
            calibrate=getattr(self, "diakrino_prior_calibrate", "chunk_zscore"),
            feature_mapping=getattr(self, "_current_feature_mapping", None),
            top_k=int(getattr(self, "diakrino_prior_top_k", 0)),
        )

    def _diakrino_screening_selection(self, X, y, n_target_features):
        """DIAKRINO screening-relevance candidate selector (diversity twin of diakrino_prior)."""
        from .methods.diakrino_prior import diakrino_prior_selection
        return diakrino_prior_selection(
            X, y, n_target_features,
            sidecar_path=getattr(self, "diakrino_prior_sidecar_path", ""),
            dataset_id=str(getattr(self, "diakrino_prior_dataset_id", "") or "") or None,
            score_column=getattr(self, "diakrino_screening_score_column", "screening_logit"),
            calibrate=getattr(self, "diakrino_prior_calibrate", "chunk_zscore"),
            feature_mapping=getattr(self, "_current_feature_mapping", None),
            top_k=int(getattr(self, "diakrino_prior_top_k", 0)),
        )

    def _diakrino_conformal_selection(self, X, y, n_target_features):
        """DIAKRINO schema-v2 conformal FDP-controlled selector (opt-in scaffold)."""
        from .diakrino_conformal_selection import diakrino_conformal_selection_method

        results, all_scores, meta = diakrino_conformal_selection_method(
            X,
            y,
            n_target_features,
            sidecar_path=getattr(self, "diakrino_prior_sidecar_path", ""),
            dataset_id=str(getattr(self, "diakrino_prior_dataset_id", "") or "") or None,
            feature_mapping=getattr(self, "_current_feature_mapping", None),
            qualification_record=str(getattr(self, "diakrino_conformal_qualification_record", "") or ""),
            enabled=bool(getattr(self, "diakrino_conformal_selection_enabled", False)),
            target_fdp=float(getattr(self, "diakrino_conformal_target_fdp", 0.20)),
            calibrate=str(getattr(self, "diakrino_conformal_calibrate", "chunk_zscore") or "chunk_zscore"),
            null_fraction=float(getattr(self, "diakrino_conformal_null_fraction", 0.50)),
            min_null_scores=int(getattr(self, "diakrino_conformal_min_null_scores", 4)),
            max_features=int(getattr(self, "diakrino_conformal_max_features", 0)),
        )
        self._diakrino_conformal_selection_meta = dict(meta)
    ...
    def _mrmr_jmi_selection(self, X, y, n_target_features):
        ...
        return mrmr_jmi_selection(
            ...
            mrmr_max_unique_pair_evaluations=self.mrmr_max_unique_pair_evaluations,
            mrmr_max_runtime_seconds=self.mrmr_max_runtime_seconds,
            mrmr_budget_fallback_mode=self.mrmr_budget_fallback_mode,
        )
    ...
    def _pathway_group_sparse_lasso_selection(self, X, y, n_target_features):
        """Soft pathway-proxy group sparse lasso (T-R-393 Phase 1)."""
        from .methods.group_fs import pathway_group_sparse_lasso_selection
        return pathway_group_sparse_lasso_selection(
            X, y, n_target_features,
            n_groups=self.pathway_group_sparse_lasso_n_groups,
            max_group_size=self.pathway_group_sparse_lasso_max_group_size,
            alpha=self.group_sparse_lasso_alpha,
            random_state=self.random_state,
        )
    ...
    def _method_execution_failure_result(
        self,
        *,
        method_name: str,
        ...
        """Return a fail-closed, serializable terminal method-execution payload."""

        normalized_status = str(status or "failed")
        provenance: Dict[str, Any] = {
            "schema_version": 1,
            "backend": str(backend),
            "status": normalized_status,
            "method_name": str(method_name),
            "method_seed": None if method_seed is None else int(method_seed),
            "timeout_seconds": float(self.method_timeout_seconds),
            "max_rss_mb": float(self.method_max_rss_mb),
            "runtime_seconds": float(max(0.0, runtime_seconds)),
            "peak_rss_bytes": int(max(0, peak_rss_bytes)),
            "worker_exit_code": (
                None if worker_exit_code is None else int(worker_exit_code)
            ),
        }
        if exception_type:
            provenance["exception_type"] = str(exception_type)
        if exception_message:
            provenance["exception_message"] = str(exception_message)[:512]
        ...
        return (
            {
                ...
                "timed_out": bool(normalized_status == "timed_out"),
                "timeout_seconds": float(self.method_timeout_seconds),
                "resource_exhausted": bool(
                    normalized_status == "rss_limit_exceeded"
                ),
    ...
    def _attach_method_execution_provenance(
        self,
        result: Any,
        *,
        method_name: str,
        ...
        """Attach parent-observed process metadata without changing method scores."""

        if not isinstance(result, tuple) or len(result) != 2:
            return self._method_execution_failure_result(
                method_name=method_name,
                method_seed=method_seed,
                status="failed",
                stop_reason="malformed_worker_result",
                runtime_seconds=runtime_seconds,
                backend=backend,
                peak_rss_bytes=peak_rss_bytes,
                worker_exit_code=worker_exit_code,
                exception_type="MalformedWorkerResult",
                exception_message=(
                    "method worker did not return a two-part result tuple"
                ),
            )

        method_result, all_scores = result
        if not isinstance(method_result, dict):
            return self._method_execution_failure_result(
                ...
                worker_exit_code=worker_exit_code,
                exception_type="MalformedWorkerResult",
                exception_message="method worker result payload must be a dictionary",
            )

        enriched = dict(method_result)
        prior_status = str(enriched.get("execution_status", "")).strip().lower()
        ...
        effective_status = (
            prior_status if prior_status in terminal_failure_statuses else str(status)
        )
        prior_provenance = enriched.get("execution_provenance", {})
        provenance = (
        ...
        enriched["execution_status"] = effective_status
        enriched["execution_provenance"] = provenance
        return enriched, all_scores
    ...
    def _run_single_method(
        ...
        """
        # Bind the method-local estimator cap without leaking it to later work.
        from tabnetics.core.runtime import get_sklearn_n_jobs, set_sklearn_n_jobs

        previous_sklearn_n_jobs = get_sklearn_n_jobs()
        ...
        try:
            ...
            runtime = perf_counter() - t0
            result = self._method_execution_failure_result(
                method_name=method_name,
                method_seed=method_seed,
                status="timed_out",
                stop_reason="method_timeout",
                runtime_seconds=runtime,
                backend="sequential_signal",
            )
            ...
            return (
                method_name,
                self._method_execution_failure_result(
                    method_name=method_name,
                    method_seed=method_seed,
                    status="failed",
                    stop_reason="method_exception",
                    runtime_seconds=runtime,
                    backend="sequential",
            ...
            set_sklearn_n_jobs(previous_sklearn_n_jobs)
    ...
    def _run_selection_methods(self, X_uncorr, y, n_target, class_pareto_min_classes: Optional[int] = None):
        ...
        method_results = {}
        method_runtimes = {}
        enabled_methods = getattr(self, "_fit_enabled_methods", self.enabled_methods)
        ...
        for _key, _spec in METHOD_REGISTRY.items():
            if _spec.maturity == 'deprecated':
                continue
            # Hard opt-in gate: default_enabled=False methods (e.g. diakrino_prior) are
            # skipped unless EXPLICITLY listed in enabled_methods, independent of
            # profile membership.  enabled_methods=None ("run-all") still excludes
            # them -> registering such a method never changes production defaults.
            if method_excluded_by_default(_spec, enabled_methods):
                continue
        ...
        logger.info("Applying %d selection methods to %d features",
                   len([m for m in method_registry if enabled_methods is None or m[0] in enabled_methods]),
                   X_uncorr.shape[1])

        # Build tasks list (filtered by enabled_methods).
        tasks = []
        for method_name, message, method_fn in method_registry:
            if enabled_methods is not None and method_name not in enabled_methods:
                continue
        ...
        parallel_dispatch = bool(_fs_n_jobs > 1 and len(tasks) > 1)
        hard_bounds_requested = bool(
            float(self.method_timeout_seconds) > 0.0
            or float(self.method_max_rss_mb) > 0.0
        )
        use_process_isolation = bool(parallel_dispatch or hard_bounds_requested)

        if use_process_isolation:
            # A method must never share selector state with a concurrent method.
            # The frozen task is the sole parent-to-worker execution contract;
            # the POSIX child inherits the configured selector via copy-on-write.
            isolated_tasks = []
            worker_arguments = {}
            max_rss_bytes = int(round(float(self.method_max_rss_mb) * 1024.0 * 1024.0))
            for ordinal, (method_name, message, method_fn, contract) in enumerate(
                tasks
            ):
                logger.info("%s", message)
                execution_task = MethodExecutionTask(
                    ordinal=int(ordinal),
                    method_name=str(method_name),
                    method_seed=self._derive_method_seed(method_name),
                    timeout_seconds=float(self.method_timeout_seconds),
                    max_rss_bytes=max(0, max_rss_bytes),
                    # Outer process concurrency is the public dispatch cap;
                    # nested sklearn/joblib work must not multiply it.
                    inner_n_jobs=1,
                )
                isolated_tasks.append(execution_task)
                worker_arguments[int(ordinal)] = (method_fn, contract)

            if not process_isolation_available():
                if hard_bounds_requested:
                    # Do not pretend a signal/thread timeout is hard when the
                    # platform cannot create an independently killable worker.
                    logger.error(
                        "Skipping bounded FS methods: POSIX process isolation "
                        "is unavailable"
                    )
                    for execution_task in isolated_tasks:
                        method_results[execution_task.method_name] = (
                            self._method_execution_failure_result(
                                method_name=execution_task.method_name,
                                method_seed=execution_task.method_seed,
                                status="isolation_unavailable",
                                stop_reason="process_isolation_unavailable",
                                runtime_seconds=0.0,
                                backend="unavailable",
                            )
                        )
                        method_runtimes[execution_task.method_name] = 0.0
                else:
                    # Multi-worker dispatch without a hard bound remains
                    # deterministic and state-safe when serialized.  This is
                    # intentionally not a fallback to the prior thread pool.
                    logger.warning(
                        "POSIX process isolation is unavailable; serializing %d "
                        "FS methods",
                        len(isolated_tasks),
                    )
                    for execution_task in isolated_tasks:
                        method_fn, contract = worker_arguments[execution_task.ordinal]
                        mname, result, runtime = self._run_single_method(
                            execution_task.method_name,
                            method_fn,
                            contract,
                            X_uncorr,
                            y,
                            ...
                            class_pareto_min_classes,
                            False,
                            execution_task.method_seed,
                        )
                        method_results[mname] = (
                            self._attach_method_execution_provenance(
                                result,
                                method_name=mname,
                                method_seed=execution_task.method_seed,
                                status="completed",
                                runtime_seconds=runtime,
                                backend="sequential_process_unavailable",
                                peak_rss_bytes=0,
                ...
                def _run_isolated_task(execution_task: MethodExecutionTask):
                    method_fn, contract = worker_arguments[execution_task.ordinal]
                    parent_parallel_n_jobs = self.parallel_n_jobs
                    self.parallel_n_jobs = int(execution_task.inner_n_jobs)
                    try:
                        return self._run_single_method(
                            execution_task.method_name,
                            method_fn,
                            contract,
                            X_uncorr,
                        ...
                        self.parallel_n_jobs = parent_parallel_n_jobs

                worker_cap = min(
                    len(isolated_tasks),
                    max(1, _fs_n_jobs if parallel_dispatch else 1),
                )
                logger.info(
                    "Running %d FS methods with %d process-isolated workers",
                    len(isolated_tasks),
                    worker_cap,
                )
                try:
                    outcomes = execute_isolated_method_tasks(
                        isolated_tasks,
                        worker=_run_isolated_task,
                        max_workers=worker_cap,
                    )
                except ProcessIsolationUnavailable:
                    # A runtime capability change after the initial probe has
                    # the same fail-closed semantics as an unavailable platform.
                    outcomes = tuple()
                    for execution_task in isolated_tasks:
                        method_results[execution_task.method_name] = (
                            self._method_execution_failure_result(
                ...
                for outcome in outcomes:
                    ...
                    if outcome.status == "completed":
                        ...
                        if (
                            ...
                            and len(payload) == 3
                            and str(payload[0]) == method_name
                        ):
                            result = payload[1]
                            raw_runtime = payload[2]
                            runtime = (
                                float(raw_runtime)
                                if isinstance(raw_runtime, (int, float, np.floating))
                                and np.isfinite(float(raw_runtime))
                                and float(raw_runtime) >= 0.0
                                else float(outcome.runtime_seconds)
                            )
                            ...
                            method_runtimes[method_name] = runtime
                            continue
                        ...
                        method_results[method_name] = (
                            self._method_execution_failure_result(
                                ...
                                exception_type="MalformedWorkerResult",
                                exception_message=(
                                    "worker payload did not match the method result "
                                    "contract"
                                ),
                            )
                        )
                        method_runtimes[method_name] = float(outcome.runtime_seconds)
                        continue

                    stop_reason_by_status = {
                        "timed_out": "method_timeout",
                        "rss_limit_exceeded": "method_rss_limit_exceeded",
                        "crashed": "worker_crash",
                        "failed": "worker_exception",
                        "launch_failed": "worker_launch_failed",
                        "orphaned_descendant": "orphaned_worker_descendant",
                    }
                    method_results[method_name] = self._method_execution_failure_result(
                        method_name=method_name,
                        method_seed=execution_task.method_seed,
                        status=outcome.status,
                        stop_reason=stop_reason_by_status.get(
                            outcome.status, "worker_execution_failed"
                        ),
                        runtime_seconds=float(outcome.runtime_seconds),
                        backend="process",
                        peak_rss_bytes=int(outcome.peak_rss_bytes),
            ...
            # Default single-worker, unbounded execution remains the original
            # in-process path for compatibility and zero process overhead.
    ...
    def _get_inner_cv_splits(self, X, y):
        ...
        return get_inner_cv_splits(
            ...
            self.inner_cv_repeats,
            split_plan_provider=getattr(self, "_resampling_plan_provider", None),
            purpose="feature_selector_oracle_cv",
        )
    ...
    def _compute_fold_conformal_efficiency(self, X_train, y_train, X_val, y_val):
        ...
        if (
            not bool(
                getattr(
                    self,
                    "_fit_use_conformal_efficiency",
                    getattr(self, "use_conformal_efficiency", False),
                )
            )
        ...
        return estimate_oracle_preferences(
            ...
            use_conformal_efficiency=bool(
                getattr(
                    self,
                    "_fit_use_conformal_efficiency",
                    self.use_conformal_efficiency,
                )
            ),
            conformal_efficiency_method=self.conformal_efficiency_method,
            complexity_conditioning=self.complexity_conditioning,
    ...
    def _mnpo_select_features(
        ...
        diakrino_relevance_vector = None
        if bool(getattr(self.oracle, "use_diakrino_relevance_oracle", False)):
            diakrino_relevance_vector = self._compute_aligned_diakrino_scores(
                str(getattr(self.oracle, "diakrino_relevance_score_column", "prior_logit")),
                calibrate="chunk_zscore",
            )
        diakrino_selector_prior = None
        diakrino_selector_prior_trust_record = None
        if bool(getattr(self.oracle, "use_diakrino_selector_prior", False)):
            diakrino_selector_prior = self._compute_diakrino_selector_weights()
            diakrino_selector_prior_trust_record = getattr(self, "_diakrino_selector_prior_trust_record", None)
        ...
        return mnpo_select_features(
            ...
            diakrino_selector_prior=diakrino_selector_prior,
            diakrino_selector_prior_trust_record=diakrino_selector_prior_trust_record,
            ...
            rashomon_enabled=bool(
                getattr(self, "_fit_rashomon_enabled", self.rashomon_enabled)
            ),
    ...
    def fit_transform(
        ...
        *,
        ...
        if not isinstance(y, np.ndarray): 
            y = np.array(y)
        self._resampling_plan_provider = resampling_plan_provider
        policy_name = str(resampling_policy or "iid").strip().lower()
        if policy_name != "iid" and resampling_plan_provider is None:
            raise ValueError(
                "non_iid_internal_resampling_unsupported:"
                "feature_selection_resampling_plan_provider_missing"
            )
        requested_methods = (
            None if self.enabled_methods is None else set(self.enabled_methods)
        )
        effective_methods = (
            {
                key
                for key, spec in METHOD_REGISTRY.items()
                if bool(spec.default_enabled)
            }
            if requested_methods is None
            else set(requested_methods)
        )
        self.resampling_diagnostics_ = {
            "policy": policy_name,
            "provider_present": bool(resampling_plan_provider is not None),
            "requested_methods": sorted(effective_methods),
            "effective_methods": sorted(effective_methods),
            "excluded_methods": [],
            "exclusion_reason": "",
            "requested_oracles": {
                "conformal_efficiency": bool(self.use_conformal_efficiency),
            },
            "effective_oracles": {
                "conformal_efficiency": bool(self.use_conformal_efficiency),
            },
            ...
            "requested_post_selection_diagnostics": {
                "rashomon_importance": bool(self.rashomon_enabled),
            },
        ...
        if resampling_plan_provider is not None and policy_name != "iid":
            excluded = sorted(
                key
                for key in effective_methods
                if key in METHOD_REGISTRY
                and METHOD_REGISTRY[key].structured_resampling != "supported"
            )
            if excluded:
                effective_methods.difference_update(excluded)
                self.resampling_diagnostics_["excluded_methods"] = excluded
                self.resampling_diagnostics_["exclusion_reason"] = (
                    "non_iid_internal_resampling_unsupported"
                )
            if not effective_methods:
                raise ValueError(
                    "non_iid_internal_resampling_unsupported:feature_selection_all_methods"
                )
        effective_conformal_efficiency = bool(self.use_conformal_efficiency)
        if policy_name != "iid" and effective_conformal_efficiency:
            effective_conformal_efficiency = False
            self.resampling_diagnostics_["excluded_oracles"] = [
                "conformal_efficiency"
            ]
            self.resampling_diagnostics_["oracle_exclusion_reason"] = (
                "non_iid_internal_resampling_unsupported:"
                "feature_selection_conformal_efficiency"
            )
        self._fit_use_conformal_efficiency = effective_conformal_efficiency
        self.oracle.use_conformal_efficiency = effective_conformal_efficiency
        self.resampling_diagnostics_["effective_oracles"] = {
            "conformal_efficiency": effective_conformal_efficiency,
        }
        effective_rashomon = bool(self.rashomon_enabled)
        if policy_name != "iid" and effective_rashomon:
            effective_rashomon = False
            self.resampling_diagnostics_["excluded_post_selection_diagnostics"] = [
                "rashomon_importance"
            ]
            self.resampling_diagnostics_["post_selection_exclusion_reason"] = (
                "non_iid_internal_resampling_unsupported:"
                "feature_selection_rashomon_importance"
            )
        self._fit_rashomon_enabled = effective_rashomon
        self.resampling_diagnostics_["effective_post_selection_diagnostics"] = {
            "rashomon_importance": effective_rashomon,
        }
        self._fit_enabled_methods = (
            None
            if requested_methods is None
            and not (resampling_plan_provider is not None and policy_name != "iid")
            else effective_methods
        )
        self.resampling_diagnostics_["effective_methods"] = sorted(effective_methods)
        ...
        try:
            ...
            if "No feature in X meets the variance threshold" not in str(exc):
                raise
            warnings.warn(
                "VarianceThreshold would drop all features; keeping all features "
                "for this selector fit.",
                RuntimeWarning,
            )
            logger.warning(
                "VarianceThreshold(threshold=%s) would drop all %d features; "
                "keeping all features for this selector fit.",
                self.variance_threshold,
                n_features,
            )
            X_var_filtered = X
            var_support = np.ones(n_features, dtype=bool)
        ...
        # Stage the current X_uncorr-column -> original-feature mapping so the opt-in
        # DIAKRINO candidate methods can align their original-indexed sidecar scores.
        self._current_feature_mapping = np.asarray(feature_mapping, dtype=np.int64)
        ...
        mnpo_summary = None
        self.mnpo_diagnostics_ = {}
        from .mnpo.portfolio import resolve_method_result_eligibility

        selector_candidate_statuses = resolve_method_result_eligibility(method_results)
        legacy_vote_method_results = {
            name: payload
            for name, payload in method_results.items()
            if bool(
                selector_candidate_statuses.get(str(name), {}).get(
        ...
        degraded_methods = [
            name
            for name, status in selector_candidate_statuses.items()
            if str(status.get("status", "eligible")) != "eligible"
        ]
        aggregation_status = "legacy_voting"
        if self.selection_strategy == 'mnpo_portfolio':
            ...
            if mnpo_output is not None:
                ...
                aggregation_status = "mnpo_portfolio"
            else:
                logger.warning("MNPO strategy fallback: using legacy weighted voting.")
                feature_votes, feature_details = self._calculate_weighted_votes(
                    legacy_vote_method_results,
                    X_uncorr.shape[1],
                )
                selected_indices_local = np.argsort(feature_votes)[::-1][:n_final_features]
                aggregation_status = "mnpo_legacy_vote_fallback"
                self.mnpo_diagnostics_ = {
                    "status": "no_eligible_mnpo_candidates",
                    "selector_candidate_statuses": {
                        name: dict(status)
                        for name, status in selector_candidate_statuses.items()
                    },
                    "mnpo_degraded_excluded_methods": list(degraded_methods),
                }
        else:
            feature_votes, feature_details = self._calculate_weighted_votes(
                legacy_vote_method_results,
        ...
        has_eligible_selector_evidence = bool(
            np.any(np.asarray(feature_votes, dtype=float) > 0.0)
        )
        fail_closed_no_eligible_evidence = bool(
            any(
                bool(status.get("fail_closed", False))
                for status in selector_candidate_statuses.values()
            )
            and not has_eligible_selector_evidence
        )
        if fail_closed_no_eligible_evidence:
            selected_indices_local = np.array([], dtype=int)
            aggregation_status = "fail_closed_no_eligible_selector_evidence"
        ...
        if (
            selected_indices_local.size < n_final_features
            and not fail_closed_no_eligible_evidence
        ):
            fill = []
        ...
        if self.importance_uq_enabled:
            try:
                ...
                uq = compute_feature_importance_uq(
                    ...
                    split_plan_provider=getattr(
                        self, "_resampling_plan_provider", None
                    ),
                )
        ...
        result = FeatureSelectionResult(
            ...
            config={
                ...
                'selection_aggregation_status': aggregation_status,
                'selection_aggregation_fail_closed': bool(
                    fail_closed_no_eligible_evidence
                ),
                'selection_aggregation_has_eligible_evidence': bool(
                    has_eligible_selector_evidence
                ),
                'selection_aggregation_eligible_methods': sorted(
                    legacy_vote_method_results.keys()
                ),
                'selection_aggregation_degraded_methods': sorted(degraded_methods),
                'selector_candidate_statuses': {
                    name: dict(status)
                    for name, status in selector_candidate_statuses.items()
                },
                ...
                'method_max_rss_mb': self.method_max_rss_mb,
                'parallel_n_jobs': self.parallel_n_jobs,
                'linear_svm_max_iter': self.linear_svm_max_iter,
                'use_robust_oracle': self.use_robust_oracle,
                'use_diversity_oracle': self.use_diversity_oracle,
                'pathway_group_sparse_lasso_n_groups': int(self.pathway_group_sparse_lasso_n_groups),
                'pathway_group_sparse_lasso_max_group_size': int(
                    self.pathway_group_sparse_lasso_max_group_size
                ),
                'use_cvar': bool(self.use_cvar),
                'cvar_alpha': float(self.cvar_alpha),
                ...
                'shapley_bayesian_prior_strength': float(self.shapley_bayesian_prior_strength),
                'use_interaction_oracle': bool(self.use_interaction_oracle),
                'use_diakrino_relevance_oracle': bool(self.use_diakrino_relevance_oracle),
                'diakrino_relevance_min_n_train': int(self.diakrino_relevance_min_n_train),
                'diakrino_relevance_score_column': str(self.diakrino_relevance_score_column),
                'use_diakrino_selector_prior': bool(self.use_diakrino_selector_prior),
                'diakrino_selector_prior_weight': float(self.diakrino_selector_prior_weight),
                'diakrino_selector_prior_calibration': str(self.diakrino_selector_prior_calibration),
                'diakrino_selector_prior_qualification_record': str(self.diakrino_selector_prior_qualification_record),
                'diakrino_selector_prior_gate': dict(getattr(self, '_diakrino_selector_prior_gate_meta', {}) or {}),
                'diakrino_conformal_selection_enabled': bool(self.diakrino_conformal_selection_enabled),
                'diakrino_conformal_target_fdp': float(self.diakrino_conformal_target_fdp),
                'diakrino_conformal_calibrate': str(self.diakrino_conformal_calibrate),
                'diakrino_conformal_null_fraction': float(self.diakrino_conformal_null_fraction),
                'diakrino_conformal_min_null_scores': int(self.diakrino_conformal_min_null_scores),
                'diakrino_conformal_max_features': int(self.diakrino_conformal_max_features),
                'diakrino_conformal_qualification_record': str(self.diakrino_conformal_qualification_record),
                'diakrino_conformal_selection_meta': dict(getattr(self, '_diakrino_conformal_selection_meta', {}) or {}),
                'interaction_oracle_min_n_train': int(self.interaction_oracle_min_n_train),
                'interaction_oracle_pool_size_cap': int(self.interaction_oracle_pool_size_cap),
                'interaction_oracle_pair_cap': int(self.interaction_oracle_pair_cap),
                'use_ubayfs': bool(self.use_ubayfs),
                ...
                'mrmr_max_unique_pair_evaluations': int(self.mrmr_max_unique_pair_evaluations),
                'mrmr_max_runtime_seconds': float(self.mrmr_max_runtime_seconds),
                'mrmr_budget_fallback_mode': str(self.mrmr_budget_fallback_mode),
                ...
                'copula_time_budget_seconds': self.copula_time_budget_seconds,
                'copula_vine_num_threads': self.copula_vine_num_threads,
                'copula_lasso_max_iter': int(self.copula_lasso_max_iter),
                'copula_lasso_n_jobs': int(self.copula_lasso_n_jobs),
                ...
                'enabled_methods': sorted(
                    getattr(self, '_fit_enabled_methods', self.enabled_methods)
                ) if getattr(self, '_fit_enabled_methods', self.enabled_methods) is not None else None,
                'resampling': dict(getattr(self, 'resampling_diagnostics_', {}) or {}),
            }
        )
