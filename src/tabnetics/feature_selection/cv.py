...
def _provider_splits(
    ...
    *,
    ...
    pairs = plan.index_pairs() if hasattr(plan, "index_pairs") else tuple(plan)
    return [
        (
...
def _provider_n_splits(n_samples, y, problem_type, inner_cv_splits):
    if int(n_samples) < 4:
        return 2
    requested = int(max(2, min(int(inner_cv_splits), int(n_samples) - 1)))
    if problem_type != "classification":
        return requested
    counts = {}
    for value in np.asarray(y, dtype=object).ravel().tolist():
        key = (type(value).__module__, type(value).__qualname__, repr(value))
        counts[key] = counts.get(key, 0) + 1
    if not counts:
        return requested
    return int(max(2, min(requested, max(2, min(counts.values())))))
...
def get_cv_strategy(
    ...
    """
    if split_plan_provider is not None:
        return _provider_splits(
            split_plan_provider,
            y,
            n_splits=_provider_n_splits(
                n_samples, y, problem_type, inner_cv_splits
            ),
            n_repeats=max(1, int(inner_cv_repeats)),
            purpose=purpose,
...
def get_inner_cv_splits(
    ...
    *,
    split_plan_provider=None,
    purpose="feature_selector_inner_cv",
):
...
def compute_feature_importance_uq(
    ...
    *,
    ...
    splits = get_inner_cv_splits(
        ...
        split_plan_provider=split_plan_provider,
        purpose="feature_importance_uq_cv",
    )
    if not splits:
