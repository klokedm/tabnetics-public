...
def estimate_oracle_preferences(
    ...
    *,
    ...
    use_diakrino_relevance_oracle = bool(_oc("use_diakrino_relevance_oracle", use_diakrino_relevance_oracle))
    ...
    if use_diakrino_relevance_oracle:
        # §2.2 DIAKRINO relevance oracle: scalar per candidate = mean chunk-calibrated DIAKRINO
        # per-feature relevance over the features that candidate SELECTED (= dot/|selected|,
        # "how much DIAKRINO mass this method captured").  Biases method trust, never selects.
        min_n_gate = int(max(2, int(_oc("diakrino_relevance_min_n_train", 100))))
        n_train = 0
        if X_pool is not None:
            try:
                n_train = int(np.asarray(X_pool).shape[0])
        ...
        if n_train <= 0 and candidate_names:
            n_train = int(max(0, evaluation[candidate_names[0]].get("n_samples", 0)))
        rel = None if diakrino_relevance_vector is None else np.asarray(diakrino_relevance_vector, dtype=float).ravel()
        if rel is None or rel.size == 0:
            oracle_components["diakrino_relevance"] = {
                "enabled": True, "applied": False, "reason": "missing_diakrino_vector",
            }
        elif n_train < min_n_gate:
            oracle_components["diakrino_relevance"] = {
                "enabled": True, "applied": False, "reason": "min_n_gate",
                "n_train": int(n_train), "diakrino_relevance_min_n_train": int(min_n_gate),
            }
        else:
            scores = np.zeros(m, dtype=float)
            for idx, name in enumerate(candidate_names):
                ...
                selected = selected[(selected >= 0) & (selected < rel.shape[0])]
                scores[idx] = float(np.mean(rel[selected])) if selected.size else 0.0
            diakrino_matrix, meta = _mnpo_matrix_from_scalar_scores(
                np.asarray(scores, dtype=float),
                tie_margin=0.01,
                use_qre_smoothing=use_qre_smoothing,
                qre_temperature_gamma=qre_temperature_gamma,
            )
            oracle_matrices["diakrino_relevance"] = diakrino_matrix
            oracle_scores["diakrino_relevance"] = np.asarray(scores, dtype=float)
            oracle_pairwise_meta["diakrino_relevance"] = dict(meta)
            oracle_components["diakrino_relevance"] = {
                "enabled": True, "applied": True, "reason": "ok",
                "n_train": int(n_train), "diakrino_relevance_min_n_train": int(min_n_gate),
            }
