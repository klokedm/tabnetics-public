...
from .dependence import (
    ...
    compute_dependence_descriptors,
    dependence_descriptor_model_input_enabled,
)
...
def _router_artifact_identity(
    artifact_dir: Path,
    *,
    manifest_bytes: bytes,
    score_models_bytes: bytes,
) -> Dict[str, Any]:
    """Return the exact router bytes bound to one runtime decision."""

    digest = hashlib.sha256()
    for label, payload in (
        ("manifest.json", manifest_bytes),
        ("score_models.joblib", score_models_bytes),
    ):
    ...
    return {
        "schema_version": "tabnetics_router_artifact_identity_v1",
        "artifact_dir": str(artifact_dir.resolve()),
        "manifest_sha256": _sha256_bytes(manifest_bytes),
        "manifest_size_bytes": int(len(manifest_bytes)),
        "score_models_sha256": _sha256_bytes(score_models_bytes),
        "score_models_size_bytes": int(len(score_models_bytes)),
...
def _metadata_float(value: Any, default: float = 0.0) -> float:
    out = _raw_float(value)
    if np.isfinite(out):
        return float(out)
    if np.isnan(out):
        return float(default)
    return float(np.sign(out) * 1e12)
...
def _cheap_descriptor(
    X: np.ndarray,
    y: np.ndarray,
    *,
    missingness_source: np.ndarray | None = None,
) -> Dict[str, float]:
    n, p = X.shape
    ...
    missingness = compute_missingness_descriptors(
        X if missingness_source is None else missingness_source
    )
...
def _validate_dataset_descriptor_xy(X: Any, y: Any) -> tuple[np.ndarray, np.ndarray]:
    X_arr = np.asarray(X, dtype=float)
    y_arr = np.asarray(y)
    if X_arr.ndim != 2:
        raise ValueError(f"auto-router descriptor X must be 2D, got shape {tuple(X_arr.shape)}")
    if y_arr.ndim != 1:
        raise ValueError(f"auto-router descriptor labels must be 1D, got shape {tuple(y_arr.shape)}")
    if int(X_arr.shape[0]) != int(y_arr.shape[0]):
        raise ValueError(
            "auto-router descriptor X and labels must have the same number of rows; "
...
def compute_dataset_descriptor(
    ...
    X_raw, y_arr = _validate_dataset_descriptor_xy(X, y)
    X_arr = _sanitize_matrix(X_raw)
    n_samples, n_features = X_arr.shape
    cheap = _cheap_descriptor(X_arr, y_arr, missingness_source=X_raw)
    ...
    dependence = compute_dependence_descriptors(X_arr, y_arr)
    feature_vector = {
        **cheap,
        "n": float(n_samples),
        "p": float(n_features),
        ...
        "mutual_info_mean": float(dependence["mutual_info_mean"]),
        "mutual_info_std": float(dependence["mutual_info_std"]),
        "pairwise_redundancy_ratio": float(dependence["pairwise_redundancy_ratio"]),
    ...
    diakrino_dispersion = meta.get("diakrino_router_dispersion_descriptor")
    if isinstance(diakrino_dispersion, Mapping):
        summary = diakrino_dispersion.get("summary")
        if isinstance(summary, Mapping):
            for key, value in dict(summary).items():
                if isinstance(value, (int, float, np.integer, np.floating)):
                    feature_vector[f"diakrino_dispersion_{key}"] = _safe_float(value, 0.0)
    return {
...
@dataclass
class DescriptorOODResult:
    """Descriptor-space OOD evaluation for one auto-router decision."""

    gate_available: bool = False
    gate_enabled: bool = False
    defaulted: bool = False
    score: float = 0.0
    normalized_excess: float = 0.0
    ...
    def to_metadata(self) -> Dict[str, Any]:
        return {
            "descriptor_ood_gate_available": bool(self.gate_available),
            "descriptor_ood_gate_enabled": bool(self.gate_enabled),
            "descriptor_ood_defaulted": bool(self.defaulted),
            "auto_router_ood_defaulted": bool(self.defaulted),
            "descriptor_ood_score": _metadata_float(self.score),
            "auto_router_ood_score": _metadata_float(self.score),
            "descriptor_ood_normalized_excess": _metadata_float(self.normalized_excess),
            "descriptor_ood_max_zscore": _metadata_float(self.max_zscore),
            "descriptor_ood_outside_feature_count": int(self.outside_feature_count),
            "descriptor_ood_feature_count": int(self.feature_count),
            "descriptor_ood_outside_features": list(self.outside_features),
            "descriptor_ood_z_threshold": _metadata_float(self.z_threshold),
            "descriptor_ood_max_outside_features": int(self.max_outside_features),
        }


@dataclass
class DescriptorOODGate:
    """Robust per-dimension descriptor envelope persisted with router artifacts."""
    ...
    policy: str = "cheap_stat_robust_envelope_zscore_v1"

    @classmethod
    def from_manifest(cls, payload: Optional[Mapping[str, Any]]) -> Optional["DescriptorOODGate"]:
        if not isinstance(payload, Mapping) or not payload:
            return None
        raw_names = payload.get("feature_names", [])
        names = tuple(str(name) for name in (raw_names if raw_names is not None else []) if str(name))
        if not names:
            return None
        median = np.asarray(payload.get("feature_median", []), dtype=float).reshape(-1)
        scale = np.asarray(payload.get("feature_scale", []), dtype=float).reshape(-1)
        low = np.asarray(payload.get("feature_low", []), dtype=float).reshape(-1)
        high = np.asarray(payload.get("feature_high", []), dtype=float).reshape(-1)
        n = min(len(names), median.size, scale.size, low.size, high.size)
        if n <= 0:
            return None
        median = np.nan_to_num(median[:n], nan=0.0, posinf=0.0, neginf=0.0)
        scale = np.nan_to_num(np.abs(scale[:n]), nan=1.0, posinf=1.0, neginf=1.0)
        scale = np.maximum(scale, 1e-9)
        low = low[:n]
        high = high[:n]
        low = np.where(np.isfinite(low), low, median)
        high = np.where(np.isfinite(high), high, median)
        ...
        return cls(
            ...
            feature_low=low,
            feature_high=high,
            z_threshold=float(payload.get("z_threshold", 4.0) or 4.0),
            score_threshold=float(payload.get("score_threshold", 0.0) or 0.0),
            max_outside_features=int(payload.get("max_outside_features", 0) or 0),
            enabled=bool(payload.get("enabled", True)),
            schema_version=int(payload.get("schema_version", 1) or 1),
            policy=str(payload.get("policy", "cheap_stat_robust_envelope_zscore_v1") or ""),
        )
    ...
    def to_manifest(self) -> Dict[str, Any]:
        return {
            ...
            "feature_median": self.feature_median.astype(float).tolist(),
            "feature_scale": self.feature_scale.astype(float).tolist(),
            "feature_low": self.feature_low.astype(float).tolist(),
            "feature_high": self.feature_high.astype(float).tolist(),
            "z_threshold": float(self.z_threshold),
            "score_threshold": float(self.score_threshold),
            "max_outside_features": int(self.max_outside_features),
        }

    def evaluate(self, descriptor: Mapping[str, Any], *, enabled: bool) -> DescriptorOODResult:
        active = bool(enabled and self.enabled)
        result = DescriptorOODResult(
            gate_available=True,
            gate_enabled=active,
        ...
        raw = np.asarray([_raw_float(payload.get(name)) for name in self.feature_names], dtype=float)
        finite = np.isfinite(raw)
        values = np.where(finite, raw, self.feature_median)
        below = finite & (raw < self.feature_low)
        above = finite & (raw > self.feature_high)
        nonfinite = ~finite
        outside = below | above | nonfinite
        zscores = np.abs(values - self.feature_median) / np.maximum(self.feature_scale, 1e-9)
        zscores = np.where(finite, zscores, 1e12)
        low_excess = (self.feature_low - values) / np.maximum(self.feature_scale, 1e-9)
        high_excess = (values - self.feature_high) / np.maximum(self.feature_scale, 1e-9)
        excess = np.maximum.reduce([low_excess, high_excess, np.zeros_like(values)])
        excess = np.where(finite, excess, 1e12)
        outside_count = int(np.sum(outside))
        max_zscore = float(np.max(zscores)) if zscores.size else 0.0
        max_excess = float(np.max(excess)) if excess.size else 0.0
        z_component = max_zscore / max(float(self.z_threshold), 1e-9)
        score = float(max(max_excess, z_component if outside_count else 0.0))
        outside_names = [
            str(name)
            for name, is_outside in zip(self.feature_names, outside.tolist())
            if bool(is_outside)
        ][:12]
        result.outside_feature_count = outside_count
        result.outside_features = outside_names
        result.normalized_excess = max_excess
        result.max_zscore = max_zscore
        result.score = score
        result.defaulted = bool(
            outside_count > int(self.max_outside_features)
            and score > float(self.score_threshold)
        )
        return result
...
class AutoRouterOutput:
    ...
    def to_snapshot(self) -> Dict[str, Any]:
        artifact_identity = dict(self.metadata.get("artifact_identity", {}) or {})
        return {
            "auto_router_enabled": True,
            "auto_router_used": True,
            "auto_router_version": str(AUTO_ROUTER_ARTIFACT_VERSION),
            "auto_router_artifact_identity": artifact_identity,
            "auto_router_artifact_sha256": str(artifact_identity.get("artifact_sha256", "")),
            "auto_router_artifact_manifest_sha256": str(
                artifact_identity.get("manifest_sha256", "")
            ),
            "auto_router_artifact_score_models_sha256": str(
                artifact_identity.get("score_models_sha256", "")
            ),
            "auto_router_selected_candidate_id": str(self.metadata.get("selected_candidate_id", "")),
            "auto_router_raw_selected_candidate_id": str(self.metadata.get("raw_selected_candidate_id", "")),
            "auto_router_default_candidate_id": str(self.metadata.get("default_candidate_id", "")),
            "auto_router_default_candidate_available": bool(
                self.metadata.get("default_candidate_available", True)
            ),
            ...
            "auto_router_cost_tie_break_margin": float(self.metadata.get("cost_tie_break_margin", 0.0)),
            "auto_router_cost_tie_break_applied": bool(
                self.metadata.get("cost_tie_break_applied", False)
            ),
            "auto_router_cost_tie_break_raw_candidate_id": str(
                self.metadata.get("cost_tie_break_raw_candidate_id", "")
            ),
            "auto_router_cost_tie_break_selected_candidate_id": str(
                self.metadata.get("cost_tie_break_selected_candidate_id", "")
            ),
            "auto_router_cost_tie_break_eligible_candidate_ids": list(
                self.metadata.get("cost_tie_break_eligible_candidate_ids", []) or []
            ),
            "auto_router_dependence_descriptor_policy": str(
                self.metadata.get("dependence_descriptor_policy", "") or ""
            ),
            "auto_router_dependence_descriptor_model_input_enabled": bool(
                self.metadata.get("dependence_descriptor_model_input_enabled", False)
            ),
            "auto_router_missingness_descriptor_policy": str(
                self.metadata.get("missingness_descriptor_policy", "") or ""
            ),
            "auto_router_missingness_descriptor_model_input_enabled": bool(
                self.metadata.get("missingness_descriptor_model_input_enabled", False)
            ),
            "auto_router_ood_defaulted": bool(self.metadata.get("auto_router_ood_defaulted", False)),
            "auto_router_ood_score": float(self.metadata.get("auto_router_ood_score", 0.0)),
            "auto_router_descriptor_ood_gate_available": bool(
                self.metadata.get("descriptor_ood_gate_available", False)
            ),
            "auto_router_descriptor_ood_gate_enabled": bool(
                self.metadata.get("descriptor_ood_gate_enabled", False)
            ),
            "auto_router_descriptor_ood_score": float(self.metadata.get("descriptor_ood_score", 0.0)),
            "auto_router_descriptor_ood_normalized_excess": float(
                self.metadata.get("descriptor_ood_normalized_excess", 0.0)
            ),
            "auto_router_descriptor_ood_max_zscore": float(
                self.metadata.get("descriptor_ood_max_zscore", 0.0)
            ),
            "auto_router_descriptor_ood_feature_count": int(
                self.metadata.get("descriptor_ood_outside_feature_count", 0)
            ),
            "auto_router_descriptor_ood_features": list(
                self.metadata.get("descriptor_ood_outside_features", []) or []
            ),
            ...
            "auto_router_crossfit_uncertainty_enabled": bool(
                self.metadata.get("crossfit_uncertainty_enabled", False)
            ),
            "auto_router_crossfit_uncertainty_applied": bool(
                self.metadata.get("crossfit_uncertainty_applied", False)
            ),
            "auto_router_crossfit_uncertainty_fallback_reason": str(
                self.metadata.get("crossfit_uncertainty_fallback_reason", "")
            ),
            "auto_router_crossfit_uncertainty_artifact_sha256": str(
                self.metadata.get("crossfit_uncertainty_artifact_sha256", "")
            ),
        }
...
class ScoreRouterConfig:
    ...
    descriptor_ood_gate_enabled: bool = False
    crossfit_uncertainty_enabled: bool = False
    cost_tie_break_margin: float = 0.0
...
class ScoreExpandedRouter:
    ...
    descriptor_ood_gate_: Optional[DescriptorOODGate] = field(default=None, init=False, repr=False)
    ...
    artifact_identity_: dict[str, Any] = field(default_factory=dict, init=False)
    crossfit_uncertainty_artifact_: Optional[CrossFitRouterUncertaintyArtifact] = field(
    ...
    def _dependence_descriptor_inputs_enabled(self) -> bool:
        return dependence_descriptor_model_input_enabled(
            self.training_metadata_,
            self.feature_names_,
        )

    def _missingness_descriptor_inputs_enabled(self) -> bool:
        return missingness_descriptor_model_input_enabled(
            self.training_metadata_,
            self.feature_names_,
        )
    ...
    def _descriptor_vector(self, descriptor: Mapping[str, Any]) -> np.ndarray:
        ...
        if not self._dependence_descriptor_inputs_enabled():
            dependence_keys = set(DEPENDENCE_DESCRIPTOR_KEYS)
            for idx, name in enumerate(self.feature_names_):
                if str(name) in dependence_keys:
                    raw[int(idx)] = 0.0
        if not self._missingness_descriptor_inputs_enabled():
            missingness_keys = set(MISSINGNESS_DESCRIPTOR_KEYS)
            for idx, name in enumerate(self.feature_names_):
                if str(name) in missingness_keys:
                    raw[int(idx)] = 0.0
    ...
    def _candidate_method_count(self, idx: int) -> int:
        candidate = self.candidates_[int(idx)]
        raw = candidate.get("method_count")
        try:
            count = int(raw)
        except Exception:
            count = 0
        if count <= 0:
            count = len(candidate.get("enabled_methods", []) or [])
        return int(max(count, 0))

    def _descriptor_with_legacy_inputs_zeroed(self, descriptor: Mapping[str, Any]) -> Mapping[str, Any]:
        if self._dependence_descriptor_inputs_enabled() and self._missingness_descriptor_inputs_enabled():
            return descriptor
        payload = dict(descriptor.get("feature_vector", descriptor))
        zero_keys: set[str] = set()
        if not self._dependence_descriptor_inputs_enabled():
            zero_keys.update(DEPENDENCE_DESCRIPTOR_KEYS)
        if not self._missingness_descriptor_inputs_enabled():
            zero_keys.update(MISSINGNESS_DESCRIPTOR_KEYS)
        if not any(str(name) in zero_keys for name in payload):
            return descriptor
    ...
    def _descriptor_ood_result(
        self,
        descriptor: Mapping[str, Any],
        *,
        descriptor_ood_gate_enabled: Optional[bool] = None,
    ) -> DescriptorOODResult:
        enabled = (
            bool(self.config.descriptor_ood_gate_enabled)
            if descriptor_ood_gate_enabled is None
            else bool(descriptor_ood_gate_enabled)
        )
        if self.descriptor_ood_gate_ is None:
            return DescriptorOODResult(gate_available=False, gate_enabled=False)
        return self.descriptor_ood_gate_.evaluate(
            self._descriptor_with_legacy_inputs_zeroed(descriptor),
            enabled=enabled,
        )
    ...
    def predict(
        ...
        *,
        descriptor_ood_gate_enabled: Optional[bool] = None,
        crossfit_uncertainty_enabled: Optional[bool] = None,
    ) -> AutoRouterOutput:
        if not self.fitted_:
            raise RuntimeError("ScoreExpandedRouter is not fitted.")
        uncertainty_enabled = (
            bool(self.config.crossfit_uncertainty_enabled)
            if crossfit_uncertainty_enabled is None
            else bool(crossfit_uncertainty_enabled)
        )
        ood_result = self._descriptor_ood_result(
            descriptor,
            descriptor_ood_gate_enabled=(
                bool(descriptor_ood_gate_enabled) or uncertainty_enabled
            ),
        )
        ...
        default_idx_raw = next(
            (idx for idx, candidate in enumerate(self.candidates_) if str(candidate.get("id")) == self.default_candidate_id_),
            None,
        )
        default_candidate_available = default_idx_raw is not None
        default_idx = int(default_idx_raw) if default_idx_raw is not None else top_idx
        default_margin = float(utility[top_idx] - utility[int(default_idx)])
        raw_top_idx = int(top_idx)
        cost_tie_break_applied = False
        cost_tie_break_raw_idx = int(top_idx)
        cost_tie_break_eligible: set[int] = set()
        guard_floor = max(float(self.config.min_utility_gain), float(self.config.decision_threshold))
        margin_policy_defaulted = bool(
            top_idx != int(default_idx)
            and (
                default_margin < guard_floor
                or float(beats_default_probability[top_idx]) < float(self.config.beats_default_probability_threshold)
            )
        )
        if ood_result.defaulted and not default_candidate_available:
            ood_result.defaulted = False
        ood_defaulted = bool(ood_result.defaulted)
        if margin_policy_defaulted or default_margin < float(self.config.min_utility_gain) or ood_defaulted:
            top_idx = int(default_idx)
            margin = max(0.0, float(utility[top_idx] - second_utility))
        else:
            cost_margin = float(max(0.0, self.config.cost_tie_break_margin))
            if cost_margin > 0.0 and len(self.candidates_) > 1:
                band_floor = float(utility[top_idx] - cost_margin)
                eligible = []
                for idx in range(len(self.candidates_)):
                    if float(utility[int(idx)]) < band_floor:
                        continue
                    candidate_default_margin = float(utility[int(idx)] - utility[int(default_idx)])
                    if int(idx) != int(default_idx) and candidate_default_margin < guard_floor:
                        continue
                    if (
                        int(idx) != int(default_idx)
                        and float(beats_default_probability[int(idx)])
                        < float(self.config.beats_default_probability_threshold)
                    ):
                        continue
                    eligible.append(int(idx))
                cost_tie_break_eligible = set(eligible)
                if eligible:
                    chosen_idx = min(
                        eligible,
                        key=lambda idx: (
                            self._candidate_method_count(int(idx)),
                            -float(utility[int(idx)]),
                            str(self.candidates_[int(idx)].get("id", "")),
                        ),
                    )
                    if int(chosen_idx) != int(top_idx):
                        cost_tie_break_applied = True
                        cost_tie_break_raw_idx = int(top_idx)
                        top_idx = int(chosen_idx)
                        remaining = [float(utility[int(i)]) for i in range(len(utility)) if int(i) != int(top_idx)]
                        next_best = max(remaining) if remaining else float(utility[top_idx])
                        margin = max(0.0, float(utility[top_idx] - next_best))
        uncertainty_reason = ""
        uncertainty_applied = False
        uncertainty_lcb: dict[int, float] = {}
        if uncertainty_enabled:
            artifact = self.crossfit_uncertainty_artifact_
            if artifact is None:
                uncertainty_reason = "uncertainty_artifact_unavailable"
            elif not default_candidate_available:
                uncertainty_reason = "default_candidate_unavailable"
            elif not (ood_result.gate_available and ood_result.gate_enabled):
                uncertainty_reason = "descriptor_ood_gate_unavailable_or_disabled"
            elif ood_result.defaulted:
                uncertainty_reason = "descriptor_ood"
            else:
                try:
                    artifact.validate_runtime(
                        base_router_sha256=str(self.artifact_identity_.get("artifact_sha256", "")),
                        candidate_ids=[str(item.get("id", "")) for item in self.candidates_],
                        descriptor_schema_sha256=router_descriptor_schema_sha256(self.feature_names_),
                    )
                    eligible: list[int] = []
                    for index, item in enumerate(self.candidates_):
                        if index == default_idx:
                            continue
                        permitted, reason, lower = artifact.decide(
                            candidate_id=str(item.get("id", "")),
                            predicted_delta_utility=float(raw_utility[index] - raw_utility[default_idx]),
                            beats_default_probability=float(beats_default_probability[index]),
                        )
                        uncertainty_lcb[index] = lower
                        if permitted:
                            eligible.append(index)
                        elif not uncertainty_reason:
                            uncertainty_reason = reason
                    if eligible:
                        top_idx = min(
                            eligible,
                            key=lambda index: (-float(uncertainty_lcb[index]), str(self.candidates_[index].get("id", ""))),
                        )
                        uncertainty_applied = True
                        uncertainty_reason = ""
                    else:
                        top_idx = default_idx
                        uncertainty_reason = uncertainty_reason or "no_candidate_passed_uncertainty"
                except RouterUncertaintyError as exc:
                    top_idx = default_idx
                    uncertainty_reason = str(exc)
            if uncertainty_reason:
                top_idx = default_idx
            policy_defaulted = bool(not uncertainty_applied)
            margin = float(uncertainty_lcb.get(top_idx, 0.0)) if uncertainty_applied else 0.0
        policy_defaulted = (
            bool(not uncertainty_applied)
            if uncertainty_enabled
            else bool(margin_policy_defaulted or ood_defaulted)
        )
        if ood_defaulted:
            confidence = 0.0
        elif margin_policy_defaulted:
            confidence = 1.0
        else:
            confidence = float(
        ...
        topk_indices = [int(idx) for idx in order[: min(5, len(order))]]
        if int(top_idx) not in topk_indices:
            topk_indices.append(int(top_idx))
        topk = []
            ...
            topk.append(
                {
                    ...
                    "cost_tie_break_eligible": bool(int(idx) in cost_tie_break_eligible),
                    "cost_tie_break_selected": bool(cost_tie_break_applied and int(idx) == int(top_idx)),
        ...
        return AutoRouterOutput(
            ...
            metadata={
                ...
                "artifact_identity": dict(self.artifact_identity_),
                "selected_candidate_id": str(candidate.get("id", "")),
                "raw_selected_candidate_id": str(self.candidates_[raw_top_idx].get("id", "")),
                "default_candidate_id": str(self.default_candidate_id_),
                "default_candidate_available": bool(default_candidate_available),
                ...
                "policy_margin_defaulted": bool(margin_policy_defaulted),
                "decision_threshold": float(self.config.decision_threshold),
                "cost_tie_break_margin": float(max(0.0, self.config.cost_tie_break_margin)),
                "cost_tie_break_applied": bool(cost_tie_break_applied),
                "cost_tie_break_raw_candidate_id": str(self.candidates_[cost_tie_break_raw_idx].get("id", ""))
                if cost_tie_break_applied
                else "",
                "cost_tie_break_selected_candidate_id": str(candidate.get("id", ""))
                if cost_tie_break_applied
                else "",
                "cost_tie_break_eligible_candidate_ids": [
                    str(self.candidates_[int(idx)].get("id", "")) for idx in sorted(cost_tie_break_eligible)
                ],
                "dependence_descriptor_policy": str(
                    self.training_metadata_.get("dependence_descriptor_policy", "") or ""
                ),
                "dependence_descriptor_model_input_enabled": bool(
                    self._dependence_descriptor_inputs_enabled()
                ),
                "missingness_descriptor_policy": str(
                    self.training_metadata_.get("missingness_descriptor_policy", "") or ""
                ),
                "missingness_descriptor_model_input_enabled": bool(
                    self._missingness_descriptor_inputs_enabled()
                ),
                "ranked_candidates": topk,
                "crossfit_uncertainty_enabled": bool(uncertainty_enabled),
                "crossfit_uncertainty_applied": bool(uncertainty_applied),
                "crossfit_uncertainty_fallback_reason": str(uncertainty_reason),
                "crossfit_uncertainty_artifact_sha256": (
                    self.crossfit_uncertainty_artifact_.artifact_sha256
                    if self.crossfit_uncertainty_artifact_ is not None else ""
                ),
                **ood_result.to_metadata(),
            },
        )
    ...
    def from_components(
        ...
        *,
        ...
        descriptor_ood_gate: Optional[Mapping[str, Any] | DescriptorOODGate] = None,
        config: Optional[ScoreRouterConfig] = None,
        metadata: Optional[Mapping[str, Any]] = None,
        artifact_identity: Optional[Mapping[str, Any]] = None,
        crossfit_uncertainty_artifact: Optional[
            Mapping[str, Any] | CrossFitRouterUncertaintyArtifact
        ] = None,
    ) -> "ScoreExpandedRouter":
        ...
        if isinstance(descriptor_ood_gate, DescriptorOODGate):
            router.descriptor_ood_gate_ = descriptor_ood_gate
        else:
            router.descriptor_ood_gate_ = DescriptorOODGate.from_manifest(descriptor_ood_gate)
        router.score_models_ = dict(score_models)
        router.training_metadata_ = dict(metadata or {})
        router.artifact_identity_ = dict(artifact_identity or {})
        if isinstance(crossfit_uncertainty_artifact, CrossFitRouterUncertaintyArtifact):
            router.crossfit_uncertainty_artifact_ = crossfit_uncertainty_artifact
        elif crossfit_uncertainty_artifact is not None:
            router.crossfit_uncertainty_artifact_ = CrossFitRouterUncertaintyArtifact.from_dict(
                crossfit_uncertainty_artifact
            )
        router.fitted_ = True
        return router
    ...
    def load(cls, model_dir: Path | str) -> "ScoreExpandedRouter":
        ...
        score_models_bytes = (path / "score_models.joblib").read_bytes()
        ...
        return cls.from_components(
            ...
            descriptor_ood_gate=manifest.get("descriptor_ood_gate"),
            score_models=joblib.load(io.BytesIO(score_models_bytes)),
            config=ScoreRouterConfig(**dict(manifest.get("config", {}) or {})),
            metadata=dict(manifest.get("training_metadata", {}) or {}),
            artifact_identity=_router_artifact_identity(
                path,
                manifest_bytes=manifest_bytes,
                score_models_bytes=score_models_bytes,
            ),
            crossfit_uncertainty_artifact=manifest.get("crossfit_uncertainty_artifact"),
        )
...
def load_default_auto_router(model_dir: Optional[Path | str] = None) -> ScoreExpandedRouter:
    """Construct a fresh router from the exact artifact bytes for each call."""

    path = Path(model_dir) if model_dir is not None else default_artifact_path()
    return ScoreExpandedRouter.load(path)
...
def predict_auto_router(
    ...
    *,
    ...
    return load_default_auto_router(model_dir).predict(
        descriptor,
        descriptor_ood_gate_enabled=bool(descriptor_ood_gate_enabled),
        crossfit_uncertainty_enabled=bool(crossfit_uncertainty_enabled),
    )
