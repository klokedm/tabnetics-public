...
- Deprecated validation exclusions from the migrated GitHub history record when needed
...
from tabnetics.core.paths import find_project_root, find_repo_root_or_none
...
CORE_PROJECT_ROOT = find_repo_root_or_none(__file__)
...
# The retired markdown tracker no longer exists in-repo. Its historical notes were
# migrated to GitHub issue #86, and the old parser was already effectively inert.
DEPRECATED_VALIDATION_EXCLUSIONS: Tuple[str, ...] = tuple()
__tabnetics_execution_isolated_state__ = {
    "DEPRECATED_VALIDATION_EXCLUSIONS": {
    ...
    "VAL21_HOST_CAPACITY_CORES": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "VAL21_HOST_WORKER_TARGETS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "VAL21_SPEC_TEMPLATE_PATH": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    "VALIDATION15_DATASETS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
    ...
    "VALIDATION18_SINGLETONS_PROFILE_IDS": {
        "provider": "clean_isolated_reference_v1",
        "dependencies": (),
    },
...
def _load_tabarena_registry() -> Tuple[Dict[str, Any], Dict[str, List[str]]]:
    try:
        ...
        repo_root_txt = str(_require_repo_root())
...
def _load_deprecated_method_sets(fs_method_sets: Sequence[str]) -> List[str]:
    """Return any deprecated method-set keys still excluded from validation."""
    fs_set = set(fs_method_sets)
    out = [t for t in DEPRECATED_VALIDATION_EXCLUSIONS if t in fs_set]
...
def build_jobs_validation1(*, dataset_shards: int = 8) -> List[Job]:
    ...
    the production baseline and to update the published results after aggregation.
    """
    fs_method_sets = _load_fs_method_sets()
...
# All FS methods for singleton sweep (sorted for reproducibility).
...
# Val-19 launches requested on public_cpu_host_1-public_cpu_host_4 only. TabPFN is not available on all
...
VAL18_CPU_HOST_CAPACITY_CORES: Dict[str, int] = {
    "host1.example.com": 64,
    "host2.example.com": 32,
    "host3.example.com": 64,
    "host4.example.com": 64,
}

VAL18_GPU_HOSTS: Tuple[str, ...] = ("public-gpu-host",)

VAL18_HOST_WORKER_TARGETS: Dict[str, Dict[str, int]] = {
    "host1.example.com": {"pods_per_host": 2, "max_workers_per_pod": 32, "target_total_workers": 64},
    "host2.example.com": {"pods_per_host": 1, "max_workers_per_pod": 32, "target_total_workers": 32},
    "host3.example.com": {"pods_per_host": 2, "max_workers_per_pod": 32, "target_total_workers": 64},
    "host4.example.com": {"pods_per_host": 2, "max_workers_per_pod": 32, "target_total_workers": 64},
    "public-gpu-host": {"pods_per_host": 2, "max_workers_per_pod": 10, "target_total_workers": 20},
}
...
VAL19_HOST_WORKER_TARGETS: Dict[str, Dict[str, int]] = {
    "host1.example.com": {"pods_per_host": 2, "max_workers_per_pod": 2, "target_total_workers": 4},
    "host2.example.com": {"pods_per_host": 1, "max_workers_per_pod": 4, "target_total_workers": 4},
    "host3.example.com": {"pods_per_host": 2, "max_workers_per_pod": 2, "target_total_workers": 4},
    "host4.example.com": {"pods_per_host": 2, "max_workers_per_pod": 2, "target_total_workers": 4},
}
...
VAL20_GPU_HOSTS: Tuple[str, ...] = ("public-gpu-host",)
...
VAL20_HOST_WORKER_TARGETS: Dict[str, Dict[str, int]] = {
    "host1.example.com": {"pods_per_host": 4, "max_workers_per_pod": 7, "target_total_workers": 28},
    "public-gpu-host": {"pods_per_host": 2, "max_workers_per_pod": 5, "target_total_workers": 10},
}
...
def _recommended_host_assignment(
    ...
    *,
    ...
    cpu_calibration_method_family: str = "cpu",
    gpu_calibration_method_family: str = "tabpfn",
    calibration_dataset_tier: str = "mixed",
) -> Dict[str, Any]:
    """Recommend a host mapping that balances CPU shard weight by host capacity."""
    resolved_worker_targets, worker_target_sources = merge_worker_targets_with_catalog(
        defaults=host_worker_targets,
        catalog_path=host_calibration_path,
        cpu_method_family=cpu_calibration_method_family,
        gpu_hosts=gpu_hosts,
        gpu_method_family=gpu_calibration_method_family,
        dataset_tier=calibration_dataset_tier,
    )
    ...
    for host in (*cpu_hosts, *gpu_hosts):
        ...
        host_summary[host] = {
            ...
            "worker_target": dict(resolved_worker_targets.get(host, {})),
            "worker_target_source": dict(worker_target_sources.get(host, {})),
        }
...
def _worker_targets_from_assignment(assignment: Mapping[str, Any]) -> Dict[str, Dict[str, int]]:
    out: Dict[str, Dict[str, int]] = {}
    for host, summary in dict(assignment.get("host_summary") or {}).items():
        target = dict(dict(summary).get("worker_target") or {})
        if target:
            out[str(host)] = {
                "pods_per_host": int(target.get("pods_per_host", 0) or 0),
                "max_workers_per_pod": int(target.get("max_workers_per_pod", 0) or 0),
                "target_total_workers": int(target.get("target_total_workers", 0) or 0),
            }
    return out
...
def _validation20_tabarena_recommended_host_assignment(
    ...
    *,
    ...
    return _recommended_host_assignment(
        ...
        host_worker_targets=VAL20_HOST_WORKER_TARGETS,
        host_calibration_path=host_calibration_path,
        cpu_calibration_method_family="tabarena",
    )
...
VALIDATION18_MNPO_PROFILE_MANIFEST: Dict[str, Dict[str, str]] = {
    ...
    "N43_core_banzhaf_no_worst_class": {"anchor": "no", "contrast_ref": "N04_core_banzhaf", "effect": "ablation_classifier_worst_class"},
    "N44_core_banzhaf_fs_stability": {"anchor": "no", "contrast_ref": "N31_core_banzhaf_perf_complexity", "effect": "fs_stability_oracle"},
...
def build_jobs_validation18_mnpo(
    *,
    ...
    """Val-18 Family N: MNPO/oracle profiles x 64 datasets x 5 seeds."""
    benchmark_datasets, _ = _load_benchmark_registry()
    if val17_root is None:
        val17_root = REPO_ROOT / "run_artifacts" / "validation-17"
    val17_runtime = _load_runtime_hints_from_summaries(val17_root, phase_tag="val17")
    ...
    # N43-N44: explicit T-R-391 signal diagnostics.
    n43_flags = (
        *_replace_flag_value(ref, "--classifier-selection-mode", "mnpo_hybrid"),
        "--classifier-oracle-k", "2",
        "--classifier-oracle-weighting-mode", "tritrust",
        "--classifier-oracle-bbc-bootstrap-rounds", "120",
        "--classifier-oracle-bbc-ci-level", "0.90",
    ...
    profiles: List[BenchmarkProfile] = [
        ...
        BenchmarkProfile("N43_core_banzhaf_no_worst_class", "mnpo_v14_core_plus_ipss", n43_flags,
                         notes="Val-18 N: T-R-391 ablation — disable classifier worst-class-recall oracle."),
        BenchmarkProfile("N44_core_banzhaf_fs_stability", "mnpo_v14_core_plus_ipss", n44_flags,
                         notes="Val-18 N: T-R-391 diagnostic — performance + complexity + FS-stability oracle."),
        # N16_core_meta_dt removed: meta-DT killed (CC-V17-2).
        # N23_broad_banzhaf_clp_payoff removed: CLP+payoff now in base ref (CC-V17-3/V17-4).
...
def _balanced_shard_assign_validation18_classifiers_bundles(
    ...
    """Keep CPU classifier work separate from TabPFN-only public-gpu-host lanes."""
    if int(num_shards) <= 1:
        return _balanced_shard_assign_profile_bundles(
            jobs, num_shards,
            run_family="val18_classifiers",
...
VALIDATION20_ORACLE_DIAGNOSTIC_PROFILE_MANIFEST: Dict[str, Dict[str, str]] = {
    "V20_O01_current_tritrust_diag24": {
        "anchor": "yes",
        "contrast_ref": "V20_O02_current_uniform_diag24",
        "effect": "expanded_cpu_tritrust_baseline",
        "panel": "diag24",
        "family": "O",
    },
    "V20_O02_current_uniform_diag24": {
        "anchor": "no",
        "contrast_ref": "V20_O01_current_tritrust_diag24",
        "effect": "expanded_cpu_uniform",
        "panel": "diag24",
        "family": "O",
    },
    "V20_O03_current_banzhaf_diag24": {
        "anchor": "no",
        "contrast_ref": "V20_O01_current_tritrust_diag24",
        "effect": "expanded_cpu_banzhaf",
        "panel": "diag24",
        "family": "O",
    },
    "V20_O04_current_shapley_diag24": {
        "anchor": "no",
        "contrast_ref": "V20_O01_current_tritrust_diag24",
        "effect": "expanded_cpu_shapley",
        "panel": "diag24",
        "family": "O",
    },
    "V20_O05_val18compat_tritrust_diag24": {
        "anchor": "no",
        "contrast_ref": "V20_O01_current_tritrust_diag24",
        "effect": "expanded_cpu_val18compat",
        "panel": "diag24",
        "family": "O",
    },
    "V20_O06_top3_no_diversity_diag24": {
        "anchor": "no",
        "contrast_ref": "V20_O01_current_tritrust_diag24",
        "effect": "expanded_cpu_top3_no_diversity",
        "panel": "diag24",
        "family": "O",
    },
}

VALIDATION20_EXTENSION_PROFILE_MANIFEST: Dict[str, Dict[str, str]] = {}
VALIDATION20_EXTENSION_PROFILE_MANIFEST.update(VALIDATION20_ENSEMBLE_PROFILE_MANIFEST)
VALIDATION20_EXTENSION_PROFILE_MANIFEST.update(VALIDATION20_ORACLE_DIAGNOSTIC_PROFILE_MANIFEST)
...
def build_jobs_validation20_ensemble(
    *,
    ...
    """Val-20 extension lane: Family E + classifier-MNPO diagnostics on DIAG24."""
    benchmark_datasets, _ = _load_benchmark_registry()
    if val17_root is None:
        val17_root = REPO_ROOT / "run_artifacts" / "validation-17"
    val17_runtime = _load_runtime_hints_from_summaries(val17_root, phase_tag="val17")
    ...
    ref = _val18_ref_flags()
    fs_method_set = "mnpo_v14_core_plus_ipss"
    expanded_cpu_pool = tuple(VAL20_EXPANDED_CLASSIFIER_POOL_NO_TABPFN)
    expanded_cpu_pool_max = str(len(expanded_cpu_pool))
    ...
    oracle_diag_base: Tuple[str, ...] = (
        *ref,
        ...
        "--classifier-oracle-bbc-ci-level", "0.90",
        "--enable-classifier-oracle-ensemble",
        "--model-candidates", *expanded_cpu_pool,
        "--model-cv-runtime-max-candidates", expanded_cpu_pool_max,
    )

    oracle_diag_job_params = {
        **cpu_job_params,
        "pool_snapshot_id": "current_expanded_cpu_no_tabpfn",
        "diagnostic_focus": "classifier_mnpo",
    }
        ...
        profiles.extend(
            [
                BenchmarkProfile(
                    ...
                    weight_mult=1.8,
                    notes=(
                        "Val-20 O01: expanded CPU-stable current pool baseline for classifier-MNPO diagnostics. "
                        "Uses tritrust/current behavior and the richer saved oracle matrices/payoff."
                    ),
                    job_params=oracle_diag_job_params,
                ),
                BenchmarkProfile(
                    "V20_O02_current_uniform_diag24",
                    fs_method_set,
                    (*oracle_diag_base, "--classifier-oracle-weighting-mode", "uniform"),
                    weight_mult=1.8,
                    notes=(
                        "Val-20 O02: uniform weighting control on the expanded CPU-stable current pool. "
                        "Tests whether the richer diagnostics show smart weighting adds signal beyond the raw oracle game."
                    ),
                    job_params=oracle_diag_job_params,
                ),
                BenchmarkProfile(
                    "V20_O03_current_banzhaf_diag24",
                    fs_method_set,
                    (*oracle_diag_base, "--classifier-oracle-weighting-mode", "banzhaf"),
                    weight_mult=1.9,
                    notes=(
                        "Val-20 O03: Banzhaf carry-over check on the expanded CPU-stable current pool. "
                        "Motivated by Val-18 Family W and the need to re-check weighting on the richer Val-19/20 pool."
                    ),
                    job_params=oracle_diag_job_params,
                ),
                BenchmarkProfile(
                    "V20_O04_current_shapley_diag24",
                    fs_method_set,
                    (*oracle_diag_base, "--classifier-oracle-weighting-mode", "shapley"),
                    weight_mult=2.0,
                    notes=(
                        "Val-20 O04: Shapley weighting sensitivity on the expanded CPU-stable current pool. "
                        "Useful mainly as a diagnostics-rich variance probe rather than a likely promotion candidate."
                    ),
                    job_params=oracle_diag_job_params,
                ),
                BenchmarkProfile(
                    "V20_O05_val18compat_tritrust_diag24",
                    fs_method_set,
                    (*oracle_diag_base, "--classifier-oracle-behavior-profile", "val18_compat"),
                    weight_mult=1.8,
                    notes=(
                        "Val-20 O05: behavior-profile carry-over check (`val18_compat`) on the expanded CPU-stable current pool. "
                        "Motivated by the Val-19 current-vs-compat bridge gap."
                    ),
                    job_params=oracle_diag_job_params,
                ),
                BenchmarkProfile(
                    "V20_O06_top3_no_diversity_diag24",
                    fs_method_set,
                    (*oracle_diag_base, "--classifier-oracle-k", "3"),
                    weight_mult=1.9,
                    notes=(
                        "Val-20 O06: isolate top-3 equilibrium spread without the portfolio-diversity extractor. "
                        "This distinguishes 'the Nash game is too concentrated' from 'the post-game diversity extractor is needed'."
                    ),
                    job_params=oracle_diag_job_params,
                ),
            ]
...
def _balanced_shard_assign_validation20_ensemble_bundles(
    ...
    """CPU-only shard assignment for the Val-20 extension lane."""
    return _balanced_shard_assign_profile_bundles(
        jobs, num_shards,
        run_family="val20_ensemble",
        profile_ids=VALIDATION20_EXTENSION_PROFILE_MANIFEST.keys(),
    )
...
def _write_work_split_md(
    ...
    *,
    ...
    if all_job_ids and all(str(jid).startswith("val20_tabarena_") for jid in all_job_ids):
        recommended_host_assignment = _validation20_tabarena_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
        host_order = tuple(VAL20_TABARENA_HOSTS)
    elif all_job_ids and all(str(jid).startswith("val20_") for jid in all_job_ids):
        recommended_host_assignment = _validation20_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
        ...
        recommended_host_assignment = _validation19_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
        ...
        recommended_host_assignment = _validation18_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
    ...
    if recommended_host_assignment:
        ...
        if host_summary:
            ...
            for host in host_order:
                ...
                worker_source = dict(summary.get("worker_target_source") or {})
                ...
                if str(worker_source.get("source") or "") == "host_calibration_catalog":
                    bits.append("source `host_calibration_catalog`")
...
VAL21_CPU_HOSTS: Tuple[str, ...] = VAL20_CPU_HOSTS
VAL21_GPU_HOSTS: Tuple[str, ...] = VAL20_GPU_HOSTS
VAL21_HOST_CAPACITY_CORES: Dict[str, int] = dict(VAL20_HOST_CAPACITY_CORES)
VAL21_HOST_WORKER_TARGETS: Dict[str, Dict[str, int]] = {
    str(host): dict(targets) for host, targets in VAL20_HOST_WORKER_TARGETS.items()
}
VAL21_RV_HOLDOUT: Tuple[str, ...] = (
    "rv_warpar10p",
    "rv_yale",
    "rv_coil20",
    "rv_relathe",
    ...
    "rv_basehock",
    "rv_usps",
    # Excluded: source data unavailable (MAT 404 / no real data path)
    # "rv_lung_discrete",
    # "rv_xena_tcga_thca",
    # "rv_xena_tcga_blca",
    # "rv_xena_tcga_cesc",
)
VAL21_SPEC_TEMPLATE_PATH: Path | None = (REPO_ROOT / "core" / "VAL21_SPEC.template.json") if REPO_ROOT is not None else None

VALIDATION21_PHASE1_PROFILE_MANIFEST: Dict[str, Dict[str, str]] = {
    "V21_WINNER": {
        "anchor": "yes",
        "contrast_ref": "V21_CURRENT_DEFAULT",
        "effect": "val20_winner_bridge",
        "panel": "full64",
        "family": "V21",
    },
    "V21_WINNER_LOCKOUT": {
        "anchor": "no",
        "contrast_ref": "V21_WINNER",
        "effect": "winner_plus_tier_lockout",
        "panel": "full64",
        "family": "V21",
    },
    "V21_WINNER_METASEL": {
        "anchor": "no",
        "contrast_ref": "V21_WINNER",
        "effect": "winner_plus_meta_selector",
        "panel": "full64",
        "family": "V21",
    },
    "V21_WINNER_REGIME": {
        "anchor": "no",
        "contrast_ref": "V21_WINNER",
        "effect": "winner_plus_regime_gating",
        "panel": "full64",
        "family": "V21",
    },
    "V21_WINNER_PREFILTER": {
        "anchor": "no",
        "contrast_ref": "V21_WINNER",
        "effect": "winner_plus_adaptive_prefilter",
        "panel": "full64",
        "family": "V21",
    },
    "V21_INTEGRATED": {
        "anchor": "no",
        "contrast_ref": "V21_WINNER",
        "effect": "winner_plus_all_integrations",
        "panel": "full64",
        "family": "V21",
    },
    "V21_CURRENT_DEFAULT": {
...
VALIDATION21_PLAN_KINDS: Tuple[str, ...] = (
    "validation21_phase1",
    "validation21_phase2",
)


def _is_validation21_placeholder(value: Any) -> bool:
    if isinstance(value, str):
        text = str(value).strip()
        return (not text) or (text.startswith("__") and text.endswith("__"))
    if isinstance(value, (list, tuple)):
        return any(_is_validation21_placeholder(item) for item in value)
    return False


def _load_validation21_spec(spec_path: Optional[Path] = None) -> Dict[str, Any]:
    path = VAL21_SPEC_TEMPLATE_PATH if spec_path is None else Path(spec_path)
    with path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    if not isinstance(payload, dict):
        raise ValueError(f"Validation21 spec must be a JSON object: {path}")
    return dict(payload)


def _resolve_validation21_spec(
    *,
    spec_path: Optional[Path] = None,
    phase: str,
) -> Dict[str, Any]:
    spec = _load_validation21_spec(spec_path)
    required = {
        "winner_profile_id": spec.get("winner_profile_id"),
        "winner_fs_method_set": spec.get("winner_fs_method_set"),
        "winner_execution_lane": spec.get("winner_execution_lane"),
        "winner_extra_args": spec.get("winner_extra_args"),
        "current_default_profile_id": spec.get("current_default_profile_id"),
        "current_default_fs_method_set": spec.get("current_default_fs_method_set"),
        "current_default_execution_lane": spec.get("current_default_execution_lane"),
        "current_default_extra_args": spec.get("current_default_extra_args"),
        "lockout_fallback_fs_method_set": spec.get("lockout_fallback_fs_method_set"),
        "meta_learning_records_path": spec.get("meta_learning_records_path"),
        "tier_classifier_model_path": spec.get("tier_classifier_model_path"),
    }
    if str(phase).strip().lower() == "phase2":
        required["phase2_profile_ids"] = spec.get("phase2_profile_ids")
    unresolved = [name for name, value in required.items() if _is_validation21_placeholder(value)]
    if unresolved:
        spec_hint = str((VAL21_SPEC_TEMPLATE_PATH if spec_path is None else Path(spec_path)).resolve())
        raise RuntimeError(
            "Validation21 spec has unresolved placeholders. Update "
            f"{spec_hint} after Val-20 completes. Missing fields: {', '.join(sorted(unresolved))}."
        )
    return spec


def _validation21_job_params(*, panel: str, execution_lane: str) -> Dict[str, Any]:
    lane = str(execution_lane or "cpu").strip().lower()
    if lane not in {"cpu", "tabpfn"}:
        raise ValueError(f"Unsupported Val-21 execution lane: {execution_lane!r}")
    return {
        "dataset_panel": str(panel),
        "execution_lane": lane,
        "preferred_hosts": list(VAL21_GPU_HOSTS if lane == "tabpfn" else VAL21_CPU_HOSTS),
    }


def _validation21_weight_mult(execution_lane: str) -> float:
    lane = str(execution_lane or "cpu").strip().lower()
    return 1.4 if lane == "tabpfn" else 1.0


def _build_validation21_phase1_profiles(spec: Dict[str, Any]) -> List[BenchmarkProfile]:
    winner_profile_id = str(spec.get("winner_profile_id") or "").strip()
    winner_fs_method_set = str(spec.get("winner_fs_method_set") or "").strip()
    winner_lane = str(spec.get("winner_execution_lane") or "").strip().lower()
    winner_args = tuple(str(arg) for arg in list(spec.get("winner_extra_args") or []))
    current_profile_id = str(spec.get("current_default_profile_id") or "").strip()
    current_fs_method_set = str(spec.get("current_default_fs_method_set") or "").strip()
    current_lane = str(spec.get("current_default_execution_lane") or "").strip().lower()
    current_args = tuple(str(arg) for arg in list(spec.get("current_default_extra_args") or []))
    lockout_fallback_set = str(spec.get("lockout_fallback_fs_method_set") or "").strip()
    meta_learning_records_path = str(spec.get("meta_learning_records_path") or "").strip()
    tier_classifier_model_path = str(spec.get("tier_classifier_model_path") or "").strip()

    winner_job_params = _validation21_job_params(panel="full64", execution_lane=winner_lane)
    current_job_params = _validation21_job_params(panel="full64", execution_lane=current_lane)

    profiles: List[BenchmarkProfile] = [
        BenchmarkProfile(
            "V21_WINNER",
            winner_fs_method_set,
            winner_args,
            weight_mult=_validation21_weight_mult(winner_lane),
            notes=f"Val-21 Phase 1 scaffold: resolved Val-20 winner profile {winner_profile_id}.",
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            "V21_WINNER_LOCKOUT",
            winner_fs_method_set,
            (
                *winner_args,
                ...
                "--tier-lockout-difficulty-source", "meta_features",
                "--tier-lockout-tier", "easy",
                "--tier-lockout-fallback-fs-method-set", lockout_fallback_set,
                "--tier-classifier-mode", "learned",
                "--tier-classifier-model-path", tier_classifier_model_path,
            ),
            weight_mult=_validation21_weight_mult(winner_lane),
            notes=(
                "Val-21 Phase 1 scaffold: winner profile plus learned tier-lockout. "
                f"Simplified fallback derived from {lockout_fallback_set}."
            ),
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            ...
            (
                *winner_args,
                "--meta-learning-selector", "decision_tree",
                "--meta-learning-records-path", meta_learning_records_path,
            ),
            weight_mult=_validation21_weight_mult(winner_lane),
            notes=(
                "Val-21 Phase 1 scaffold: winner profile plus composite meta-learning selector. "
                f"Records artifact: {meta_learning_records_path}."
            ),
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            ...
            (
                *winner_args,
                "--regime-gating-enabled",
                "--regime-gating-difficulty-source", "meta_features",
                "--regime-gating-min-samples-per-class", "7.0",
                "--regime-gating-use-expanded-features",
                "--regime-gating-min-fisher-f1", "0.10",
                "--regime-gating-max-n1-borderline", "0.40",
            ),
            weight_mult=_validation21_weight_mult(winner_lane),
            notes="Val-21 Phase 1 scaffold: winner profile plus data-driven regime gating.",
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            "V21_WINNER_PREFILTER",
            ...
            (
                *winner_args,
                ...
                "--prefilter-adaptive-top-k-scaling", "0.5",
            ),
            weight_mult=_validation21_weight_mult(winner_lane),
            notes="Val-21 Phase 1 scaffold: winner profile plus adaptive prefilter top-k.",
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            "V21_INTEGRATED",
            ...
            weight_mult=_validation21_weight_mult(winner_lane),
            notes=(
                "Val-21 Phase 1 scaffold: winner profile plus lockout, meta-selector, "
                "regime gating, and adaptive prefilter."
            ),
            job_params=winner_job_params,
        ),
        BenchmarkProfile(
            "V21_CURRENT_DEFAULT",
            current_fs_method_set,
            current_args,
            weight_mult=_validation21_weight_mult(current_lane),
            notes=f"Val-21 Phase 1 control scaffold: current default profile {current_profile_id}.",
            job_params=current_job_params,
        ),
    ]
    _val18_validate_method_sets(profiles, "validation21_phase1")
    return profiles


def build_jobs_validation21_phase1(
    *,
    dataset_shards: int = 8,
    val17_root: Optional[Path] = None,
    spec_path: Optional[Path] = None,
    ...
    if val17_root is None:
        val17_root = REPO_ROOT / "run_artifacts" / "validation-17"
    val17_runtime = _load_runtime_hints_from_summaries(val17_root, phase_tag="val17")
    runtime_est = _val18_runtime_estimate(list(VAL18_FULL64), benchmark_datasets, val17_runtime)
    profiles = _build_validation21_phase1_profiles(
        _resolve_validation21_spec(spec_path=spec_path, phase="phase1")
    )
    return _val18_build_jobs(
        run_family="val21_phase1",
        profiles=profiles,
        dataset_ids=list(VAL18_FULL64),
        seeds=[11, 23, 37, 42, 59],
        dataset_shards=dataset_shards,
        runtime_est=runtime_est,
    )
...
def build_jobs_validation21_phase2(
    *,
    dataset_shards: int = 4,
    val17_root: Optional[Path] = None,
    spec_path: Optional[Path] = None,
    ...
    if val17_root is None:
        val17_root = REPO_ROOT / "run_artifacts" / "validation-17"
    val17_runtime = _load_runtime_hints_from_summaries(val17_root, phase_tag="val17")
    runtime_est = _val18_runtime_estimate(list(VAL21_RV_HOLDOUT), benchmark_datasets, val17_runtime)
    spec = _resolve_validation21_spec(spec_path=spec_path, phase="phase2")
    phase1_profiles = {
        profile.profile_id: profile
        for profile in _build_validation21_phase1_profiles(spec)
    }
    selected_profile_ids = [
        str(profile_id).strip()
        for profile_id in list(spec.get("phase2_profile_ids") or [])
        if str(profile_id).strip()
    ]
    profiles: List[BenchmarkProfile] = []
    missing_profile_ids: List[str] = []
    for profile_id in selected_profile_ids:
        profile = phase1_profiles.get(profile_id)
        if profile is None:
    ...
    if missing_profile_ids:
        raise RuntimeError(
            "Validation21 Phase 2 profile selection references unknown Phase 1 IDs: "
            + ", ".join(sorted(missing_profile_ids))
        )
    if not profiles:
        raise RuntimeError("Validation21 Phase 2 requires at least one resolved profile ID.")
    return _val18_build_jobs(
        run_family="val21_phase2",
        profiles=profiles,
        dataset_ids=list(VAL21_RV_HOLDOUT),
        seeds=[11, 23, 37, 42, 59],
        dataset_shards=dataset_shards,
        runtime_est=runtime_est,
    )
...
def _balanced_shard_assign_validation21_phase1_bundles(
    jobs: Sequence[Job], num_shards: int
) -> Dict[int, List[str]]:
    return _balanced_shard_assign_profile_bundles(
        jobs,
        num_shards,
        run_family="val21_phase1",
        profile_ids=VALIDATION21_PHASE1_PROFILE_MANIFEST.keys(),
    )


def _balanced_shard_assign_validation21_phase2_bundles(
    jobs: Sequence[Job], num_shards: int
) -> Dict[int, List[str]]:
    phase1_ids = tuple(VALIDATION21_PHASE1_PROFILE_MANIFEST.keys())
    profile_ids = tuple(
        str(job.job_id).split("/")[1]
        for job in jobs
        if len(str(job.job_id).split("/")) == 3 and str(job.job_id).startswith("val21_phase2/")
    ) or phase1_ids
    return _balanced_shard_assign_profile_bundles(
        jobs,
        num_shards,
...
def _validation21_recommended_host_assignment(
    shards: Dict[int, List[str]],
    jobs_by_id: Dict[str, Job],
    *,
    host_calibration_path: Path | None = None,
    ...
    return _recommended_host_assignment(
        shards,
        jobs_by_id,
        cpu_hosts=VAL21_CPU_HOSTS,
        gpu_hosts=VAL21_GPU_HOSTS,
        cpu_host_capacity_cores=VAL21_HOST_CAPACITY_CORES,
        host_worker_targets=VAL21_HOST_WORKER_TARGETS,
        host_calibration_path=host_calibration_path,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Plan kind: validation20_ensemble (Val-20 standalone extension lane: "
            "Family-E ensemble/oracle enhancements plus Family-O classifier-MNPO "
            "diagnostics on the expanded CPU-stable current pool); "
            ...
            "Plan kind: validation21_phase1/validation21_phase2 (Val-21 post-Val-20 "
            "meta-feature calibration scaffold. Requires a resolved --val21-spec JSON; "
            "the bundled template intentionally fails until winner/control placeholders are filled); "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "validation20_tabarena_w2/validation20_tabarena_w3/validation21_phase1/"
            "validation21_phase2)."
        ),
    )
    parser.add_argument(
        "--val21-spec",
        type=str,
        default=str(VAL21_SPEC_TEMPLATE_PATH),
        help=(
            "(validation21_phase1/validation21_phase2) Path to the resolved Val-21 spec JSON. "
            "The default template is intentionally placeholder-only until Val-20 results land."
        ),
    )
    parser.add_argument(
        "--val4-root",
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional host calibration JSON. Defaults to "
            "run_artifacts/meta/host_calibration.json when present; live nproc/memory "
            "checks remain authoritative for final run specs."
        ),
    )
    ...
    raw_host_calibration = str(getattr(args, "host_calibration_catalog", "") or "").strip()
    if raw_host_calibration:
        host_calibration_path: Path | None = resolve_host_calibration_path(
            raw_host_calibration, repo_root=_require_repo_root(),
        )
    else:
        default_calibration_path = default_host_calibration_path(_require_repo_root())
        host_calibration_path = default_calibration_path if default_calibration_path.exists() else None
    ...
    if (
        ...
        or plan_kind in set(VALIDATION21_PLAN_KINDS)
    ...
    elif plan_kind == "validation21_phase1":
        val17_root_raw = str(getattr(args, "val17_root", "") or "").strip()
        val17_root = Path(val17_root_raw).expanduser().resolve() if val17_root_raw else None
        val21_spec_raw = str(getattr(args, "val21_spec", "") or "").strip()
        val21_spec = Path(val21_spec_raw).expanduser().resolve() if val21_spec_raw else None
        jobs = build_jobs_validation21_phase1(
            dataset_shards=int(getattr(args, "dataset_shards", 8) or 8),
            val17_root=val17_root,
            spec_path=val21_spec,
        )
    elif plan_kind == "validation21_phase2":
        val17_root_raw = str(getattr(args, "val17_root", "") or "").strip()
        val17_root = Path(val17_root_raw).expanduser().resolve() if val17_root_raw else None
        val21_spec_raw = str(getattr(args, "val21_spec", "") or "").strip()
        val21_spec = Path(val21_spec_raw).expanduser().resolve() if val21_spec_raw else None
    ...
    if plan_kind == "validation5":
        ...
        shards = _balanced_shard_assign_validation21_phase1_bundles(jobs, num_pods)
    elif plan_kind == "validation21_phase2":
        shards = _balanced_shard_assign_validation21_phase2_bundles(jobs, num_pods)
    else:
        shards = _balanced_shard_assign(jobs, num_pods)
    jobs_by_id = {j.job_id: j for j in jobs}

    validation18_assignment = (
        _validation18_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
        if str(plan_kind).startswith("validation18_")
        else {}
    )
    validation19_assignment = (
    ...
    validation20_tabarena_assignment = (
    ...
    validation21_assignment = (
        _validation21_recommended_host_assignment(
            shards, jobs_by_id, host_calibration_path=host_calibration_path,
        )
        if plan_kind in set(VALIDATION21_PLAN_KINDS)
        else {}
    )
    ...
    plan = {
        ...
        "notes": {
            ...
            "output_tag": safe_tag,
            "host_calibration_catalog": str(host_calibration_path or ""),
            "host_calibration_policy": (
                "Advisory prior only; live nproc and memory-headroom checks remain authoritative."
            ),
            ...
            "validation18_host_policy": (
                {
                    ...
                    "host_worker_targets": _worker_targets_from_assignment(validation18_assignment),
                    "recommended_host_assignment": validation18_assignment,
                }
                if str(plan_kind).startswith("validation18_")
                else {}
            ),
            ...
            "validation19_host_policy": (
                {
                    ...
                    "tabpfn_hosts": [],
                    "cpu_host_capacity_cores": dict(VAL18_CPU_HOST_CAPACITY_CORES),
                    "host_worker_targets": _worker_targets_from_assignment(validation19_assignment),
                    "recommended_host_assignment": validation19_assignment,
                }
                if plan_kind == "validation19_classifiers"
                else {}
            ),
            ...
            "validation20_host_policy": (
                {
                    ...
                    "host_worker_targets": _worker_targets_from_assignment(validation20_assignment),
                    "recommended_host_assignment": validation20_assignment,
                }
                if plan_kind in {"validation20_wave1", "validation20_ensemble"}
                else {}
            ),
            "validation20_tabarena_host_policy": (
                {
                    ...
                    "cpu_only": True,
                    "cpu_host_capacity_cores": dict(VAL20_HOST_CAPACITY_CORES),
                    "host_worker_targets": _worker_targets_from_assignment(validation20_tabarena_assignment),
                    "recommended_host_assignment": validation20_tabarena_assignment,
                }
                if plan_kind in VALIDATION20_TABARENA_PLAN_KINDS
                else {}
            ),
            "validation21_profile_manifest": (
                VALIDATION21_PHASE1_PROFILE_MANIFEST
                if plan_kind in set(VALIDATION21_PLAN_KINDS)
                else {}
            ),
            "validation21_spec_path": (
                str(Path(getattr(args, "val21_spec", "") or "").expanduser().resolve())
                if plan_kind in set(VALIDATION21_PLAN_KINDS)
                and str(getattr(args, "val21_spec", "") or "").strip()
                else ""
            ),
            "validation21_dataset_panel": (
                "full64"
                if plan_kind == "validation21_phase1"
                else "rv_holdout"
                if plan_kind == "validation21_phase2"
                else ""
            ),
            "validation21_host_policy": (
                {
                    "available_hosts": [*VAL21_CPU_HOSTS, *VAL21_GPU_HOSTS],
                    "cpu_hosts": list(VAL21_CPU_HOSTS),
                    "tabpfn_hosts": list(VAL21_GPU_HOSTS),
                    "cpu_host_capacity_cores": dict(VAL21_HOST_CAPACITY_CORES),
                    "host_worker_targets": _worker_targets_from_assignment(validation21_assignment),
                    "recommended_host_assignment": validation21_assignment,
                }
                if plan_kind in set(VALIDATION21_PLAN_KINDS)
                else {}
            ),
    ...
    shard_doc = {
        ...
        "validation21_recommended_host_assignment": (
            validation21_assignment
            if plan_kind in set(VALIDATION21_PLAN_KINDS)
            else {}
        ),
