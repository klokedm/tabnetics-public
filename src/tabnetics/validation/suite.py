...
This module implements the datasets listed in the validation guide and exposes:
...
Operational policy in the packaged public surface:
- validation-catalog datasets are expected to load from the authoritative
  HuggingFace mirror of the public upstream sources when configured
- the CLI defaults to ``--no-synthetic-fallback`` and
  ``--dataset-integrity-policy error`` for evidence-bearing runs
- if synthetic fallback is explicitly re-enabled for non-authoritative
  workflows, the chosen data source is still recorded in the run metadata
"""
...
from typing import Any, Callable, Dict, Iterable, List, Optional, Sequence, Tuple, TYPE_CHECKING, TypeVar
...
from tabnetics.validation.core.provenance import build_code_provenance

if TYPE_CHECKING:
    from tabnetics.pipeline.resampling import FitResamplingContext, ResolvedSplitPlan
...
class LoadedTabularDataset:
    ...
    modality_membership: Optional[Dict[str, Tuple[int, ...]]] = None
...
COMPONENT_DEFAULTS: Dict[str, bool] = {
    ...
    # Listed in the validation guide protocol. `df.interval_likelihood` and
...
def _load_hf_parquet_dataset(
    ...
    *,
    ...
    # Token-free path: when TABNETICS_HF_LOCAL_PARQUET points at a cached HF snapshot
    # data dir, read the dataset's parquet directly (bit-identical bytes, no private-repo
    # auth/metadata call). Unset -> identical to the prior behaviour.
    import os as _os
    import glob as _glob
    _local = _os.environ.get("TABNETICS_HF_LOCAL_PARQUET", "").strip()
    if _local:
        _files = sorted(_glob.glob(_os.path.join(_local, f"{dataset_id}-*.parquet")))
        if not _files:
            raise RuntimeError(
                f"TABNETICS_HF_LOCAL_PARQUET={_local} set but no parquet matched '{dataset_id}-*.parquet'"
            )
        import pandas as _pd
        import numpy as _np
        _df = _pd.concat([_pd.read_parquet(_f) for _f in _files], ignore_index=True)
        if "features" not in _df.columns or "label" not in _df.columns:
            raise RuntimeError(
                f"local parquet '{dataset_id}' missing features/label; got {list(_df.columns)}"
            )
        _X = _np.array(_df["features"].tolist(), dtype=_np.float32)
        _y = _safe_label_encode(_np.array(_df["label"].tolist(), dtype=_np.int64))
        # Bit-identical to the HF bundle bytes (read from the cached snapshot), so it
        # carries the authoritative hf_bundle source label; the local read is recorded in notes.
        return LoadedTabularDataset(
            X=_X,
            y=_y,
            data_source=f"hf_bundle:{repo_id}/{dataset_id}",
            notes=f"loaded_from_hf_bundle:config={dataset_id},split=train,via=local_parquet_cache",
        )
    try:
...
def _resolve_train_test_split(
    ...
    *,
    ...
    train_idx = np.asarray(plan.primary.train_indices, dtype=int)
    test_idx = np.asarray(plan.primary.test_indices, dtype=int)
    return (
...
def _safe_train_test_split(
    ...
    """Compatibility wrapper returning only the four legacy array partitions."""

    X_train, X_test, y_train, y_test, _, _ = _resolve_train_test_split(
        X,
        y,
        test_size,
        seed,
...
def _select_fs_subsample(
    ...
    *,
    ...
    if fs_fraction >= 0.999:
        fs_idx = np.arange(int(X_train.shape[0]), dtype=int)
        return (X_train, y_train, fs_idx) if return_indices else (X_train, y_train)
    ...
    if (
        isinstance(resampling_context, FitResamplingContext)
        and resampling_context.policy.kind != "iid"
    ):
        fs_idx = np.asarray(
            resolve_fit_subsample(
                resampling_context,
                np.asarray(y_train).ravel(),
                fraction=fs_fraction,
                seed=int(seed),
                balanced=True,
                min_per_class=2,
            ),
    ...
    elif len(classes) < 2 or counts.min() < 2:
    ...
    selected = (X_train[fs_idx], y_train[fs_idx])
    return (*selected, fs_idx) if return_indices else selected
...
def _run_fs_core(
    ...
    fit_context = ensure_fit_resampling_context(
        ...
        n_rows=int(np.asarray(y_train).size),
    )
    X_fs, y_fs, fs_idx = _select_fs_subsample(
        X_train_proc,
        y_train,
        fs_fraction=fs_fraction,
        seed=seed,
        resampling_context=fit_context,
        return_indices=True,
    )
    fs_context = fit_context.take(fs_idx)
    ...
    if fs_context.policy.kind != "iid":
        def split_plan_provider(**request: Any) -> ResolvedSplitPlan:
            return resolve_cv(
                fs_context,
                request["y"],
    ...
    if fit_context.policy.kind != "iid":
        _, class_counts = np.unique(np.asarray(y_train).ravel(), return_counts=True)
        if class_counts.size and int(class_counts.min()) >= 2:
            classifier_cv_plan = resolve_cv(
                fit_context,
                np.asarray(y_train).ravel(),
                n_splits=int(max(2, min(5, class_counts.min()))),
                seed=int(seed),
                purpose="validation_classifier_cv",
                n_repeats=1,
                stratified=True,
                shuffle=True,
            )
    model, model_name = _select_model_via_cv(
        X_train_sel,
        y_train,
        seed,
        cv_plan=classifier_cv_plan,
    )
    ...
    return {
        ...
        "fs_context_fingerprint": str(fs_context.fingerprint),
        "resampling_policy": fit_context.policy.to_record(),
        "feature_selection_resampling": dict(
            getattr(selector, "resampling_diagnostics_", {}) or {}
        ),
        "classifier_cv_plan": (
            {} if classifier_cv_plan is None else classifier_cv_plan.to_metadata()
        ),
    }
...
def run_validation_suite(args: argparse.Namespace) -> Path:
        ...
        if spec.pipeline == "fs":
            ...
            for seed in args.seeds:
                try:
                    ...
                    fit_resampling_context = full_resampling_context.take(
                        outer_split_plan.primary.train_indices,
                        ...
                        metrics.update(
                            {
                                "resampling_context_fingerprint": str(
                                    full_resampling_context.fingerprint
                                ),
                                "outer_split_fingerprint": str(
                                    outer_split_plan.primary.fingerprint
                                ),
                                "outer_leakage_audit": (
                                    outer_split_plan.primary.audit.to_dict()
                                ),
                                "pipeline": "fs",
                                "dataset_id": ds_id,
    ...
    metadata = {
        ...
        "code_provenance": build_code_provenance(
            ...
            command=sys.argv,
            extra_paths={"metadata": metadata_path, "dataset_manifest": datasets_path},
        ),
    }
