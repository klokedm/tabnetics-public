...
For the broadest ready-to-run setup used across this guide, install:

```bash
pip install "tabnetics[feature-selection-optional,benchmarks]"
```

That enables the expanded feature-selection and benchmark/backend library set immediately. If you only want the minimal core package, use `pip install tabnetics` instead. Optional integrations still keep their own upstream licenses/terms; see [Third-party integrations and licenses](#third-party-integrations-and-licenses).
