...
Tabnetics itself is licensed under Apache 2.0, but several optional features call into separate upstream projects. Enabling those integrations does not relicense them: you still need to comply with the upstream license or use terms for each library you install or activate.

This table covers the direct optional integrations surfaced by the current public docs/code paths. It is not a full transitive dependency manifest for every extra or wheel dependency; for redistribution or commercial deployment, review the exact upstream license text for the versions you ship.

| Library | Used for in Tabnetics | Surface | Upstream license / terms | Notes |
|---|---|---|---|---|
| [Boruta](https://github.com/danielhomola/boruta_py) (`boruta`) | Boruta feature selection | `feature-selection-optional`, `benchmarks`; `boruta` method | BSD-3-Clause | Optional selector backend. |
| [SHAP](https://pypi.org/project/shap/) (`shap`) | TreeSHAP values for the `treeshap` selector when installed manually | Manual optional install; `treeshap` method | MIT | If `shap` is absent, Tabnetics falls back to model-importance-based scoring for that path. |
| [pyvinecopulib](https://pypi.org/project/pyvinecopulib/) (`pyvinecopulib`) | Vine-copula knockoff generation | `feature-selection-optional`, `benchmarks`; `copula_knockoff` methods | MIT | Required for the packaged copula knockoff backend. |
| [MAPIE](https://github.com/scikit-learn-contrib/MAPIE) (`mapie`) | Conformal prediction / uncertainty wrappers | `feature-selection-optional`, `benchmarks`; conformal classifier outputs | BSD-3-Clause | Optional uncertainty layer, not a point-accuracy optimizer. |
| [datasets](https://github.com/huggingface/datasets) (`datasets`) | HuggingFace dataset bundle loading for benchmark/validation workflows | `benchmarks`, `validation` | Apache-2.0 | Used for the public validation/benchmark bundle path. |
| [FLAML](https://github.com/microsoft/FLAML) (`flaml`) | AutoML classifier backend/oracle candidates | `benchmarks`; `classification_backend="flaml"` | MIT | Benchmark/backend extra only. |
| [LightGBM](https://github.com/microsoft/LightGBM) (`lightgbm`) | Gradient-boosted tree classifier candidate | `benchmarks` | MIT | Optional benchmark classifier backend. |
| [XGBoost](https://pypi.org/project/xgboost/) (`xgboost`) | Gradient-boosted tree classifier candidate | `benchmarks` | Apache-2.0 | Optional benchmark classifier backend. |
| [CatBoost](https://catboost.ai/) (`catboost`) | Gradient-boosted tree classifier candidate | `benchmarks` | Apache-2.0 | Optional benchmark classifier backend. |
| [TabPFN](https://pypi.org/project/TabPFN/) (`tabpfn`) | Foundation-model classifier candidate | `benchmarks`; `tabpfn` classifier path | Prior Labs License (Apache 2.0 with additional attribution) for code and TabPFN-2 weights; default TabPFN-2.5 weights are non-commercial | Review upstream terms carefully before commercial use, redistribution, or product deployment. |
...
\*Optional: requires `pip install pytabkit` (PyTorch). Graceful fallback when not installed.
...
| `general` | LR, SVM-RBF, RF, KNN, elastic-net, NB, XGBoost, LightGBM | CDF off, MNPO hybrid selection, 10-candidate cap | Moderate general tabular |
| `general_full` (default) | 16 classifiers (full surface excl. TabPFN) | CDF off, no runtime cap, tritrust oracle k=3, ensemble, post-Val-17 FS defaults | Broad general tabular |
| `general_tabular` | 11 tree-weighted classifiers (RF, ExtraTrees, XGBoost, LightGBM, CatBoost, LR, elastic-net, SVM-linear, SVM-RBF, KNN, NB) | CDF off, no screening/folding, adaptive FS fraction by N/p ratio, Banzhaf classifier oracle k=3, no HDLSS machinery | N >> p datasets (many samples, few features) |
...
Prefilter is only activated when `p > 200` (with BH correction disabled per Val-18 P03 evidence). Both FS and classifier oracles use Banzhaf weighting (Val-18 N04 evidence). Classifier-side conformal uses APS (per Val-18 C07 evidence). Screening and folding are always off (these are HDLSS-specific). HDLSS-oriented classifiers (GPC, PLS-DA, NSC, vote\_ensemble, shrinkage\_LDA) are excluded in favour of tree ensembles.
