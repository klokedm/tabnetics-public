...
Training-set balancing is a separate opt-in extra:

```bash
pip install "tabnetics[balancing]"
```
...
- [Training-class balancing](#training-class-balancing)
- [Fitted estimator and bundles](#fitted-estimator-and-bundles)
...
This default config keeps the packaged V25 auto-router disabled. Set
`auto_router_enabled=True` only for a controlled router evaluation; when
enabled, it computes a training-only dataset descriptor, selects a supported
method/config candidate, then runs the delegated pipeline with the router
...
- `auto_router_enabled=False` is the default; the V25 calibrated score-router is available only when explicitly enabled.
...
Tabnetics exposes five deterministic training-balance modes through
`TrainingBalanceConfig`: `none` (the default), `smote`, `propensity_match`,
`random_over`, and `random_under`. Enabled balancing is applied independently
to each Stage-2 classifier-CV training fold and to the final classifier's
training rows. Validation and test rows are never resampled and retain their
natural class prevalence.

```python
from tabnetics.pipeline import DFFSConfig, TrainingBalanceConfig

config = DFFSConfig(
    random_seed=42,
    training_balance=TrainingBalanceConfig(
        method="random_under",
        random_state=42,
    ),
)
...
The bounded random methods use `sampling_strategy="auto"`:

- `random_over` duplicates minority rows without geometric interpolation. It
  rejects sample weights and does not retain an executable output row context,
  because duplicated source identifiers would no longer be unique.
- `random_under` samples without replacement and subsets row context and sample
  weights together. It may discard useful majority-class variation.
- `smote` synthesizes continuous feature rows and rejects sample weights.
- `propensity_match` is binary predictive-class matching on cross-fitted model
  scores. It is not a causal estimator and does not estimate a treatment effect.

All enabled modes currently require dense, finite, continuous features and an
IID or stratified resampling policy. They fail closed for rare or already
balanced class configurations that make the requested operation inapplicable;
Tabnetics never silently falls back to another balancing method. These methods
are opt-in candidates, not promoted defaults or performance guarantees.

## Fitted estimator and bundles

Use `DFFSClassifier` when you need a sklearn-compatible fitted object rather
than an evaluation result. Its `fit()` path resolves feature-selection and
classifier policy through training-only CV, then refits the chosen model on all
supplied rows. It does not create a holdout or report held-out metrics.

```python
from tabnetics.pipeline import DFFSClassifier, DFFSConfig
...
labels = estimator.predict(X_new)
probabilities = estimator.predict_proba(X_new)
selected_names = estimator.get_feature_names_out()
```

For DataFrame inputs, strict inference rejects missing, extra, reordered, and
dtype-incompatible columns. Set `feature_alignment="reorder"` only when the
same named, type-compatible columns have been reviewed and need an explicit
order restoration. The most recent compatibility decision is recorded in
`last_inference_schema_report_`.

`to_safe_bundle()` produces a versioned JSON-only bundle only for its current
allowlisted route: numeric input, median imputation, standard scaling,
positional feature selection, and sklearn `LogisticRegression`. Load it with
`load_safe_dffs_bundle()`. Other fitted routes, including generic/custom
estimators, Tabnetics Diakrino/native models, typed preprocessors, distribution transforms,
batch correction, folding, and ratio features fail closed rather than being
described as safe interchange.

```python
from tabnetics.pipeline import load_safe_dffs_bundle

safe_bundle = estimator.to_safe_bundle()
portable_model = load_safe_dffs_bundle(safe_bundle)
labels = portable_model.predict(X_new)
```

The older `DFFSReproducibleModel` bundle is a trusted-runtime compatibility
format containing arbitrary pickle payloads. It is never loaded by default:

```python
from tabnetics.pipeline.pipeline import load_df_fs_model_bundle

legacy = load_df_fs_model_bundle(
    "trusted-local-bundle.json",
    trusted_legacy_pickle=True,
)
```

Only use `trusted_legacy_pickle=True` for an artifact whose origin and contents
you explicitly trust.

## Auto router

The auto-router is an experimental opt-in entry point. It selects among supported candidates using descriptors computed directly from the training data, but remains disabled until frozen registered-holdout validation supports promotion.
...
The default keeps explicit/manual configuration:
...
This CSV example uses the fixed manual defaults. Add `auto_router_enabled=True` only when explicitly evaluating the packaged router.
