(function () {
  ...
  function campaignScopeLabel(value) {
    const map = {
      full: "Full benchmark panel",
      diagnostic: "Diagnostic subset",
      targeted: "Targeted slice",
    };
    return map[readText(value)] || readText(value) || "Unknown";
  }
  ...
  function inflateBenchmarkPayload(payload) {
    return {
      ...
      autoRouter: inflateAutoRouterPayload(payload.auto_router || {}),
  ...
  function inflateAutoRouterPayload(payload) {
    return {
      metadata: payload.metadata || { available: false },
      metrics: unpackTable(payload.metrics),
      datasetResults: unpackTable(payload.dataset_results),
      candidates: unpackTable(payload.candidates),
    };
  }
  ...
  function initializeBenchmarkFilters() {
    ...
    populateSelect("tn-benchmark-campaign-scope", filters.campaign_scopes || [], campaignScopeLabel);
    ...
    datasetProfiles.forEach((row) => {
      ...
      if (!existing) {
        groups.set(key, {
          ...
          campaign_scope_label: row.campaign_scope_label,
  ...
  function filterBenchmarkData() {
    ...
    const datasetProfiles = state.benchmark.datasetProfiles.filter((row) => {
      ...
      if (f.campaignScope !== "all" && row.campaign_scope !== f.campaignScope) {
        return false;
      }
    ...
    const allowedCells = new Set(
      datasetProfiles.map((row) => `${row.dataset_id}\u0001${row.profile}\u0001${row.campaign}\u0001${row.campaign_scope}`)
    );
    const runs = state.benchmark.runs.filter(
      (row) =>
        allowedCells.has(`${row.dataset_id}\u0001${row.profile}\u0001${row.campaign}\u0001${row.campaign_scope}`)
    );
  ...
  function benchmarkTableColumns(mode) {
    if (mode === "profiles") {
      return [
        ...
        { title: "Scope", field: "campaign_scope_label", minWidth: 160, formatter: textCellFormatter },
  ...
  function benchmarkTableInitialSort(mode) {
    if (mode === "runs") {
      return [{ column: "balanced_accuracy", dir: "desc" }];
    }
    return [{ column: "best_ba", dir: "desc" }];
  }

  function benchmarkTableData(mode, filtered) {
    return mode === "profiles" ? filtered.profiles : mode === "runs" ? filtered.runs : filtered.datasets;
  }
  ...
  function destroyTable(key) {
    ...
    state.tableModes[key] = null;
  }

  function ensureBenchmarkTable(mode, initialData) {
    if (state.tables.benchmark && state.tableModes.benchmark === mode) {
      return { table: state.tables.benchmark, isNew: false };
    }
    destroyTable("benchmark");
    const table = new Tabulator(byId("tn-benchmark-table"), {
      layout: "fitColumns",
      pagination: true,
      paginationSize: mode === "runs" ? 20 : 12,
      movableColumns: true,
      placeholder: "No rows match the current filters.",
      initialSort: benchmarkTableInitialSort(mode),
      columns: benchmarkTableColumns(mode),
      data: initialData,
      rowClick(_event, row) {
    ...
    state.tableModes.benchmark = mode;
    return { table, isNew: true };
  }

  function renderBenchmarkTable(filtered) {
    const mode = state.benchmarkFilters.tableMode;
    const data = benchmarkTableData(mode, filtered);
    const { table, isNew } = ensureBenchmarkTable(mode, data);
  ...
  function routerMetricRow() {
    const router = state.autoRouter || {};
    const rows = router.metrics || [];
    return rows.find((row) => String(row.slice || "").startsWith("V25")) || {};
  }

  function renderAutoRouterMetrics() {
    const container = byId("tn-router-metrics");
    if (!container) {
      return;
    }
    const router = state.autoRouter || {};
    const meta = router.metadata || {};
    const row = routerMetricRow();
    if (!meta.available) {
      container.innerHTML = [metricCard("Auto Router", "Unavailable", meta.message || "No router bundle was published.")].join("");
      return;
    }
    container.innerHTML = [
      metricCard("Router", meta.router_version || "V25", meta.selected_model_family || "Selected model family"),
      metricCard("Training Policy Groups", formatInt(meta.training_policy_groups), `${formatInt(meta.training_dataset_count)} datasets`),
      metricCard("BA Delta", formatFloat(row.mean_delta_balanced_accuracy_vs_default, 4), "V25 OOF selected minus default"),
      metricCard("Macro-F1 Delta", formatFloat(row.mean_delta_macro_f1_vs_default, 4), "V25 OOF selected minus default"),
      metricCard("Non-default Picks", formatInt(row.non_default_selected_count), `${formatInt(row.policy_defaulted_count)} policy defaulted`),
      metricCard("Holdout", "Pending", meta.holdout_status || "Independent V25 holdout validation is pending."),
    ].join("");
  }

  function ensureSimpleTable(key, elementId, columns, data, pageSize) {
    if (state.tables[key]) {
      state.tables[key].replaceData(data);
      return state.tables[key];
    }
    const table = new Tabulator(byId(elementId), {
      layout: "fitColumns",
      pagination: true,
      paginationSize: pageSize || 12,
      movableColumns: true,
  ...
  function renderAutoRouterSummaryTable() {
    const router = state.autoRouter || {};
    const rows = router.metrics || [];
    ensureSimpleTable(
      "autoRouterSummary",
      "tn-router-summary-table",
      [
        { title: "Slice", field: "slice", minWidth: 220, formatter: textCellFormatter },
        { title: "Groups", field: "n_policy_groups", hozAlign: "right", formatter: intCellFormatter },
        { title: "BA Delta", field: "mean_delta_balanced_accuracy_vs_default", hozAlign: "right", formatter: percentCellFormatter(4) },
        { title: "F1 Delta", field: "mean_delta_macro_f1_vs_default", hozAlign: "right", formatter: percentCellFormatter(4) },
        { title: "Harm > .01", field: "harm_gt_0_01_vs_default", hozAlign: "right", formatter: intCellFormatter },
        { title: "Severe > .03", field: "severe_harm_gt_0_03_vs_default", hozAlign: "right", formatter: intCellFormatter },
        { title: "Note", field: "note", minWidth: 300, formatter: textCellFormatter },
      ],
      rows,
      6
    );
  ...
  function renderAutoRouterCandidateChart() {
    const chart = ensureChart("autoRouterCandidate", "tn-router-candidate-chart");
    if (!chart) {
      return;
    }
    const rows = ((state.autoRouter && state.autoRouter.candidates) || [])
      .slice()
      .sort((a, b) => Number(a.selected_count) - Number(b.selected_count));
    if (!rows.length) {
      renderEmptyChart(chart, "No V25 candidate rows available.");
      return;
    }
    chart.setOption({
      backgroundColor: "transparent",
      animationDuration: 300,
      grid: { left: 96, right: 28, top: 24, bottom: 40 },
      tooltip: {
        trigger: "axis",
        axisPointer: { type: "shadow" },
        formatter(params) {
          const row = params[0].data.row;
          return `<strong>${escapeHtml(row.id)}</strong><br/>Selected: ${formatInt(row.selected_count)}<br/>Methods: ${formatInt(
            row.method_count
          )}<br/>DF stage: ${escapeHtml(row.df_stage_position)}<br/>Classifier: ${escapeHtml(row.classification_backend)}`;
        },
      },
      xAxis: {
        type: "value",
      ...
      yAxis: {
        type: "category",
        data: rows.map((row) => String(row.id || "").replace(/^v24_/, "")),
        axisTick: { show: false },
        axisLine: { lineStyle: { color: COLORS.line } },
      },
      series: [
        {
          type: "bar",
          data: rows.map((row) => ({
            value: Number(row.selected_count),
            row,
            itemStyle: { color: COLORS.accent },
          })),
          barWidth: 18,
  ...
  function renderAutoRouterDatasetTable() {
    const rows = ((state.autoRouter && state.autoRouter.datasetResults) || []).slice();
    ensureSimpleTable(
      "autoRouterDatasets",
      "tn-router-dataset-table",
      [
        { title: "Dataset", field: "dataset_id", minWidth: 210, formatter: textCellFormatter },
        { title: "Groups", field: "n_policy_groups", hozAlign: "right", formatter: intCellFormatter },
        { title: "Selected BA", field: "selected_mean_balanced_accuracy", hozAlign: "right", formatter: percentCellFormatter(3) },
        { title: "Default BA", field: "default_mean_balanced_accuracy", hozAlign: "right", formatter: percentCellFormatter(3) },
        { title: "BA Delta", field: "mean_delta_balanced_accuracy_vs_default", hozAlign: "right", formatter: percentCellFormatter(4) },
        { title: "F1 Delta", field: "mean_delta_macro_f1_vs_default", hozAlign: "right", formatter: percentCellFormatter(4) },
        { title: "Non-default", field: "non_default_selected_count", hozAlign: "right", formatter: intCellFormatter },
        { title: "Policy Defaulted", field: "policy_defaulted_count", hozAlign: "right", formatter: intCellFormatter },
        { title: "Top Candidate", field: "top_selected_candidate_id", minWidth: 260, formatter: textCellFormatter },
      ],
      rows,
      12
    );
  ...
  function renderAutoRouter() {
    renderAutoRouterMetrics();
    renderAutoRouterSummaryTable();
    renderAutoRouterCandidateChart();
    renderAutoRouterDatasetTable();
  }
  ...
  function tabarenaTableInitialSort(mode) {
    if (mode === "per_dataset") {
      return [{ column: "delta_vs_best", dir: "asc" }];
    }
    return [{ column: "elo", dir: "desc" }];
  }

  function tabarenaTableData(mode, filtered) {
    const dataMap = {
  ...
  function ensureTabarenaTable(mode, initialData) {
    if (state.tables.tabarena && state.tableModes.tabarena === mode) {
      return { table: state.tables.tabarena, isNew: false };
    }
    destroyTable("tabarena");
    const table = new Tabulator(byId("tn-tabarena-table"), {
      layout: "fitColumns",
      pagination: true,
      paginationSize: mode === "per_dataset" ? 12 : 16,
      movableColumns: true,
      placeholder: "No TabArena rows match the current filters.",
      initialSort: tabarenaTableInitialSort(mode),
      columns: tabarenaTableColumns(mode),
      data: initialData,
    });
    state.tables.tabarena = table;
    state.tableModes.tabarena = mode;
    return { table, isNew: true };
  }

  function renderTabarenaTable(filtered) {
    const mode = state.tabarenaFilters.tableMode;
    const data = tabarenaTableData(mode, filtered);
    const { table, isNew } = ensureTabarenaTable(mode, data);
  ...
  function benchmarkReadyStatusMessage() {
    const meta = (state.benchmark && state.benchmark.metadata) || {};
    if (!meta.available) {
      return "HDLSS benchmark browser ready.";
    }
    const tabarenaSuffix = state.tabarenaLoaded ? " TabArena snapshot ready." : " TabArena loads on demand.";
    return `HDLSS benchmark browser ready: ${formatInt(meta.run_count)} runs across ${formatInt(
      meta.profile_count
    )} profiles and ${formatInt(meta.dataset_count)} datasets.${tabarenaSuffix}`;
  }
  ...
  function setActiveTab(tab) {
    ...
    if (tab === "benchmark") {
      renderBenchmark();
      setStatus(benchmarkReadyStatusMessage(), "ready");
      return;
    }
    if (tab === "auto-router") {
      renderAutoRouter();
      setStatus("Auto-router evidence ready.", "ready");
      return;
    }
  ...
  async function loadBenchmark() {
    ...
    state.autoRouter = state.benchmark.autoRouter;
  ...
  function bindBenchmarkControls() {
    [
      ...
      ["tn-benchmark-campaign-scope", "campaignScope"],
