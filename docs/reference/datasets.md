...
- `def extract_meta_features(X: np.ndarray, y: np.ndarray, *, expanded: bool = False, skip_distance_matrix: bool = False) -> Dict[str, float]` (function) - [Source](https://github.com/klokedm/tabnetics-public/blob/main/src/tabnetics/datasets/meta_features.py#L12). Extract dataset meta-features useful for tier assignment and analysis.
