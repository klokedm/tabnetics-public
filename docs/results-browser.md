...
This browser complements the narrative [Benchmark Results](RESULTS.md) and [TabArena Results](TABARENA_RESULTS.md) pages with a fully static, client-side explorer. It runs on GitHub Pages with no backend: the page fetches compact JSON bundles generated from the current public result mirrors at publish time, then renders interactive charts with ECharts and tables with Tabulator directly in the browser.

Use the benchmark tab when you want to slice the HDLSS validation surface by family, campaign, tier, or domain. Use the TabArena tab when you want to inspect the general-tabular comparison snapshot and the per-dataset gap against the current official best method.
...
>
  <div class="tn-browser-shell">
    <section class="tn-browser-hero">
      <div>
        <p class="tn-browser-kicker">Static dashboard · GitHub Pages compatible</p>
        <h2>Interactive result explorer</h2>
        <p class="tn-browser-copy">
          Filter thousands of benchmark runs, inspect profile frontiers, and compare the TabArena snapshot without leaving the public site.
        </p>
      </div>
