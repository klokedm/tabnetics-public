...
**A research component, not a new default.** Diakrino is opt-in on purpose. In controlled tests its ranking is *not* additive to the classical selector union on real HDLSS panels, so it is meant to be combined with the classical stack rather than substituted for it. The [Diakrino results page](https://tabnetics.org/TABENTICS_DIAKRINO_RESULTS.html) has the paired campaign evidence and the limitations, stated plainly. A peer-reviewed article describing the work is in preparation.
...
Part of the **Tabnetics Diakrino** model (see [Tabnetics Diakrino results](https://tabnetics.org/TABENTICS_DIAKRINO_RESULTS.html)) was pre-trained on the EuroHPC supercomputer **MareNostrum 5** within the scope of a EuroHPC Joint Undertaking project:
