...
**First stable release.** Tabnetics moves from beta to production-stable with version 1.0.0, reflecting four completed validation campaigns (Val-18 through Val-21) and a mature pipeline surface. The public benchmark evidence base now covers **56,107 successful runs** across **278 pipeline profiles** on **63 HDLSS datasets**. The best profiles clear **36/63** datasets at balanced accuracy `>= 0.90`, achieve **26** perfect-classification datasets, and land **41 / 21 / 1** in the strict-holdout SOTA split (**above / within / below**).

**Val-21 integrated.** The newest validation campaign (Val-21) adds 7 decision-focused profiles testing winner-bridge, meta-selector, prefilter, and regime variants across 43 datasets. Val-21 results are now included in the interactive results browser and the aggregate evidence base alongside Val-18/Val-19/Val-20.

**Cross-campaign technical report.** A unified technical report covering the full Val-18/Val-19/Val-20 evidence base is available. The report presents matched-overlap paired comparisons with Wilcoxon signed-rank tests, per-tier decomposition, and a pre-registered hypothesis scorecard covering 22 testable hypotheses across the three campaigns. Key findings: BH filtering is strongly validated as a safety feature (disabling costs −0.046 BA on 63 datasets, p < 1e-4), FLAML at 60 s is the cleanest frontier improvement (+0.003 BA, p = 0.017), and the full FS+DF pipeline scaffold remains directionally beneficial.
...
**Import-safety fix.** Module-level `find_repo_root()` calls in the validation subsystem no longer crash when the package is installed via pip outside a source checkout.
