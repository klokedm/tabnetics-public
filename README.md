...
The goal is to make sensible Tabnetics usage the default path: users should not have to know which validation-campaign flags won on which dataset family before trying the library. See [Auto Router](https://tabnetics.org/AUTO_ROUTER.html) for the model card, evidence summary, opt-out flag, and direct inspection API.
